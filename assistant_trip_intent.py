...
def model_trip_intent(config, prompt, candidates, *, invoke, selected_ref=None):
    ...
    payload = {'max_output_tokens': 1800,
        ...
        'change只含kind,days,startDate,monthDay；kind是shift_days、start_date或unspecified，未提供字段用null。'
        '推迟三天为days=3，提前三天为days=-3。只使用明确的YYYY-MM-DD或YYYY年M月D日转写startDate。'
        '仅M月D日使用monthDay=MM-DD并让startDate为空，不猜年份；缺参数或矛盾时kind=unspecified。'
