...
NVIDIA配置工具在独立分支通过审查，并经组合复审合入main `18f30dd`，只新增工具/测试/说明三文件，原295源文件与74运行文件不变。工具尚未在生产写入配置，源码合并不等于服务采用新配置。Google Photos OAuth/协议、图片净化/加密仍在独立已审候选；精选/逐台电视许可、库存和完整成员关系尚未接线。电视先按浏览器实现，型号稍后补充。
