...
## 当前候选交接：Expo 财务，已验证未部署

冻结 main `a2081051657647564345a54b61662682ff4dd663` 与 integration／应用来源 `2d08258fc67388e4de237747eb35db3fe3b0d8cf` 同树 `9508cddb1b4cdde1caf70c268673110a0bd50194`。各作者在独立分支实现主页面、导入面板、持久回执、迁移、测试和文档，经非作者审查与组合验证后合入 main。本文是该源码之后的文档草稿，不改变受验源码或证明生产已发布。
...
本候选新增 `hub_import_receipts`：每户 53→54 表，平台注册库仍两表；旧 `hub_imports` 七列保持，新交易保存首次批次来源，重复／冲突不覆盖。下一步由集成人按 [回执迁移与完整恢复](FINANCE-RECEIPTS-RELEASE.md) 完成新镜像 Linux 验收、实际完整备份、生产迁移与正常 TLS 读回，之后才更新顶部线上身份。不能在当前 53 表生产上重放历史迁移，也不能把候选的 54 表索引当成生产状态。
