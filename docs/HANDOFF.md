...
`/app/finance` 提供共同资金、本人完整账本与月预算；从「导入账单」完成选文件、工作表／金额列、逐行预览和明确确认，按实际结果月份回到账本。交易详情可查看首次来源并核对分类／共享，订单、付款、退款和重复关系采用候选→金额预览→确认→可撤销。未知写入先读回，版本冲突保留草稿；完整身份在私有请求前后复核，后台／离线隐藏。使用与实现见 [Expo 财务](EXPO-FINANCE.md)。本人持仓与资产来源报告已迁入 Expo；来源更新继续保留经典入口，新的文件预览／确认面板单独验收。
