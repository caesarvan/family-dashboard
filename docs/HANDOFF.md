...
**最新上线：Expo 采购实付核对（2026-09-18 08:03:15，北京时间），08:04:16 正常 TLS 读回通过。** 从采购项「核对实付」或本人交易详情「核对采购实付」进入，明确预览后只共享采购实付与完成状态。58→58 原数据和角色保持；独立只读发布审计通过。原件、固定安装身份与未验范围集中在[采购实付验收](EXPO-SHOPPING-SETTLEMENT-ACCEPTANCE.md#expo-shopping-settlement-release)，下文既有发布历史保留。
