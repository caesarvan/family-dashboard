...
稳定主线基线 `904fde5665b6f5063ac90f9c752556a70daeb108`；本次组合 `add881ed6a7d455e2743ce2affa6cf8b931b47d2` 包含 API `3794a8a`、UI `6fd938d`、冻结合同和迁移初始化接入。手机路径为保存的旅行 → 旅行回顾 → 在电视回顾 → 选设备／可选共享路线 → 核对电视内容 → 明确开始；[候选使用说明](TV-TRIP-USER-GUIDE.md)记录 route-only、恢复与撤权边界。

当前生产仍为下节助理清单 `e8f0427`，75/9、五服务不变；本地新装候选77/9不代表线上迁移。本人完整A–D仍0/4，实体电视待验。本地 API 94个唯一用例分轮最终通过并独审；UI 40个唯一Node用例分轮最终通过及两组类型检查独审，旧失败保留。组合 fresh npm ci、双tsc、Expo导出23文件实际退出0；schema迁移32、完整API接缝定向5、本地演练初版10分别通过，不相加为一次全量或Linux结果。浏览器结果尚待固定终态与非作者审查；不得凭导出或本地演练宣称Linux候选、生产迁移或恢复已通过。证据入口及固定build SHA见[使用说明的验证范围](TV-TRIP-USER-GUIDE.md#verification)。
