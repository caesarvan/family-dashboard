...
稳定主线基线 `904fde5665b6f5063ac90f9c752556a70daeb108`；本次文档基于已审组合 `2bb5fecd5ec4fbdfffe714a5498ef3c31f2c86b2`。手机路径为保存的旅行 → 旅行回顾 → 在电视回顾 → 选设备／可选共享路线 → 核对电视内容 → 明确开始；[候选使用说明](TV-TRIP-USER-GUIDE.md)记录 route-only、未知结果恢复及撤权边界。

当前生产仍为下节助理清单 `e8f0427`，75/9、五服务不变；候选77/9不代表线上迁移。真实本地双浏览器已由R3第二场景与R4第一场景分轮覆盖两条唯一流程，并经非作者原件与视觉审查；R3整轮仍为失败，早轮原件保留。实际浏览器源 `526a078`／`6853e06` 的运行代码仍为 `add881ed`；后续TVP `dc275dc` 元数据读取优化另有24项真实Flask／SQLite及根独审，不能写成浏览器同头验证。

原 `add881ed` fresh npm ci、双tsc与23文件Expo导出通过，179项构建输入至上述组合逐字一致。TVL `818c34e` 已绑定 `aa55f3e` 运行代码并通过实际输入准备独审，Linux尚未执行；最终118节点选择与容量证据由独立发布增量继续绑定，本页未记Linux通过数量。API94、UI40、schema32、完整API接缝5及本地演练各轮分别留证，不累加为一次全量。固定身份与原件索引见[候选验收](TV-TRIP-ACCEPTANCE.md#tv-trip-candidate)。A–D本人完整验收仍0/4，真实云与实体电视另验。
