...
旅行详情／财务均可进入「我的旅行费用」，仅归集本人已有 CNY 净付款；共享预算并列，不写旅行共享已付或原交易。候选源码 `302a62d` 已完成实际 Linux 46/46、零跳过及隔离两户三库73→75十阶段演练；构建／浏览器使用固定 `0ad4e41`，浏览器 R2 3/3已有非作者原件审查，R1两项工具失败保留。Linux 与迁移执行的非作者审查尚待完成，不能由工具源码审查代替。完整身份、最短操作与原件见[候选集中验收](JOURNEY-FINANCE-ACCEPTANCE.md#journey-finance-candidate)。

生产尚未执行本批计划，仍为 `53ea3fd`／73/9／五服务；本次候选文档不代表已上线75表，也不改变本人A–D完整验收0/4。下方已发布批次保持原身份。

## 最新发布：通用账单收支方向列

2026-09-21 00:26:30（北京时间）激活，00:27:04只读回读通过；非作者生产审计已通过。实际安装source仍为固定53ea，文档提交不会改变运行包。入口为「财务 → 导入 → 通用支付流水 → 收支方向（可选）→ 按所选列预览 → 确认导入 · 仅本人」。改列必须重预览；若保存结果不明，点击「核对保存结果」；不要重新发起同一导入。同文件不双记、不覆盖已有方向。固定身份、分轮验证和生产结果集中在[本批验收](FINANCE-FLOW-ACCEPTANCE.md#finance-flow-release)，发布合同见[固定适配](FINANCE-FLOW-RELEASE.md)。
