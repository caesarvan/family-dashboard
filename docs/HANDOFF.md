...
**最新上线：个人账户与多家庭成员协作（2026-09-18 13:51:39，北京时间）。** 注册及绑定、邀请加入、移除与退出、多家庭切换已激活，实际一户两库完成 58/2→61/9 迁移及启动保全；个人数据不自动共享。操作、固定运行身份与验证边界见[本轮发布](MEMBERSHIPS-RELEASE-R3.md)。此前失败和完整库组恢复记录保留；下方为各历史版本的当时状态。

**最新上线：Expo 助理查找已有旅行（2026-09-18 09:30:34，北京时间），09:31:16 正常 TLS 读回通过。** 显式搜索后打开原旅行，返回保留查询页并重新读取；R3 六组／四图、经典浏览器独立单项与 Linux 166 通过＋1 精确 Windows 跳过分开验收。独立只读审计通过，09:33:38 fresh 读回一致。首轮失败、测试拆分、安装身份及审计边界见[集中验收记录](EXPO-ASSISTANT-TRIP-SEARCH-ACCEPTANCE.md#expo-assistant-trip-search-release)。首页待办编辑入口属于独立后续候选，不在本批安装范围。
