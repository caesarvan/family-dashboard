...
**最新上线：财务列映射（2026-09-18 06:56:39，北京时间），06:57:18 正常 TLS 读回通过。** 非作者独立审计通过。 「家庭资金 → 导入账单 → 通用表格 → 手动指定列」沿用本人预览、原请求确认与历史回执核对。实际安装身份、分轮原件和待验项集中在[本批验收](FINANCE-COLUMN-MAPPING-ACCEPTANCE.md)，操作见[四列映射](EXPO-FINANCE-COLUMN-MAPPING.md)。本批已完成包含现有角色值的 58→58 完整保全，不重放成员迁移。
