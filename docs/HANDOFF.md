...
部署人员分清 [空服务器安装](DEPLOYMENT.md#3-空服务器首次安装) 与已有数据更新。当前为账户 58 表，已完成的 55→58 及更早固定迁移均不能重放；下一次更新须绑定实际 58 表基线和镜像／清单。完整恢复边界见 [账户迁移](FINANCE-ACCOUNTS-MIGRATION.md)。
