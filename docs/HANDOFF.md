...
本批开发基线 `904fde5665b6f5063ac90f9c752556a70daeb108`；本批实际运行源码为 `d4346d34ed5271699f2cffac7c29aa098edc8a07`，固定包 `d1784a3d…`、镜像 `3c55bfba…`；后续交付文档不改变该身份。手机路径为保存的旅行 → 旅行回顾 → 在电视回顾 → 选设备／可选共享路线 → 核对电视内容 → 明确开始；[使用说明](TV-TRIP-USER-GUIDE.md)记录 route-only、未知结果恢复及撤权边界。

**已发布。** 2026-09-21 08:03:18（北京时间）激活、08:03:49只读回读通过；生产非作者独审已通过，32份安全回执已核。实际一户两库完整备份后75→77、平台9不变；旧75表、settings及序列保留，新两表在app-only初始化后停止核对时为空，迁移后与停止态摘要一致。1279安装文件、三个应用服务各137运行文件及decoder原4文件核对通过；五服务running、无OOM或重启，app／decoder健康，HTTPS证书正常、timer active、环境文件0600。本次未做生产恢复；只读回查不读取活库或重验备份SQLite字节。

真实本地双浏览器已由R3第二场景与R4第一场景分轮覆盖两条唯一流程，并经非作者原件与视觉审查；R3整轮仍为失败，早轮原件保留。实际浏览器源 `526a078`／`6853e06` 的运行代码仍为 `add881ed`；后续TVP `dc275dc` 元数据读取优化另有24项真实Flask／SQLite及根独审，不能写成浏览器同头验证。

原 `add881ed` fresh npm ci、双tsc与23文件Expo导出通过，179项构建输入由最终包校验复用。新镜像实际Linux118/118、零跳过，137运行文件与60个 `/app` 根模块导入核验通过。四个真实Flask WSGI元数据请求在384MiB／零swap下同时进行并全部返回200：2000条合成媒体、100站、两台电视，累计cgroup峰值225.3MiB，max/OOM/kill均0，测量请求不读媒体BLOB；不是Gunicorn/Nginx吞吐或混合媒体峰值。TVL `818c34e` 以 `aa55f3e` runtime闭合绑定同一候选镜像，实际隔离两户三库75→77／平台9的十阶段已退出0，迁移非作者独审通过；含真实0400失败、部分77／75拒重放、完整75与非空77恢复。API94、UI40、schema32、完整API接缝5及本地演练各轮分别留证，不累加为一次全量。固定身份与原件索引见[本批验收](TV-TRIP-ACCEPTANCE.md#tv-trip-release)。A–D本人完整验收仍0/4，真实云与实体电视另验。

## 上一版发布：助理清单调整与结果恢复

**已发布。** 2026-09-21 05:41:15（北京时间）激活，05:41:40只读回读通过。待办／采购草案逐项调整分工、日期和预算；未知响应按原计划核对回执，刷新后先核验身份再恢复；首页采购保留负责人、日期和优先级。
...
运行源码与前端构建均固定 `e8f0427`。后续两次浏览器工具修正只改变测试工具及说明，未重建运行包；手机R1和桌面R3分轮通过，旧整轮失败保留。Linux55/55、真实Flask/Node12条场景及发布边界见[助理清单验收](ASSISTANT-LIST-ACCEPTANCE.md#assistant-list-release)。接口见[清单契约](ASSISTANT-LIST-EDITING.md)，后续更新按[固定发布合同](ASSISTANT-LIST-RELEASE.md)准备全新计划；本次计划已消费，不能重放。
...
固定应用源码 `5fae5c5`，前端构建源 `75a79cd`；Linux 精确 55/55、零跳过，手机 R1 与桌面 R2 分轮完成，失败调用及修正均保留。运行内容与后续纯文档提交分别记录；本批没有 DDL，不重放此前 73→75 迁移或本次已消费计划。入口、完整身份、测试与原件见[集中验收](MEDIA-CONFIRMED-DATE-ACCEPTANCE.md#media-date-release)，接口见[日期契约](MEDIA-CONFIRMED-DATE.md)，后续发布先读[固定适配](MEDIA-CONFIRMED-DATE-RELEASE.md)。

## 此前发布：本人旅行费用归集

旅行详情／财务均可进入「我的旅行费用」，仅归集本人已有 CNY 净付款；共享预算并列，不写旅行共享已付或原交易。实际源码 `302a62d` 于2026-09-21 02:38:33（北京时间）激活，02:38:56只读回读通过，非作者生产独审已通过。Linux 46/46、零跳过及两户三库73→75十阶段演练均已实际执行并经非作者审查；构建／浏览器使用固定 `0ad4e41`，R2 3/3已有原件独审，R1两项工具失败保留。完整身份、最短操作与原件见[集中验收](JOURNEY-FINANCE-ACCEPTANCE.md#journey-finance-release)。

生产一户两库已完成73→75／平台9不变：完整备份、旧73表／行／序列保全、新两表在app-only停止核验时为空。1222安装文件、三个应用服务各136运行文件及decoder原4文件核对通过；五服务运行且无OOM／重启，app／decoder健康，HTTPS与timer正常。计划已消费，不能重放；本次未运行生产恢复，也不改变本人A–D完整验收0/4。下方各批保留历史身份。

## 此前发布：通用账单收支方向列

2026-09-21 00:26:30（北京时间）激活，00:27:04只读回读通过；非作者生产审计已通过。该批实际安装source为固定53ea；后续旅行费用发布已替换，原文档与回执保留其历史身份。入口为「财务 → 导入 → 通用支付流水 → 收支方向（可选）→ 按所选列预览 → 确认导入 · 仅本人」。改列必须重预览；若保存结果不明，点击「核对保存结果」；不要重新发起同一导入。同文件不双记、不覆盖已有方向。固定身份、分轮验证和生产结果集中在[本批验收](FINANCE-FLOW-ACCEPTANCE.md#finance-flow-release)，发布合同见[固定适配](FINANCE-FLOW-RELEASE.md)。

生产一户两库完整备份和app-only停止态全表／settings／序列保全通过，无DDL；1198安装文件、三个应用服务各135运行文件及decoder原4文件核对，五服务运行、app／decoder健康、HTTPS与timer正常。计划已消费，不可重放。

本批保持73/9及五服务，只改财务模块和前端导出；媒体资源与旧73→73恢复演练仅在未变路径范围继承。没有新的财务资源负载或生产恢复证明，本人A–D仍0/4，实体设备另验。

## 此前发布：助理地点入口与重复照片提示

2026-09-20 23:06:47（北京时间）激活，23:07:24生产只读回读通过；非作者生产审计已通过。固定source `63dc634`／manifest `7b77dfb`／应用镜像 `b63c1f5`，完整身份和原件见[本批集中验收](DISCOVERY-ACCEPTANCE.md#discovery-release)。本次是73→73五服务源码更新：一户两库完整备份、app-only停止态全表／settings／序列保全通过，无DDL或迁移；1188安装文件、三个应用服务各135运行文件及decoder原4文件核对，五服务运行、app／decoder健康、HTTPS正常TLS与备份timer恢复。计划已消费，禁止重放。

用户从「家庭助理 → 搜索：地点名称 → 在地图查看」往返原旅行／照片，返回保留查询和页码；从本人已保存照片详情主动「查找重复照片」，分页打开原照片并返回提示。展示副本一致不等于原图一致，不自动合并、删除、共享或授权电视；身份／撤权／离线清除提示，有草稿沿原离开确认。

分别保留455源的125项Linux、6d50源R2浏览器前五流程；455源R3另补验一个受影响浏览器流程。63dc的运行文件、完整前端和构建输入逐字节相同，继承这些浏览器证据并另执行9项Linux，不能改写为63dc新跑浏览器或相加为一次全量。重复提示资源只覆盖四个元数据请求；有效媒体路径资源与73→73两户三库恢复演练沿已审父版继承，没有本次新增混合负载或生产恢复证明。构建loader、预览500和资源R1失败原件均保留。

本批源码合入main `1d589e6d401d2d760f7361a2cab36839841a7208` 时，比实际包source多README和三份discovery后验文档，运行源码相同；后续纯文档提交仍不改变已安装身份。财务收支方向[PR #47](https://github.com/caesarvan/family-dashboard/pull/47)代码已合入，生产状态以文首财务批次为准。本人A–D仍0/4，真实账号、手机与实体电视另验。

## 此前发布：设备照片与那年今日

2026-09-20 20:04:27（北京时间）激活，20:05:10生产只读回读通过；随后非作者复核通过。固定应用source `79e5ef9`、新镜像 `ede2e6f`，完整身份和证据集中在[本批验收](LOCAL-PHOTO-ACCEPTANCE.md#local-photo-release)。191项实际Linux、设备照片R3完整三流程／九图、那年今日分轮三流程及独立R3资源验证已完成；真实73→73两户三库演练的正常和注入漂移场景通过，非作者执行复核已通过。生产一户两库保持73/9，1156安装文件、app／sync／media各135运行文件与decoder原4文件核对通过；五服务运行，app／decoder健康，备份timer active。实际激活与Git合入分别留证。

入口为「相册 → 选择照片 → 设备照片 → 上传并生成预览 → 明确保存」，默认私有，共享和逐台电视许可分别确认。本地文件日期未知，不将修改时间伪装成拍摄日期；「那年今日」仍只回看本人有可靠来源日期的往年同日静态照片。后端由 `media_local_upload.py` 与原媒体状态机负责固定批次／slot／幂等、净化加密和来源权限；助理搜索响应前重新读取媒体原行，前端沿原身份保护与确认接口。

本批保持73/9和五服务，decoder镜像／原4文件不变；原全组备份、app-only停止后严格保全和失败后显式完整组恢复仍由固定发布控制器执行。没有71→73迁移，不重放旧视频计划。资源R1 OOM、R2触限、浏览器R1／R2及首次构建失败均保留；真实手机与实体TV仍待验收；iCloud／NAS实时连接和本地视频上传仍待实现。
...
新接缝为独立解码服务、私密加密视频副本和设备播放进度；下一次发布须适配当前 73/9 及五服务，禁止重放本次已消费计划或旧四服务迁移。R5 控制器正常／部分失败完整组恢复已在隔离场景实际通过；生产本次未做恢复。该视频批次当时不含设备本地照片和「那年今日」；它们的后续发布以文首及集中验收为准。
...
**2026-09-20 06:28:25（北京时间）激活成功，06:35:47 独立生产只读审计通过。** 正式 source `456585bc1a4f820ca238b3308f09a8d591628f41` 与 main `40ea731b28510f7bd7f3db3d0e06fade62a530f8` 同树 `65a93964e5f20088a8cd0bad878fd03c84dccc94`。使用、完整包／计划身份、生产审计及原件定位集中在[任务依赖验收](TASK-DEPENDENCIES-ACCEPTANCE.md#task-dependencies-release)；已消费计划禁止重放。真实一户两库保持 69/9，无 DDL 或旧任务批量回填。

`task_dependencies.py` 负责本地任务图和完成门禁；原 CRUD、旅行重建／改期、例行事项、助理、导出及本地任务发布沿同一实体和权限接入。前端由 `taskDependencies.ts`、`TaskDependencyFields.tsx`、ItemEditor／ListScreen／HomeScreen 展示与编辑，原 ID／revision 提交，失败保留草稿。服务端是最终完成判断依据；云镜像本身不支持前置依赖，真实云写入仍待明确目的地验证。

后端作者专项 36 与相邻 284、组合 70、正式两组 TypeScript／64 Node／23 导出、R1 前两项＋R4 第三项浏览器及实际 Linux 354 项分别记录，存在重叠，不相加为一次全量。R1/R2 工具定位失败和 R4 关闭阶段异步警告保留。生产核对 1007 安装文件、三个应用服务各 128 运行文件、75 个 TLS 资源；四服务正常，原 69/9 全表与两份停机备份字节核验通过，未做生产恢复。

该任务依赖历史批次不包含站内提醒；提醒的后续证据与当前状态见文首集中入口。

## 此前发布：助理资料与照片搜索

2026-09-20 05:15:29（北京时间）激活，05:16:46 独立只读审计通过；正式 source `fd3ef3692528a18e3e6afc779fb15efe8463d559`，main `7ff58c446f3396d13e69ad4668e8df6fb01b5daa` 同树。资料／照片搜索返回原页码，重新核权打开原内容，本人未关联资料和共享撤销边界保持；包含采购 HelperText 修正。R1／R2 分段浏览器与 Linux 175、一户两库 69/9 的当时证据见[集中验收](ASSISTANT-DOCUMENT-SEARCH-ACCEPTANCE.md#assistant-document-search-release)。该历史包不包含任务依赖；后续纯文档不改变任何固定包身份。
...
本轮将采购日期／优先级接到普通清单、旅行编辑、助理草案和明确改期，保持 live 实体清空优先、旧字段兼容及原实体身份。正式新构建 106 Node／两组 TypeScript、4 浏览器流程／8 图、一次真实模型的 29 条检查和实际 Linux 298 项分别通过，保持 69/9，无自动回填或云写入。该采购批次当时的 390px 离线 HelperText 裁切已随 05:15 的资料搜索发布修复；旧验收事实保留，资料搜索未包含在当时的采购包中。A–D 本人完整验收仍为 0/4，实体电视缺设备。
...
[PR #5](https://github.com/caesarvan/family-dashboard/pull/5) 已合入 main `b35a141af0822b980891a5a3d07ba1a8b90ff50b`；生产包绑定源码 `8f7af253261f28269465f3a77b6c8433f2dc8142`。本批不改业务后端、schema、Docker 或 env，104 个非 Expo 运行文件与父版一致；一户两库完整保全，保持 69/9。本次计划已消费，不可重放；后续纯文档提交不改变已安装包身份。

正式 R2 构建的两组 TypeScript、53 Node、23 产物，R3 临时真实浏览器八条流程／六图，Linux 待办 API 61 项及发布策略 48 个唯一用例分别通过。浏览器 R1 刷新竞态、R2 长路径导致零业务流程执行、SCP R1 传输失败及策略测试的长路径重跑均保留原件；各轮不累加。模拟提供方不能证明真实 Microsoft／Google 云写入，实体电视和 A–D 本人完整验收仍为 0/4。

实现接缝为 `JourneyTasksPanel.tsx`、`taskPublish.ts` 及原旅行导航保护；定时读取期间禁用修改操作，回到前台先重读，迟到响应须重新核验成员与家庭。采购的结构化截止、优先级与改期联动是独立候选；其后端 229／Node 104 结果不属于本次发布。
...
旅行需求中的准备、采购和分工已接通。采购结构化截止日期与改期联动仍为独立候选，未包含在本次安装包，不能声称采购已随旅行改期；原请求回执、版本核对与未知结果恢复约束继续保留。
...
**此前上线：采购关联库存与首页本地待办编辑（2026-09-18 15:12:56，北京时间）。** 15:14:36 独立只读审计通过，生产实际一户两库保持 61→61 / 9→9，原成功成员迁移 marker 保留。真实浏览器采用 R2/R3/R4 分轮证据，不能写成一次 5/5；Linux 47/47 与两户三库合成演练分别通过。本人真实库存、云端和实体电视尚待验收；本次发布后的纯文档提交不改变运行包。固定身份、失败历史与线上审计集中在[本批验收](SHOPPING-INVENTORY-ACCEPTANCE.md#shopping-inventory-release)，发布步骤见[普通更新](STEADY-RELEASE.md)。

**此前上线：个人账户与多家庭成员协作（2026-09-18 13:51:39，北京时间）。** 注册及绑定、邀请加入、移除与退出、多家庭切换已激活，实际一户两库完成 58/2→61/9 迁移及启动保全；个人数据不自动共享。操作、固定运行身份与验证边界见[本轮发布](MEMBERSHIPS-RELEASE-R3.md)。此前失败和完整库组恢复记录保留；下方为各历史版本的当时状态。

**此前上线：Expo 助理查找已有旅行（2026-09-18 09:30:34，北京时间），09:31:16 正常 TLS 读回通过。** 显式搜索后打开原旅行，返回保留查询页并重新读取；R3 六组／四图、经典浏览器独立单项与 Linux 166 通过＋1 精确 Windows 跳过分开验收。独立只读审计通过，09:33:38 fresh 读回一致。首轮失败、测试拆分、安装身份及审计边界见[集中验收记录](EXPO-ASSISTANT-TRIP-SEARCH-ACCEPTANCE.md#expo-assistant-trip-search-release)。该 09:30 版本不含首页待办编辑；最新状态见顶部本批记录。

**此前上线：Expo 采购实付核对（2026-09-18 08:03:15，北京时间），08:04:16 正常 TLS 读回通过。** 从采购项「核对实付」或本人交易详情「核对采购实付」进入，明确预览后只共享采购实付与完成状态。58→58 原数据和角色保持；独立只读发布审计通过。原件、固定安装身份与未验范围集中在[采购实付验收](EXPO-SHOPPING-SETTLEMENT-ACCEPTANCE.md#expo-shopping-settlement-release)，下文既有发布历史保留。

**此前上线：财务列映射（2026-09-18 06:56:39，北京时间），06:57:18 正常 TLS 读回通过。** 非作者独立审计通过。 「家庭资金 → 导入账单 → 通用表格 → 手动指定列」沿用本人预览、原请求确认与历史回执核对。实际安装身份、分轮原件和待验项集中在[本批验收](FINANCE-COLUMN-MAPPING-ACCEPTANCE.md)，操作见[四列映射](EXPO-FINANCE-COLUMN-MAPPING.md)。本批已完成包含现有角色值的 58→58 完整保全，不重放成员迁移。

**此前上线：家庭与成员（2026-09-18 05:49，北京时间）。** 从「更多 → 家庭与成员」调整另一成员的角色或让其当前浏览器全部退出；对方仍可重新登录，个人数据与电视许可不变。现有两位成员初始化为共同管理员。使用与实际验收见[家庭与成员](EXPO-HOUSEHOLD-MEMBERS.md)及[集中验收记录](EXPO-HOUSEHOLD-MEMBERS-ACCEPTANCE.md#household-members-release)。通用加入／退出、多家庭成员关系仍未实现；实体设备仍未验收。
...
**此前上线：单趟旅行回顾（2026-09-18 01:14，北京时间）。** 已完成旅行计划的旅行，可在详情打开「旅行回顾」，切换「行程安排」「关联地点」「旅行照片」，只读取当前有权查看的内容。返回时重新读取原旅行；不会自动共享、推断路线或增加电视许可。本地十条流程／十二图及服务器 180 项通过、1 项精确 Windows 跳过分别验证，58→58 原数据保持，01:15:15 正常 TLS 读回完成。使用见 [旅行回顾](EXPO-TRIP-RECAP.md)，完整身份与证据见 [发布验收](EXPO-TRIP-RECAP-ACCEPTANCE.md#expo-trip-recap-release)。独立审计通过；本人真实资料、云与实体电视仍未验收。

**此前上线：本人资产账户与历史估值（2026-09-18 00:23）。** 从「财务 → 我的资产账户」记录资产或负债，按日期查看原币估值，支持归档、恢复与个人导出。账户明细仅本人可见，金额不与持仓、来源报告或公共荷包重复相加。12 条本地浏览器流程及服务器 207 项检查通过，另 1 项为 Windows 专属跳过；58 表升级和原数据保留、HTTPS 与新备份已核验。操作见 [资产账户](EXPO-FINANCE-ACCOUNTS.md)，完整身份和证据见 [发布验收](EXPO-FINANCE-ACCOUNTS-ACCEPTANCE.md#finance-accounts-release)。实体电视待设备可用后验收。

**此前已发布：Expo 资产来源导入，2026-09-17 23:11:09（北京时间）激活，23:11:48 正常 TLS 读回通过。** 从「财务 → 更新资产来源」选择或粘贴来源 JSON，预览覆盖范围和记录后明确确认；未知保存结果可按原编号核对。本人数据、共同汇总范围和金额算法保持。 安装 main `e0e3beb33b1b419dd68583e89d03df8cd97ed25a`／source `3c113a9fab65910fd0a938e0105e142a058ce2e8`，同树 `4ea5a46328f16af86d814d009ac561fa2eadafed`。R2 本地 12 条流程／12 图与 Linux 183 通过、唯一精确 Windows 跳过分别验收；672 包文件、114 运行文件、75 HTTPS 资源和 34 个唯一匿名接口读回通过，55→55 原数据保持。 使用见 [来源导入](EXPO-FINANCE-SOURCE-IMPORT.md)，完整证据见 [发布验收](EXPO-FINANCE-SOURCE-ACCEPTANCE.md#expo-source-release)。后续纯文档不改变安装包身份。该来源版本当时未包含手动资产账户；后续发布见文首，实体电视仍待验收。

**此前已发布：Expo 本人资产与来源报告，2026-09-17 21:58:49（北京时间）激活，21:59:22 正常 TLS 读回通过。** 安装 main `0bd330a6203f968c15f43bd03fae7df5623af715`／source `f0317fb6038d0b177f35376479ddeee4b787c3f3`，同树 `f9f14984abef8318424133771cfbaa4c145f2c28`。六分类报告与当前真实会话读取、来源导入会话复核及精确历史回执查询已上线；无新表或路由。R2 本地九组流程／十二图及 Linux 347 通过、唯一精确 Windows 跳过分别验收；661 包文件、114 运行文件、75 HTTPS 资源和 34 个唯一匿名接口读回通过，55→55 原数据保持。完整证据见 [发布验收](EXPO-BASELINE-ACCEPTANCE.md#expo-baseline-release)。该 21:58 安装包未包含新版来源文件预览／确认 UI，后续按文首记录发布；实体电视仍待本人可用时验收。发布后纯文档不改变已安装身份。

**此前已发布：Expo 旅行分段编辑，2026-09-17 21:12:18（北京时间）激活，21:12:47 正常 TLS 读回通过。** 安装 main `f97de964815fe88da016ac55bb41b4bc327d3022`／source `66df7ff0149a44686912d1ffac7da31112a1510b`，同树 `3107968b667bbce7e488fc8c90c9e51cb0312986`。15 组本地流程／19 图及 Linux 260 通过、唯一精确 Windows 跳过分别验收；646 包文件、114 运行文件、75 HTTPS 资源和 29 个匿名接口读回通过，55→55 数据保持。21:16:20 独立只读复核通过；完整身份、来源选择修正和范围见 [发布验收](EXPO-JOURNEY-SEGMENTS-ACCEPTANCE.md#expo-segments-release)。后续文档不改变安装包；个人资产报告随后按文首记录发布，实体电视仍未验收。
...
下一位 agent 继续从集成人分配的独立分支和 worktree 开始；该历史版本发布当时需绑定 55 表；后续须按文首重新绑定当前 58 表及实际镜像／清单，不可重放本次 54→55 或更早迁移。已合入模块不重复复制或应用，发布／完整恢复边界见 [持仓发布](HOLDINGS-RELEASE.md)。
...
`/app/finance` 提供共同资金、本人完整账本与月预算；从「导入账单」完成选文件、工作表／金额列、逐行预览和明确确认，按实际结果月份回到账本。交易详情可查看首次来源并核对分类／共享，订单、付款、退款和重复关系采用候选→金额预览→确认→可撤销。未知写入先读回，版本冲突保留草稿；完整身份在私有请求前后复核，后台／离线隐藏。使用与实现见 [Expo 财务](EXPO-FINANCE.md)；[通用四列映射](EXPO-FINANCE-COLUMN-MAPPING.md)已发布，原自动识别、工作表／金额列选择仍兼容。本人持仓与资产来源报告已迁入 Expo；来源文件预览／确认已于 23:11 发布，见 [来源导入验收](EXPO-FINANCE-SOURCE-ACCEPTANCE.md#expo-source-release)。
