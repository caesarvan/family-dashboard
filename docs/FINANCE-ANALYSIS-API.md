...
新增刷新请求的截止日不得晚于北京时间（Asia/Shanghai）今天，超出则在出网前返回 400，提示调整日期。北京时间凌晨 00:00–07:59 选择本地今天时，实际公开数据请求的截止日裁到 UTC 今天；请求原始日期、幂等编号及报告原币估值日期保持不变，显示汇率仍保留实际观测日期。原成功回执优先恢复，不因当前日期边界或时钟变化重新出网；真正的公开来源日期参数错误返回 400，不伪装成网络故障。
...
摘要同时绑定原账户 revision 和涉及本区间的现金流变更历史；账户改名、归档／恢复等版本变化也保守地使核对失效。现金流新增后删除、移出后移回不会使旧确认自动恢复；始终在区间外的现金流变更不影响本区间。更改参考汇率只重新计算折算，不代替本人重新核对原币资料。
