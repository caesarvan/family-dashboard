...
刷新后原Token已丢失且回执为`found:false`时，读回该电视当前状态后可提供用户主动的“重新预览”。点击仅结束本地旧恢复记录并建立新预览；必须再次明确确认才提交新开始，不自动重试或自动播放，不宣称旧请求已失败或已取消。两个尚未提交的预览若绑定相同播放revision，无论提交顺序都只有先提交者成功，另一请求必须409且不得写回执；若原请求已提交后才生成新预览，则新的明确确认属于新的用户意图。浏览器和API验证须覆盖这个竞态，避免丢失Token导致入口永久卡住。
