...
状态按 `updated_at DESC` 取实体，之后分组；同时间不承诺稳定顺序。客户端应自行按日程时间等排序。响应不会包含个人月度财务、私有账户明细、OAuth 邮箱/令牌、密码和财务导入私有原文。`events` 按当前会话的可见范围过滤：新建普通本地日程默认仅创建者可见，明确共享后家庭成员及已配对电视才可读。已明确选择的云日历完整标题／地点、旧未标记日程和旅行派生日程保持原家庭共享规则。日程 `owner` 是负责人标签，不是访问权限。

本地日程隐私属于当前源码候选，部署状态以 [README](../README.md) 为准。`visibility: private|shared` 与服务器生成的 `createdBy` 是逐条日程权限字段；普通 CRUD、ICS 导入、助理及数据副本使用同一规则。新建默认 private，只有创建者能更改共享范围；隐藏和不存在的对象统一返回 `404 calendar_event_unavailable`。完整字段、兼容和错误合同见[本地日程隐私](CALENDAR-PRIVACY.md)。
