...
状态：独立 UI 候选，基线 `49e69a6952c91e6014296ca5ed55eb4873733b58`。本分支只新增面板、模型、Node 测试和本文；入口、完整组合、浏览器与发布仍待集成人验证。后端会话读后／写锁复核由独立分支审查，本页不把客户端围栏当作服务端保证。
...
主要 testID：`shopping-settlement-panel`、`settlement-preview`、`settlement-unknown`、`settlement-current`。标签：`查找付款或采购`、`选择付款：标题`、`选择采购：标题`、`整项实付（元）`、`已买到`、`预览共享变化`、`我确认共享整项实付和已买到状态`、`确认保存`。更新／解除按钮的 accessible name 含关联 ID，显示文案不展示该技术字段。按钮至少 44px，复用 Paper 主题、密度与 SelectionRow 键盘行为；实际四宽、深浅色和布局尚待浏览器。
