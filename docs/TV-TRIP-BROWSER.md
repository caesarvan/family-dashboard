...
第三轮 `526a0784885d4f5888f2d53bd4b62d11e1ad86c8` 的结果恢复与撤权流程完整通过；播放控制流程已观察到真实视频结束并显示后续照片，但结束回执计数失败。业务解析器直接消费 `request.stream`，工具的 `after_request` 再读 JSON 时全部进度字段为空，无法据此核验结束回执。原服务日志及失败保留；新工具从真实浏览器 response 的原 request JSON、路径和状态记录进度，不拦截或替换响应，并与原生 ended、视频 ID/revision、暂停时 playId 与递增 sequence 核对，仍要求恢复后恰一次结束回执200。原通过流程不重跑，修正后只补验播放控制流程；本轮整体仍为失败，后续 next、刷新和 dashboard 尚未到达。
