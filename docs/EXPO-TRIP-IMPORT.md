...
状态：独立候选，基于 `2c2079889d1f6fa2bdd5e8aa37b40e96e1c4c9b1`；尚未组合浏览器验收或发布。
...
- 新读写复用 `SegmentFence` 的完整 role／household／member／authVersion／CSRF 前后检查与当前 CSRF；读取、回执和父交接都带 epoch。
  客户端围栏不替代服务端事务权限校验；本分支没有改动后端，也不声明其新增任何会话保证。
...
- 尚未执行本模块真实浏览器、四宽截图、原生文件选择、本人真实文件、云日历或生产发布。后端与接线的独立审查／组合检查另记录。
