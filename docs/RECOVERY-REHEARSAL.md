...
当前 Git 候选夹具增加旅行资料，使用 43 张户内表；与旧 42 表镜像不匹配，不能复用上述结果或直接运行下面的旧镜像 ID。新夹具在两户中保存八份虚构资料，包括本人私有、明确共享和删除旅行后保留的资料，检查文件原字节、元数据、权限和导出边界。新的实际 Docker 演练尚待匹配镜像构建和验证；合成 API／本地夹具通过不等于 Docker 已通过。
