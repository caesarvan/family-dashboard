...
实际运行结果与历史测试分开记录在 [VALIDATION](VALIDATION.md)。历史后端计数与后续新增安全测试分别保留原范围；本工具不要求重复运行与其无关的 UI 测试。
