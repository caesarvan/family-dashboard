...
当前运行版本为 **2026-09-16 20:38:46（北京时间）** 发布的地图旅行相册与本人照片时间建议，保留既有照片处理、库存和助理本地照片／地点搜索。main `e20c269`／integration `c716e4d` 同树；373 源文件、145 方法／路径模板、53 张家庭表及平台两表。实际 1 户两库备份后 53→53 无迁移更新，原数据与配置保持；20:39:27 全源、运行文件和 HTTPS 静态读回通过。完整安装身份见 [HANDOFF](HANDOFF.md)，分段证据见 [VALIDATION](VALIDATION.md)。本文为发布后本地交接，不修改已发布 manifest。

本人在此前 JPEG 补丁后真实选择 5 张、预览并明确保存 5 张、0 失败。本轮未重新执行真实 Google 或实体电视验收，新旅行关联入口尚待本人反馈。家庭助理现在可本地检索已保存照片说明／关联旅行标题及地点文字，见 [搜索契约](ASSISTANT-SEARCH.md)；不识别图片、不自动关联旅行、不新增共享或电视许可。
...
- 在本人照片详情的“按照片时间推荐”点“查看推荐”，核对旅行日期与参考时区，再点“确认关联这次旅行”。先保存未完成的照片编辑；确认只改关联，不自动共享、允许电视或标记到访。新导入才保存来源创建时间；旧照片显示时间未知时，使用下方“关联旅行”手动选择，不把导入时间当照片时间。
- 提示照片或旅行已更新时，重新读取并核对，再次明确确认。网络结果不明时先重新读取当前照片；已经关联会显示“已关联这次旅行”，不自动重复写入。
- 在地图选中已关联旅行的地点，打开该旅行照片；这是只读视图，仅含当前可见的已确认照片。“返回地图”重新读取权限并恢复原筛选、页码和选中地点；普通“相册”入口仍可导入和编辑。日期匹配只是线索，不推断 GPS 或已到访。

接口与详细操作见 [时间建议](MEDIA-JOURNEY-SUGGESTIONS.md)、[建议界面](MEDIA-JOURNEY-SUGGESTIONS-UI.md)、[地图相册](MAP-TRAVEL-PHOTOS.md)。
...
| 照片旅行建议与地图回看 | [本人时间建议](MEDIA-JOURNEY-SUGGESTIONS.md)、[建议界面](MEDIA-JOURNEY-SUGGESTIONS-UI.md)、[地图相册](MAP-TRAVEL-PHOTOS.md) |
...
本轮建议 Windows 作者后端 144 项、根独立专项 34 项分别通过；最终镜像 Linux 144 项／150.48s。真实临时 Flask／SQLite／Edge 地图桥接 13 组、建议保存与冲突／丢响应读回 6 组分开通过，均为合成数据；详见 [本轮记录](VALIDATION.md)。

此前 JPEG 发布的 Windows 作者图片 139 项、worker／结果 60 项；非作者图片 186 项、worker／结果 60 项及桥接 1 项分段通过；实际 Linux 一轮 199 passed／36.50s。此前诊断发布的 Windows、UI／桥接和 Linux 105 项单独记录在 [VALIDATION](VALIDATION.md)。首次媒体发布原 Linux 182 passed／1 failed 与随后单项 1 passed、两户三库恢复 1 passed 保留历史记录，不与本轮混算。

**已确认与待验收：** 本人已授权；保留历史十选三未知和后来十选五的逐项 `invalid_image` 记录。JPEG 发布后本人实际重试 5 张，预览、明确保存与服务器读回全部成功，0 失败；本批真实导入闭环已验证，原失败的具体原因仍不能确定。长期真实 Google 配额／网络行为、实体电视固件与长期展示仍待验收。照片时间建议与地图相册已上线，但本人实际旅行关联与新入口仍待验收；不会自动关联。**仍未实现：** 视频、iCloud／NAS和回忆精选。家庭物品已完成正式接线、48→53 迁移与上线，手工登记和导出见 [交付说明](INVENTORY-DELIVERY.md)；本人真实库存及新增搜索体验仍待反馈。
