...
当前已于 **2026-09-16 18:57:14（北京时间）** 发布相册逐项结果诊断。镜像 `sha256:43e8214c5241f0b94271d19503c3f4d26c398bb4a18596258bbf4f1e75db17bd`，安装 main `d9c51b9`／integration `2aac0007` 同树；344 源文件、130 方法／路径模板、48 张家庭表及平台两表，无迁移。实际 1 户两库完整备份及原行／schema／序列保持；18:58:16 全源、运行文件和 HTTPS 静态文件读回通过。完整身份见 [README](../README.md)，分段证据见 [VALIDATION](VALIDATION.md)。本次文档只在本地更新，不修改已发布 manifest。
...
本人已完成 Google 授权并曾选择 10 张普通照片、保存 3 张；另外 7 项原因未记录，旧记录会显示“已保存 3 张；本次其他处理结果未记录”。不要据此猜测格式、损坏或未勾选；新诊断只适用于之后的选片，不自动重下载。详见 [处理结果](MEDIA-IMPORT-RESULTS.md) 与 [结果界面](MEDIA-RESULTS-UI.md)。
...
18:05 的首次媒体发布按 [媒体迁移](MEDIA-MIGRATION.md) 从 44 表增加四表；18:57 本轮只更新诊断，48 表及原 AI／Google／Microsoft 配置和密钥保持。**当前已经是 48 表，不能重放本轮 44→48，也不能使用历史 43→44／43→43 控制器。** 下一次发布应重新绑定实际基线。媒体两户三库隔离恢复已在实际候选镜像完成，见 [媒体恢复](MEDIA-RECOVERY.md)；这不是生产覆盖恢复。
...
本轮 Windows 后端 105 项、独立补充 4 项、界面 10+15 组、组合 Edge 10 组与真实后端桥接 1 pytest／4 组分别通过；实际 Linux 105 passed／93.97s。首次媒体发布原 Linux 182 passed／1 failed 与随后单项 1 passed、两户三库恢复 1 passed 保留历史记录，不与本轮混算。

**已确认与待验收：** 本人已授权并有历史十选三保存反馈，尚不能视为真实全流程完整成功。新诊断需下一次本人选片验证，真实 Google 配额／网络行为、实体电视固件与长期展示仍待验收。**仍未实现：** 视频、iCloud／NAS、自动旅行关联建议和回忆精选。库存核心、API、界面与真实临时闭环在独立候选，正式库存接线／迁移／上线不属于此次 48 表版本。
