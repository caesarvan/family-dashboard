...
这是未发布的本地候选工具。当前已安装版本仍以 [README](../README.md) 和 [旅行地点与日历验收](EXPO-TRIP-COORDINATION-ACCEPTANCE.md) 为准；生成脚本、通过测试或合并 Git 都不代表已经发布。
...
`stage.py` 和实际 r2 `validate.py` 保持逐字节不变，包括隔离验证的 768 MiB 内存和 512 MiB tmpfs。激活只更换本轮发布名称；读回只另加两个新增 GET 接口的匿名拒绝检查。旧环境摘要、web 镜像、锁顺序、备份调用、回滚材料和 TLS 检查不放宽。后续执行仍须独立审查最终生成的九个文件并绑定；历史候选不能重放。post 入口继续要求独立审查确认的 `--reviewed-sha256`。
