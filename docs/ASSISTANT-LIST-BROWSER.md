...
桌面 R2 已到达真实丢响应后的 GET 回执、刷新恢复、恰一次 POST 与离线恢复；随后工具错误地等待账户菜单自动切换，整体仍失败。实际助理已根据尾部 `/me` 隐藏旧身份内容并明确提示用户刷新。原结果 `assistant-list-20260920T211751407048Z/result.json`（SHA256 `78ed79066d30f4c4c2285a9b1bbe9fa6bc968dab45133fa6c45df404f5dac9e4`）保留。定向修正先在原页面断言失效提示、无旧回执、MutationObserver 零泄漏及恢复标识清除，再显式刷新等待新成员账户菜单，并再次核对无旧内容／标识；不能用刷新掩盖迟到内容安装。
