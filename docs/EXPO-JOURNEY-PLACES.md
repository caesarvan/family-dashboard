...
本页功能已于 2026-09-17 09:25:21（北京时间）上线，09:25:52 正常 TLS 读回通过，入口与地图返回已接通。运行身份、本地第三轮 20 组浏览器检查和四宽可见区域复核见 [发布验收](EXPO-TRIP-COORDINATION-ACCEPTANCE.md)，使用入口见 [README](../README.md)。上线不代替本人实际流程或实体设备验收。
...
组件作者的 Node／语法和 API 检查与组合 TypeScript、浏览器、四宽视觉分别记录；本轮组合与部署结果见上方发布验收。地图底图与坐标选取沿用原地图入口；本人真实云日历、模型、实体电视及完整跨模块场景仍须另验。
