...
本轮固定算子已于 2026-09-18 04:42:28（北京时间）完成发布，实际 287 通过／1 精确平台跳过及保全证据见[集中验收](EXPO-TRIP-IMPORT-ACCEPTANCE.md#expo-trip-import-release)。以下保留生成契约与作者测试记录，已完成阶段不能重放。

这是固定基线的发布工具，生成不等于上线。入口为 `python -B deploy/expo_trip_import_release/prepare.py --source-root <私有access目录> --output <其中不存在的expo-trip-import-tools-*目录> [--freeze <最终冻结配置>]`。不得复用旧发布目录。
...
最终配置必须包含适配器列出的 12 个 pytest 模块，并由实际 collection 得出计数；作者验证阶段未填写最终冻结配置；后续已按本轮 288 个实际收集项冻结，执行结果见上方验收页。生成结果保持 unbound，需另由非作者审查实际生成的文件和私有编排脚本，再绑定最终源码、构建、验收原件及配置。缺少绑定时所有五个生产阶段入口必须拒绝运行。

本分支实际验证：8 个 pytest 通过；针对九个真实父工具的本地检查通过，122 项保护输入篡改均被拒绝，五个未绑定入口均被拒绝。该检查阻止所有子进程与网络，使用临时 synthetic freeze 的 `9991` 仅验证配置机械流程，不是实际测试计数。原件保留在本工作树 `test-results/trip-import-release-r1/`。该作者验证阶段未打包、绑定、连接或部署服务器；后续实际部署见本页顶部。
