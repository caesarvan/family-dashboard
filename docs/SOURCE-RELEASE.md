...
此入口用于经过逐文件审查的既有根 Python 与静态文件更新。当前已安装 integration `82f4401`／main `79faaf3` 同树源码；镜像 `sha256:ea399441e6696bc29214842ab8c47e3db943714c1ae72a05ca1266bdc4a1b739` 于 **2026-09-16 13:24:33（北京时间）** 发布。Windows／Linux 各 435 项受影响后端、Linux 275 项工具、七组 215 项浏览器与真实双户演练分别核验后，实际单户／2 库备份、三阶段数据保持及读回通过。原工具 `4c05e50` 和精确路由索引路径策略 `fd0a128` 均经非作者审查；历史发布与原件见 [VALIDATION](VALIDATION.md)。本页发布后修订仅在本地，尚未同步服务器。纯静态更新使用 [STATIC-RELEASE](STATIC-RELEASE.md)，不能靠修改 READY.mode 扩大入口权限。
...
完整账本新增一条只读 API，需同时更新既有 `docs/contract-inventory.json`。本轮 SOURCE 策略仅新增这个精确文档路径；其他 JSON、配置和原有受保护路径仍拒绝，STATIC 策略保持。它不属于镜像运行文件，不改变构建上下文、DDL、依赖或生产操作权限。工具专项验证路径边界及原环境保持；它与后来实际新镜像、演练和生产发布的证据分别记录。
