...
完整账本候选新增一条只读 API，需同时更新既有 `docs/contract-inventory.json`。本轮 SOURCE 策略仅新增这个精确文档路径；其他 JSON、配置和原有受保护路径仍拒绝，STATIC 策略保持。它不属于镜像运行文件，不改变构建上下文、DDL、依赖或生产操作权限。工具专项使用命令替身验证路径边界及原环境保持，不代表新镜像或生产已完成验收。
