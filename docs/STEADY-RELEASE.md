...
本工具是**尚待组合镜像演练和正式部署验证的发布实现**。用于已完成成员迁移后的普通源码更新，家庭库保持 61 张业务表，平台库保持 9 张业务表。它不执行 `warm_once`，不退休、覆盖或重建成功迁移的 `membership-release-attempt.json`。

当前仅接受已经独立审计的 R3 基线，不能通过环境变量或任意 CLI 参数切换：
