...
照片/地点搜索最初以 `f9f7301` 为基线开发，依赖已接入的 `household_media` 与 `journey_places`。本轮物品增量以 `67614bf` 为基线复用现有搜索路由；匹配名称/规格/位置，数量使用 `inventory_core.project_item` 当前授权投影。未改 schema、媒体引擎、地图模块、应用注册或外部服务。需经非作者审查与集成；源码候选不代表生产启用。字段及经典页接缝见 [物品搜索](ASSISTANT-INVENTORY-SEARCH.md)。

## 用户流程

在家庭助理中输入 `搜索 酒店`、`查找：海边` 或 `找一下 旅行标题`。明确搜索始终在本地完成，即使勾选 AI 也不会发送搜索文字或结果。搜索记录标题/地点、本人及当前可读共享照片的说明/关联旅行标题、本人及共享地点的名称/国家/城市/关联旅行标题，以及本人/同户明确共享物品的名称、规格和存放位置。不会搜索照片原文件名、账户信息、图片内容、坐标、个人财务或未确认的导入内容。
...
`journey` 非空时仅为 `{id,tripId,title}`。照片 `title` 是说明；无说明但匹配关联旅行时为“精选照片”。地点 `status` 沿用 `visited/planned/wish`，不推断已到访。物品 match 为 `{kind:"inventory",id,title,variant,location,unit,visibility,revision,onHandQty,inTransitQty,plannedQty}`；不包含物品来源、财务数据或批次。打开详情时重新校验当前权限与版本。旧记录 match 仍为 `{id,kind,title,start,due,owner}`；无值字段为 null。所有来源均无坐标/账号/文件名/预览字节。
...
- 物品使用当前家庭相同事务的未归档 `owner OR shared` 候选和库存领域投影，默认私密，不搜索订单/批次/回执，不自动创建采购或财务支出。
- 每次 GET 为当前数据库快照；并发增删/撤权可能改变下一页位置和 total，不承诺跨请求快照游标。候选扫描受原实体、照片、地点及库存配额约束，响应最多30条。不增加云调用或地理编码。
...
搜索同样捕获成员上下文；GET 及明确 `/plan` 搜索在最终读事务内同时校验 current 和 validate_context，代次变化时拒绝，不把原上下文替换为新捕获值。
...
模型仅继续接收用户主动提交的非搜索文字，以及明确勾选的近期事件/待办标题 allowlist。照片、地点、库存、私有财务、账户和文件名均不加入模型上下文。
