...
新版入口为“更多 → 账户与自动同步”或顶部账户菜单的“连接与账户”（`/app/connections`，页面标题“账户与同步”），使用当前 Expo 官网的白底、黑色按钮、浅灰卡片。连接、选择来源、核对共享范围、查看逐来源同步结果和断开绑定在同一页完成；完整操作见 [Expo 账户界面](EXPO-ACCOUNTS.md)。此入口已于2026-09-17 03:01:51（北京时间）发布；本人新入口授权与长期真实云同步仍需单独验收，版本与验证见 [验收记录](VALIDATION.md)。
...
授权失效但仍有来源时，可明确“停止日历与清单同步”，无需重新授权，保留绑定和照片；它只移除这些来源的镜像并释放主清单。“断开绑定”则会额外移除已导入看板的照片副本并撤销家庭／电视展示，原平台原件保留。两者均先确认，结果不明先读回。
