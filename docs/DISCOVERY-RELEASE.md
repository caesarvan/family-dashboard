...
当前仍是未发布候选：最终R3 Expo构建、分段浏览器、实际Linux镜像与125项Linux测试均已完成并独审。重复提示资源R1在四个请求开始前因只读数据库观察失败，原件保留；工具修正后的真实资源验收仍是发布阻断。尚未组装本批真实计划或上线。用户入口、固定身份和失败历史集中见 [候选验收](DISCOVERY-ACCEPTANCE.md#discovery-candidate)。业务是助理打开原地点，以及本人主动核对 Google／设备照片的展示副本一致性；不改变照片原图、共享或电视许可。
