...
本功能于 2026-09-17 11:09:53（北京时间）发布，11:10:38 正常 TLS 读回通过；固定运行身份、分段结果与失败修订见 [改期发布验收](JOURNEY-RESCHEDULE-ACCEPTANCE.md#journey-reschedule-release)。实际安装版本仍以 [README](../README.md) 和 [验证记录](VALIDATION.md) 为准。它补齐旅行日期调整时的任务、地点和本地日历协调；完整产品目标仍见 [产品计划](PRODUCT-PLAN.md)。
...
提交中或保存结果未知时，先在当前页面核对原操作；应用内导航和新建入口会暂时提示留在此页。支持 `beforeunload` 的浏览器还会对关闭或刷新提示确认。此保护不等于持久化草稿；强制关闭、浏览器进程终止或退出登录后，仍应重新读取实际旅行与日历状态，不能假定未保存。
...
- 本地 SQLite、浏览器及模拟提供方的验证与真实账号、真实模型和实体电视验收分别记录。本轮生产数据保全和读回已通过，不代替本人真实云改期、模型或实体电视验收。
