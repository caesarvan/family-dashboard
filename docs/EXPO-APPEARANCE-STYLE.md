...
HomeScreen 本轮直接声明的按钮与新增外观面板按钮最小高度为 44px；待办勾选使用 Paper 48px 图标控件，保留 checkbox 语义、禁用状态及空格操作，避免原 Checkbox.Android 固定 36px 的操作区域。账户连接、家庭资金与持仓的固定白底、说明文字和提示背景改用主题语义色，防止深色文字落在固定白底上。它们的业务逻辑和内部表格密度不变，共享卡片内边距随主题密度调整。
