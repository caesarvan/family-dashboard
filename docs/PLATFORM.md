...
当前组合结构为 **106 个 Flask 方法／路径模板、43 张户内表与 2 张平台表**。本次旅行资料增加一表、五个成员操作；此前消费观察、例行和采购模块保持原权限与数据。历史批次不代表当前总数，结构见 [存储索引](PLATFORM-ROUTES.md)，资料权限见 [资料契约](JOURNEY-DOCUMENTS.md)。
