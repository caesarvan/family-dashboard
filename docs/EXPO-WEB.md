...
| `/tv` | 候选：安全 Expo 导出存在时 302 到精确 `/app/tv`，不转发查询参数；没有导出则保留经典电视页 |
| `/tv?classic=1` | 候选：显式使用经典电视页 |
| `/demo` | 保持原演示页面与判定 |
...
电视候选 `/app/tv` 在根布局首次渲染时即排除 `HouseholdProvider`，不启动成员页面的登录与个人数据读取。独立控制器使用 `X-Display-Mode: tv`，通过 `/api/me` → `/api/state` → `/api/me` 核对同一有效电视身份后才显示共享 DTO；配对沿用当前签名家庭，照片另走电视专用短授权接口。隐藏／断网／身份变化时清空显示，恢复前重新核对；不将成员 Cookie 当作电视授权。
