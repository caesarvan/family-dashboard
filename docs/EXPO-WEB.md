...
**电视入口已于 2026-09-17 14:19:19（北京时间）发布，14:19:53 正常 TLS 读回通过。** 下表新路由、精确跳转、经典回退及静态资源已核对，证据见 [本轮验收](EXPO-TV-ACCEPTANCE.md#expo-tv-release)。使用与身份／照片边界见 [Expo 电视展示](EXPO-TV.md)，不代表实体电视已验收。
...
| `/tv` | 安全 Expo 导出存在时 302 到精确 `/app/tv`，不转发查询参数；没有导出则保留经典电视页 |
| `/tv?classic=1` | 显式使用经典电视页 |
| `/demo` | 保持原演示页面与判定 |
...
Google Photos 授权成功的 `/?auth=photos-connected` 在有 Expo 导出时固定跳到 `/app/photos`，无导出则保留经典相册。日历/清单账号的 `connected` 和授权错误进入 `/app/connections`，只转发固定状态与白名单原因；前端消费后清除这些参数，未登录时显示固定错误并保留登录入口。没有导出则回退经典页。不会转发 provider code、token 或任意 next。来源管理操作与验证边界见 [Expo 账户](EXPO-ACCOUNTS.md)。
...
成员页面启动顺序：`GET /api/me` → 未登录时 JSON `POST /api/login` → 再 `GET /api/me` → `GET /api/state`。所有 fetch 使用同源 Cookie；写请求携带新 `X-CSRF-Token`。新本地待办显式 `sourceId:""`，避免省略时写共同云清单；编辑／删除携带单条 revision，409 重读后明确确认，网络未知不自动创建重复记录。详细字段见 [基础 API](API.md)。家庭切换继续顶层导航 `/space/<slug>`，清客户端身份缓存后重新登录；该接口原有 303 返回 `/`，会由新入口继续接入。

电视页 `/app/tv` 在根布局首次渲染时即排除 `HouseholdProvider`，不启动成员页面的登录与个人数据读取。独立控制器使用 `X-Display-Mode: tv`，通过 `/api/me` → `/api/state` → `/api/me` 核对同一有效电视身份后才显示共享 DTO；配对沿用当前签名家庭，照片另走电视专用短授权接口。隐藏／断网／身份变化时清空显示，恢复前重新核对；不将成员 Cookie 当作电视授权。
