...
已有旅行改期已于 2026-09-19 17:39:23（北京时间）上线，独立只读审计通过；固定身份及分轮证据见[集中验收](ASSISTANT-TRIP-CHANGE-ACCEPTANCE.md#assistant-trip-change-release)。输入“把冰岛旅行延后三天”后，点击原有“整理并预览”，进入已有旅行改期页面并按本次 AI 选择读取建议。明确的搜索前缀仍优先走本地搜索；已有旅行修改不会落入新旅行简报。

## 使用流程

1. 在同一页保留“改期需求”和原有可选 AI 设置，首次进入会读取建议；修改需求或 AI 选择后可点击“整理改期建议”／“重新整理需求”。使用 AI 时明确说明发送本次文字及必要候选的旅行标题、日期、时区，不附带近期清单或其他家庭内容。
...
上述为前端作者阶段的结果，当时尚未组合 API、接线或运行真实模型／浏览器。后续正式构建、真实模型与浏览器 R2 已分别完成，结果见[集中验收](ASSISTANT-TRIP-CHANGE-ACCEPTANCE.md)；不把后续结果记作该作者阶段执行。
