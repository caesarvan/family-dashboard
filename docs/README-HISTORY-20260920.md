# README 历史快照 · 2026-09-20

> 这是提交 `6b93459471f4c123749373964285042f9230bb2e` 的 README 完整历史快照。除将相对 Markdown 链接按本目录重定位、换行统一为 LF 外，下方原文保持；原有“当前”、表数量、服务数量及未实现描述只代表各历史段落的当时口径，不能用作当前发布操作依据。当前状态和接手入口见 [项目 README](../README.md)，最新已发布证据见 [设备照片与那年今日验收](LOCAL-PHOTO-ACCEPTANCE.md#local-photo-release)。旧迁移和已消费计划禁止重放。

---

# 家庭中枢 · Family Dashboard

**最新上线：设备照片手动导入与「那年今日」。** 2026-09-20 20:04:27（北京时间）激活，20:05:10生产只读回读通过，随后非作者复核通过。上传预览后明确私密保存，家庭共享和逐台电视授权分别设置；回忆只使用可核对的本人来源日期。[最短操作、固定身份与集中验收](../docs/LOCAL-PHOTO-ACCEPTANCE.md#local-photo-release)。

**此前上线：视频导入、成员播放与电视混合轮播。** 2026-09-20 17:26:15（北京时间）激活，17:33:17 生产只读检查通过，随后非作者复核通过。相册支持明确选择并保存视频、成员播放和逐台授权的照片／视频轮播；真实 Google 视频和实体电视仍待验。[使用、固定运行身份与集中验收](../docs/MEDIA-VIDEO-ACCEPTANCE.md#media-video-release)。该视频批次当时不含设备照片上传和「那年今日」；后续发布见文首。

**此前上线：本地日程默认私密与明确共享，2026-09-20 10:11:17（北京时间）激活，10:18:48 独立生产只读审计通过。** 从首页或日程添加安排，默认「仅自己」；明确选择「家庭共享」后才向家庭成员和电视展示，创建者可收回共享。旧记录与已授权云来源保持原范围。[使用与集中验收](../docs/CALENDAR-PRIVACY-ACCEPTANCE.md#calendar-privacy-release)。

**此前上线：日程时间重叠，2026-09-20 08:47:27（北京时间）激活，08:49:19 独立生产只读审计通过。** 在首页按「今日／本周／前后 3 天」查看重叠提示，进入日程展开完整标题、地点和重叠时段，直接调整原本地安排。同步日程保持只读，需在原应用调整。[使用与集中验收](../docs/CALENDAR-CONFLICTS-ACCEPTANCE.md#calendar-conflicts-release)。
...
**此前上线：本地待办前置事项（2026-09-20 06:28:25，北京时间），06:35:47 独立只读审计通过。** 在「共同待办 → 添加／编辑 → 前置事项」选择本地任务，首页与清单显示阻塞原因；前置未完成时不能完成后续任务。任务依赖、原 ID 与版本保护已接到旅行、例行事项和明确云冲突处理。[使用与集中验收](../docs/TASK-DEPENDENCIES-ACCEPTANCE.md#task-dependencies-release)。本地浏览器按 R1 前两项＋R4 第三项分段验证，实际 Linux 354 项通过；一户两库保持 69/9。该任务依赖批次未包含站内提醒；A–D 本人完整验收 0/4、真实云写入与实体电视另验。

**此前上线：助理资料与照片搜索（2026-09-20 05:15:29，北京时间），05:16:46 独立只读审计通过。** 从「更多 → 家庭助理 → 找资料照片」查找当前授权内容，打开原详情并返回原搜索；包含采购错误提示完整换行修复。[使用与分段验收](../docs/ASSISTANT-DOCUMENT-SEARCH-ACCEPTANCE.md#assistant-document-search-release)。该历史批次的 R1／R2 与 Linux 175 项保持原范围，不代替本次任务依赖验收。

**此前上线：采购截止日与优先级（2026-09-20 03:54:39，北京时间），03:55:48 独立只读审计通过。** 「采购」可填写截止和低／普通／高优先级；旅行草案携带这些字段，旅行改期只移动本人明确选中的采购。后端、正式新构建、4 条真实临时浏览器流程、一次真实模型及 Linux 298 项分别通过。[最短使用路径与集中验收](../docs/SHOPPING-SCHEDULE-ACCEPTANCE.md#shopping-schedule-release)。该采购批次当时的 390px 离线提示裁切已随 05:15 的资料搜索发布修复；旧验收事实保留，真实云／实体电视及 A–D 本人完整验收（0/4）另验。
...
此前本人账户分析于 2026-09-19 16:20:25（北京时间）上线，16:20:57 独立只读审计通过；其固定运行身份与历史证据见[资产分析验收](../docs/FINANCE-ANALYSIS-ACCEPTANCE.md#finance-analysis-release)。
...
**此前上线：采购关联库存与首页待办编辑（2026-09-18 15:12:56，北京时间）。** 采购项可打开「登记或查看库存」，明确登记批次、收货和退回，再返回原搜索/筛选；首页本地待办可编辑负责人和截止日期。浏览器五条流程分轮取得证据，Linux 47 项与两户三库演练通过；生产实际一户两库保持 61/9，15:14:36 独立只读审计通过。本人真实库存、云端交互和实体电视仍待验收。完整身份与边界见[本批验收](../docs/SHOPPING-INVENTORY-ACCEPTANCE.md#shopping-inventory-release)。

**此前上线：个人账户与多家庭成员协作（2026-09-18 13:51:39，北京时间）。** 注册及绑定、邀请加入、移除与退出、多家庭切换已激活，实际一户两库完成 58/2→61/9 迁移及启动保全；个人数据不自动共享。操作、固定运行身份与验证边界见[本轮发布](../docs/MEMBERSHIPS-RELEASE-R3.md)。此前失败和完整库组恢复记录保留；下方为各历史版本的当时状态。

**此前上线：助理查找已有旅行（2026-09-18 09:30，北京时间）。** 在「家庭助理」输入「找一下冰岛旅行」「搜索 冰岛旅行」或「查找：冰岛旅行」，点「查看旅行」打开原详情，再返回原搜索页；显式搜索只在本地进行。09:31 正常 TLS 读回通过。[使用与验收](../docs/EXPO-ASSISTANT-TRIP-SEARCH-ACCEPTANCE.md#expo-assistant-trip-search-release)。
...
**此前上线：家庭与成员（2026-09-18 05:49，北京时间）。** 从「更多 → 家庭与成员」调整另一成员的角色或让其当前浏览器全部退出；对方仍可重新登录，个人数据与电视许可不变。现有两位成员初始化为共同管理员。使用与实际验收见[家庭与成员](../docs/EXPO-HOUSEHOLD-MEMBERS.md)及[集中验收记录](../docs/EXPO-HOUSEHOLD-MEMBERS-ACCEPTANCE.md#household-members-release)。通用加入／退出、多家庭成员关系仍未实现；实体设备仍未验收。

**此前上线：旅行 JSON 导入（2026-09-18 04:42，北京时间）。** 从「旅行 → 导入旅行」选择文件或粘贴 JSON，先预览再明确创建；保存结果不明时可用原操作编号核对。R2 本地 12 组流程／12 图、Linux 287 通过及 1 项精确 Windows 跳过分别通过；58→58 原数据保持，04:43:27 HTTPS 读回完成。[使用与恢复](../docs/EXPO-TRIP-IMPORT.md) · [集中验收记录](../docs/EXPO-TRIP-IMPORT-ACCEPTANCE.md#expo-trip-import-release)。本人真实资料、云写入及实体电视仍未在本轮验收。
...
**此前上线：旅行分段编辑，2026-09-17 21:12:18（北京时间）激活，21:12:47 正常 TLS 读回通过。** 从旅行详情进入「行程分段」，编辑城市、航班、住宿和活动，核对预览后保存；支持并发核对及原操作结果恢复。本地 15 组流程／19 图、服务器 260 项通过及唯一精确 Windows 跳过分别核验，55→55 原数据与配置保持。使用见 [行程分段](../docs/EXPO-JOURNEY-SEGMENTS.md)，运行身份及独立读回见 [发布验收](../docs/EXPO-JOURNEY-SEGMENTS-ACCEPTANCE.md#expo-segments-release)。当时个人资产报告仍为候选，现已按文首记录上线；实体电视仍待验收。

**此前上线：家庭例行计划，2026-09-17 19:21:11（北京时间）激活，19:21:45 正常 TLS 读回通过。** 从「更多 → 家庭例行计划」创建每日／每周／每月家务或采购，预览后三期并确认，支持当前事项完成、暂停／恢复、跳过和归档。R3 本地十组流程及服务器 277 项通过、1 项精确 Windows 跳过分别核验；55→55 数据与账户配置保持。使用见 [家庭例行计划](../docs/EXPO-ROUTINES.md)，运行身份及边界见 [发布验收](../docs/EXPO-ROUTINES-ACCEPTANCE.md#expo-routines-release)。真实家庭长期调度、真实云与实体电视仍另验。
...
**旅行地点默认仅本人可见、已计划、隐藏共享坐标。** 经纬度可以留空，不推测位置或到访。日历预览不写入云端，明确确认后才加入持续同步队列；队列受理与云端已确认分别显示。普通编辑总日期不会自动移动关联日期；需要联动时从详情进入「调整日期」，逐项选择并预览确认。详细行程分段与旅行资料已支持新版管理；经典入口保留为回退。
...
进入「旅行 → 地图」，可以保存心愿、已计划和本人确认已到访的地点，按年份、成员及旅行筛选，并打开关联旅行。地点默认仅本人可见；明确共享时可隐藏或粗化坐标。世界概览底图与应用同源，无需外部地图账号。电视不显示地点详情；当时的相册候选现已按顶部记录发布，手动单趟回顾已从旅行详情开放，自动回忆仍未实现。使用见 [地图界面](../docs/JOURNEY-MAP-UI.md)、[地点接口](../docs/JOURNEY-PLACES.md)。
...
| 待办 | 已上线[本地前置事项](../docs/TASK-DEPENDENCIES-ACCEPTANCE.md#task-dependencies-release)：选择／解除依赖、阻塞提示及事务内完成校验；本地任务与云清单新建/完成回写；旅行/助理/已有任务可确认发布到 Microsoft To Do 或 Google Tasks 主清单，保留原 ID/负责人并回流完成状态 | 新发布队列尚未完成真实云写验收；轮询同步；不确定创建结果需查远端，不盲目重建 |
...
| 助理 | 已上线[资料与照片搜索](../docs/ASSISTANT-DOCUMENT-SEARCH-ACCEPTANCE.md)：搜索当前授权元数据、打开原详情并返回原页码，不自动下载；已上线[旅行准备、采购与成员分工](../docs/ASSISTANT-TRIP-ITEMS-ACCEPTANCE.md)：一句需求整理为可编辑清单，再预览并明确保存；[预算与支出只读查询](../docs/EXPO-FINANCE-QUERY.md)：默认本人／本月、分币种净支出及预算，明确共享消费与公共手工快照；[已有旅行改期](../docs/ASSISTANT-TRIP-CHANGE-ACCEPTANCE.md)：原文→候选／缺参→核对联动→原预览确认；旅行文字简报、可选 AI 与手工补齐、明确重新整理和旅行预览确认；本地近期概览、搜索、待办/采购草案与逐项确认；已发布“本周待处理”，从待办、冲突日程、采购金额、旅行准备和公共资金打开原记录；存在到期或异常例行计划时显示第六组，处理后回读 | 行动台范围为今天至七天后并含逾期待办，不是完整自然周；默认本地规则；改期真实模型已用合成旅行验证，本人真实旅行与云日历完整链另验 |
...
| `/api/assistant/finance-query` | 本人支出预算、明确共享消费与公共手工快照的只读查询；可选模型只解析问题，不附带账本或金额；[字段与边界](../docs/ASSISTANT-FINANCE-QUERY.md) |
...
安装、DNS、HTTPS 证书与续期、OAuth 回调、备份及恢复见 [部署指导](../docs/DEPLOYMENT.md)。当前为 69 张户内表及 9 张平台表，首次安装与已有数据更新使用各自流程；本轮固定发布及数据保全见[旅行清单发布验收](../docs/ASSISTANT-TRIP-ITEMS-ACCEPTANCE.md#assistant-trip-items-release)。[54 表回执恢复](../docs/FINANCE-RECEIPTS-RELEASE.md) 和历史 43 表发布入口仅供追溯，不能用作当前发布入口：

- **首次安装**：配置新服务器与独立密钥，初始化新数据卷，签发证书，启动 app／sync／media／web 四个服务，再绑定第三方账户。
- **已有环境更新**：冻结实际已安装 BASE 和审查后的候选，在独立目录构建与验证；通过部署演练和独立发布审查后，停止写者、核验全组备份、安装源码并分阶段核对全部数据，再开放服务。当前为 69 张户内表及 9 张平台表；任何后续结构、依赖或配置变更需另行审查，不能重用历史 43→43 控制器或已完成的 53→54 迁移。
...
历史版本完成 37→40、40→42、[42→43](../docs/JOURNEY-DOCUMENTS-RELEASE.md)、43→44、44→48、[48→53](../docs/INVENTORY-MIGRATION.md)、[53→54](../docs/FINANCE-RECEIPTS-RELEASE.md) 及 [54→55](../docs/HOLDINGS-RELEASE.md)。[静态 43→43](../docs/STATIC-RELEASE.md)、[SOURCE 43→43](../docs/SOURCE-RELEASE.md) 和 DEPLOYMENT 第 5.2 节的历史算法均不可用于当前 69/9 表结构；已完成的一次性迁移（含 55→58、58/2→61/9、61/9→66/9 和 66→69）不能重放。
...
| [新版旅行分段](../docs/EXPO-JOURNEY-SEGMENTS.md)／[快照 API](../docs/EXPO-JOURNEY-SEGMENTS-API.md)／[验收](../docs/EXPO-JOURNEY-SEGMENTS-ACCEPTANCE.md) | 已发布；城市、航班、住宿、活动，明确预览与保存、并发核对和原操作恢复，实际身份见验收页 |
...
| [本地待办前置事项](../docs/EXPO-TASK-DEPENDENCIES.md) · [API](../docs/TASK-DEPENDENCIES.md) · [集中验收](../docs/TASK-DEPENDENCIES-ACCEPTANCE.md#task-dependencies-release) | 使用步骤、完成与删除约束、实际发布身份和分段测试边界 |
...
| [助理资料与照片搜索](../docs/ASSISTANT-DOCUMENT-SEARCH-UI.md) · [集中验收](../docs/ASSISTANT-DOCUMENT-SEARCH-ACCEPTANCE.md#assistant-document-search-release) · [发布入口](../docs/ASSISTANT-DOCUMENT-SEARCH-RELEASE.md) | 查找已保存元数据、重新核权打开原内容、返回原搜索；当前发布状态及分段证据见集中页 |
