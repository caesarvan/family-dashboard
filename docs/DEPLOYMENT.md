...
当前 Git 集成源码增加一张 `journey_documents` 表、两个显式索引和解除关联触发器；线上仍是页首 42 表版本。**当前候选不能使用第 5.2 节历史 42→42 零结构流程。** 新服务器从候选源码初始化时为 43 张户内表及 2 张平台表；已有服务器仅能在审查、组合测试与完整备份通过后进行一次性 42→43 升级。候选是否已经发布以实际验收记录为准。
...
2026-09-15 16:25:03 的旅行执行与订单兼容版没有新增表或列。使用第 5.2 节的零结构流程；停止入口与写入、验证全家庭备份后，再初始化、精确读回并启动。下方 40→42／37→40 是历史首次迁移，不能重复用于当前 42 表数据库。

## 历史消费观察 40→42 升级要求（15:10:00）

本版首次 40→42 升级仅增加 `finance_spending_observations`、`finance_spending_receipts` 和 `finance_spending_receipts_owner` 索引。完整 DDL 在 `spending_observations.py` 的 `SCHEMA_SQL`，按本次已发布清单核对。原 40 张表、平台注册表、历史业务／认证／例行记录、schema 对象及 SQLite 内部序号保持；新增两表初始为空，部署不预导入真实个人报告。
...
**第 5.2 节用于已经完成本次两表迁移后的 42→42 零 schema 更新，不能用于本次 40→42 首次升级。** 本轮专用迁移及故障恢复证据单列在 [VALIDATION](VALIDATION.md)。其他 agent 未取得该发布清单与匹配的迁移核对器时，只进行隔离开发；空服务器新建直接由当前源码初始化 42 张户内表及 2 张平台表。
...
该历史 37→40 升级使用当时专门审核的三表迁移流程，不能运行第 5.2 节当前 42→42 的零 schema 更新代替：
...
第 5.2 节的零 schema 示例不能承担 35→37、37→40 或 40→42 首次迁移，35／37／40 表输入都应拒绝。空服务器使用当前源码直接建立 42 张户内表及 2 张平台表，采购两表、例行三表和消费观察两表初始为空；已有数据按实际结构选择专门流程。交接数量以最终清单为准，旧 193／200 文件属于历史。
...
本文保留 Dockerfile、compose.yaml、deploy 的安装和维护说明；第 5.2 节适用于已完成消费观察迁移的 42 表版本，不能代替历史 40→42 首次迁移或更早的升级。源码／步骤核对日期为 2026-09-15；下方服务器快照保留 2026-09-14 历史，不代表之后实时状态。接口／权限见 [README](../README.md)，账号注册见 [ACCOUNT-SYNC](ACCOUNT-SYNC.md)，证据见 [VALIDATION](VALIDATION.md)。
...
完整来源预览／CAS、旅行时间明细及独立消费观察均已在历史版本发布，当前保留。对应源码为 `finance_source_bridge.py`、`journey_time.py`、`deploy/prepare-finance-source.py` 和 `pytest.ini`；固定 `tzdata==2026.4`，Docker `PYTHONTZPATH=""` 使用该包。后续零 schema 更新按第 5.2 节先停旧进程、备份、串行初始化，再分步启动；任何新增结构必须使用该版本专门审核的首次迁移流程。
...
首次空数据卷使用本次完整源码会创建 42 张户内表及两位用户，并初始化 2 张平台注册表；采购两张私有表、三张例行表和两张消费观察私有表初始为空。首次安装的真实服务器流程仍需单独验收。未来启动只做幂等建表/兼容列迁移，不会重置已有密码或导入演示数据。UID 10001 必须能在 `/data` 创建数据库、WAL、备份及 `cloud-locks/`。暂不启动 Compose 的 web：正式 `nginx.conf` 同时引用 IP 与域名两套证书，缺任何一套都会启动失败。
...
历史 40→40 校验片段的 58 项离线检查仅对应当时版本。当前第 5.2 节为 42→42：整份文档散列、实际抽取 Python 代码与消费观察 SCHEMA_SQL 必须绑定同一离线复验记录，检查临时两户非空历史、完整备份和 fake app warm。该范围与历史 40→42 首次迁移／激活恢复分别记录，不是 Docker 或生产恢复实测。
