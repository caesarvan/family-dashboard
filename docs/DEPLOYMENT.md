...
专用 [发布控制器](../deploy/activate_journey_documents_release.py) 已独立审查并用于本次实际迁移。初版 66 项、第一轮配置修订 83 项保留为历史，最终控制器 100 项命令替身测试与真实单户配置迁移演练另有证据；不能把这些范围混算。四个 CLI 参数、只读挂载权限、READY 五类证据、原始记录及失败处理见 [完整发布指导](JOURNEY-DOCUMENTS-RELEASE.md)。
...
本次发布前独立核对了旧 222 文件源树和仍记载 218 文件的旧 `RELEASE-MANIFEST.json`，两者分别固定 SHA 并保留原字节。此前 225 文件的 Git 文档交接也不能冒充旧服务器的实际状态。20:23 的正式发布已安装新的 237 文件源码和发布清单；上述 222／218 只属于一次性控制器的原基线，不是当前安装目标。完整历史与交接边界见 [HANDOFF](HANDOFF.md)。

## 严格限定的静态更新候选

下一轮纯前端更新采用新的 [STATIC-RELEASE](STATIC-RELEASE.md) 契约，保留上述一次性迁移代码。它要求根 Python、依赖、Dockerfile、Compose、Nginx 和运行配置与批准的实际旧清单保持一致，只允许已审查的静态文件及精确限定的测试、文档和部署工具变化。构建工具仅追加一个静态层；发布控制器另行比较配置基线、备份全部家庭并核对启动前后完整数据。

该路径没有 warm 或 DDL 迁移步骤；它比较 43 张表中已有记录（包含旅行资料 BLOB）、schema、外键、索引、trigger、序号及平台注册数据。真实隔离 Docker 演练和生产发布分别记载；工具进入候选不表示任意后端更新或恢复流程已经获准。不能只修改表数复用历史算法。

## 历史 42→42 更新

2026-09-15 16:25:03 的历史旅行执行版没有结构变化，当时使用第 5.2 节 42→42 流程。**该节所有代码现在仅供历史追溯，不能用于当前 43 表库。** 本次已完成的一次性 42→43 同样不能重复执行。未来 43→43 零结构检查尚未实现和验证，必须另行准备与审查，不能只改表数绕过。
...
发布候选必须来自已按 [Git 流程](GIT-WORKFLOW.md) 审查并合入 main 的明确提交，在独立干净发布工作树中构建；记录 commit、归档 SHA、镜像 ID、测试与迁移类型。Git 合并不会更新正在运行的容器。各 agent 在自己的 worktree 开发，不直接改服务器目录；本次已审查的旅行资料已完成独立 42→43；当前 43 表不能执行本节历史 42→42 算法。

历史基线说明：16:25 原发布清单为 218 文件，17:35 独立核对实际源树为 222 文件，后续 225 文件 Git 文档交接当时未更改运行镜像。20:23 发布已安装新的 237 文件清单。任何后续更新仍须逐文件核对真实源树及其清单，不能仅覆盖清单消除差异；旧 222 源树与 218 原清单留作本次迁移前证据。
...
> **历史代码限定：以下保留的是 42→42 更新程序及当时的验证条件，所有代码块原字节不变。当前已为 43 表，不可执行这些更新命令。43→43 程序尚待独立实现／验证；一次性 42→43 工具也不能重复使用。**
