...
## 2026-09-19：缺会话引擎的初始化顺序修正（候选）

成员存储接入后，`db()` 会校验当前成员会话；若会话引擎缺失，旧初始化顺序会先触发 `KeyError`，消费观察入口还会将其映射为格式错误 400，未能返回已约定的 503。`ImportSession` 现先执行 `require_member()`，再确认会话引擎存在，最后取得数据库连接并捕获身份。原成员／电视限制、快照与提交前后会话复核均保持；无新表或数据迁移。

原两种来源的缺引擎与会话不匹配测试保留全部 503／401 断言。财务只读查询另覆盖本地及允许模型两种真实路由请求：缺引擎时只返回 503 错误，不投影财务、不调用模型且业务表不变。验证仅使用本机临时数据库与合成记录，不代表已修复的 Linux 镜像、线上部署或本人真实财务验收。

本轮先在未修改源码上复现原两项失败，保留 `test-results/session-guard-repro-r1/` 原件；修后限定回归 **70 通过、0 失败／错误／跳过，64.49秒**，其中原导入会话专项56项、财务查询缺引擎及撤权／身份／权限边界14项。修后命令、stdout／stderr、JUnit 与受验源码前后散列见 `test-results/session-guard-regression-r1/`；JUnit SHA256 为 `d59353b8d7ae474bb5878f2d236ee5e4ee085ab156737e00f1b83595f83198c2`。原件均不进入 Git。
