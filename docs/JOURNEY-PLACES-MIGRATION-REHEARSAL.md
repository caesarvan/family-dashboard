...
输入是已审完整候选目录、候选/实际基线 manifest SHA、父/新镜像 SHA 和不存在的报告目录。候选必须同时包含已审地点检查器与支持 `legacy43` 的恢复 fixture；单独检出此作者分支不代表所有依赖已齐备。

```text
python tests/rehearse_journey_places_migration.py --source CANDIDATE --parent OLD_IMAGE --image NEW_IMAGE --manifest-sha CANDIDATE_MANIFEST_SHA --base-manifest-sha BASE_MANIFEST_SHA --output NEW_REPORT_DIRECTORY
```
