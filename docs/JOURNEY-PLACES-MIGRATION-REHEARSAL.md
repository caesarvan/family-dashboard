...
输入是已审完整候选目录、候选/实际基线 manifest SHA、父/新镜像 SHA 和不存在的报告目录。候选必须同时包含已审地点检查器与支持 `legacy43` 的恢复 fixture；单独检出此作者分支不代表所有依赖已齐备。必须使用 `-B` 且不设置字节码缓存前缀；每次源码复核还拒绝整个候选树中的 `.pyc/.pyo`，因为仅禁写不能阻止 Python 读取旧缓存。
