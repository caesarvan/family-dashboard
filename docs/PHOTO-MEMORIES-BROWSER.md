...
R1 实际浏览器前两流程通过，第三流程在工具的精确「刷新」按钮定位超时；原页面无障碍快照显示按钮名称包含图标，且自然轮询已执行真实切换登录、清空本人内容并显示伴侣照片。窄修只移除多余点击，直接等待同一个真实轮询回调，保留原身份和撤权断言。R1 仍为 2/3，后续仅复验 `identity_and_revocation`，不能把尚未执行的真实撤权段写成通过。

R2 仅复验第三流程，业务断言及三张截图已完成，但退出时 Windows 无法删除仍有 SQLite 连接占用的临时库，整体仍失败。后续窄修为撤权 fixture 显式关闭连接，同时保留原事务提交/回滚与异常传播；不忽略清理错误、不延时重试。连接寿命专项使用保留强引用的真实 SQLite，验证成功、断言失败和 SQL 异常均关闭连接；正式第三流程仍需再次独立审查后运行。
