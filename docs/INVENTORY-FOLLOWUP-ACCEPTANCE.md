...
R2 截图等待字体、目标可见和至少 600ms 稳定几何，确认输入框标签没有稳定重叠。**黑色主按钮文字仍有疑似叠影，根因与显示验收尚待完成；不能据功能测试宣称视觉通过。**
...
脚本静态检查使用指定 Python `-B -X utf8` 读取文件，执行 `ast.parse` 和 `compile(..., 'exec')`，不导入 app、不起服务、不写 pyc；提交前执行 `git diff --check`。R2 已取得运行与稳定截图证据，但按钮显示仍待诊断；尚无性能、部署或真实家庭验收结论。
