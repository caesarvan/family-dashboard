...
仅复用 `test_calendar_publish.Remote.transport` 代替外部日历 HTTP，两个 provider 各自运行独立临时家庭与 `Remote`。OAuth 账号和授予范围是写入临时库的合成 fixture；后续日历业务 HTTP、真实 provider adapter、持久队列与 worker 均使用产品实现。Python 禁止非 loopback socket，浏览器禁止非本次本地 origin。旅行采用本地文字提取，任何模型调用都使测试失败，不以合成模型冒充成功。
...
2. **地点确认与地图。** 生成目的地建议和取消均零写；默认坐标为空、私人、已计划。输入合成坐标后明确保存，验证没有到访确认或共享，再进入已选地点地图并返回关联旅行。
