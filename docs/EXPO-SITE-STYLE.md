...
2026-09-17 再次对照官网与当前首页，保留现有配色和组件，调整家庭场景的信息密度：未核对共同资金显示一段说明和「核对资金」，已确认金额继续突出；桌面卡片顶端对齐，日程不被相邻资金卡拉高。顶部图标按钮的内部焦点框按圆形裁切并内缩，保留键盘焦点，修复账户按钮出现残缺方框的问题。手机、桌面新构建的操作及视觉验收与上线状态分别记录，不把本地 DOM 样式试验当作已发布结果。
