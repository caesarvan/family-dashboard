...
## 已发布可用性调整（2026-09-17 11:09）

2026-09-17 再次对照官网与当前首页，保留现有配色和组件，调整家庭场景的信息密度：未核对共同资金显示一段说明和「核对资金」，已确认金额继续突出；桌面卡片顶端对齐，日程不被相邻资金卡拉高。顶部图标按钮的内部焦点框按圆形裁切并内缩，保留键盘焦点，修复账户按钮出现残缺方框的问题。该版已随改期 r3 构建在真实临时浏览器通过首页四宽卡片高度、资金入口与键盘焦点检查；整组 16 项通过并产出 18 张截图，固定身份与前两轮失败修订见 [改期验收](JOURNEY-RESCHEDULE-ACCEPTANCE.md)。该版已于 11:09:53 激活、11:10:38 正常 TLS 读回通过；这些证据不代表实体电视或全部内部滚动内容已获视觉验收。
