...
## 视频导入与混合播放：2026-09-20 17:26:15 发布

2026-09-20 17:26:15（北京时间）激活，17:33:17 生产只读检查通过，随后非作者复核通过。相册支持明确选择并保存视频、成员播放和逐台授权的照片／视频轮播；真实 Google 视频和实体电视仍待验。[集中验收](MEDIA-VIDEO-ACCEPTANCE.md#media-video-release)分别记录正式构建、三条本地视频浏览器流程、Linux 迁移与媒体资源、R5 完整控制器演练，以及生产一户两库保全。

R5 正常及注入部分失败两场景实际恢复完整三库，5347 份原件独立核验；R1–R4 失败原件保留，不把分轮测试相加。正式生产核对 1103 安装文件、三个服务各 134 运行文件与 decoder 4 文件，五服务零重启／非 OOM、app 与 decoder healthy、正常 TLS 200、备份 timer active。原 71／9 全表与 sequence 指纹保留、新两表在 app-only 停写窗口为空；此后 worker 可正常写入，当前只读审计不打开实时数据库、不证明实际用户视频处理或生产恢复。

## 本地日程隐私：2026-09-20 10:11:17 发布

2026-09-20 10:11:17（北京时间）激活，10:18:48 独立生产只读审计通过。[使用与集中验收](CALENDAR-PRIVACY-ACCEPTANCE.md#calendar-privacy-release)记录正式构建（两组 TypeScript、55 Node、23 导出）、后端 43 个唯一用例、实际 Linux 43/43 及发布工具 27 个唯一用例的各自范围。浏览器为 R1 后两项＋R4 首项分段通过；R1 断言、R2 标签、R3 CSP 失败原件保留，没有一次全三项最终重跑。

生产核对 1059 个安装文件、三个应用服务各 130 个运行文件、75 项正常 TLS 资源；四服务运行、零重启、app healthy。一户两库 71→71／9→9 全部数据在停写／app-only 窗口保全，两份备份只核字节、未恢复。29＋3 匿名 GET 拒绝不代替 POST 授权；真实云写入、实体电视及 A–D 本人完整验收（0/4）仍待验证。

## 日程时间重叠：2026-09-20 08:47:27 发布

2026-09-20 08:47:27（北京时间）激活，08:49:19 独立生产只读审计通过。[使用与集中验收](CALENDAR-CONFLICTS-ACCEPTANCE.md#calendar-conflicts-release)。核心 12 新用例＋1 原日历用例、fresh 安装／两组 TypeScript／34 Node／23 导出分别通过；浏览器为 R1 后两项与 R2 仅首项分段通过，共 3 流程、8 张有效截图。R1 首项合法滚动的截图误报及关闭警告保留，不能写成一次 3/3。

发布适配 32 个独立用例分轮通过（首轮 30 通过、两条测试假设修正后 2 项窄重验）；实际 Linux 3/3、零跳过，验证鉴权与 CSRF、日历导入去重、单日结束边界。测试存在重叠，不累加；没有重跑父版 82 项提醒后端。

生产审计核对 1043 个安装文件、三个应用服务各 129 个运行文件、75 个 TLS 静态资源及三个页面；四服务 running、零重启、app healthy。实际一户两库 71→71／9→9，全部表、settings、序列、提醒状态及回执保持；两份备份只核字节，未生产恢复。29＋3 项匿名 GET 返回 401，不代替 POST 授权。真实云写入、实体电视及 A–D 本人完整验收（0/4）仍待验证。

## 本人站内提醒：2026-09-20 07:52:22 发布

组合后端 82 项、fresh Expo 两组 TypeScript／31 Node／23 导出分别通过；真实浏览器 R1 全三流程通过，R2 第三流程补验中途失败保留，R3 仅第三流程 1/1 完成。不能把 R3 记成全三流程，也不把重复用例相加。

实际隔离 Linux 82/82、0 skip；另有真实两户三库 69→71／app-only 原 69 表保全、非空提醒回执重启及旧／新整组恢复演练。Windows 迁移工具 33 个唯一用例来自 32+1 分段，发布工具 45+3，演练工具 31+4（其中 1 项重复）；这些不是同一次 Linux 测试。实际原件与失败边界集中在[提醒验收](TASK-REMINDERS-ACCEPTANCE.md#task-reminders-release)。2026-09-20 07:52:22（北京时间）激活，07:56:49 独立生产只读审计通过。审计核对 1030 安装文件、三个应用服务各 129 运行文件、75 个 TLS 资源和三个入口；四服务正常，原 69 表／平台保全、两新表 app-only 为空。两份备份只核字节，未生产恢复；32 项匿名 GET 拒绝不能替代 POST 或本人流程。本人 A–D 仍 0/4，真实云目标待选，实体电视暂无法验证。

## 本地待办前置事项：2026-09-20 06:28:25 发布

**06:28:25（北京时间）实际激活，06:35:47 独立生产只读审计通过。** 正式 source `456585bc1a4f820ca238b3308f09a8d591628f41`，main `40ea731b28510f7bd7f3db3d0e06fade62a530f8` 同树。固定包、完整原件 SHA 与使用方式见[集中验收](TASK-DEPENDENCIES-ACCEPTANCE.md#task-dependencies-release)。

作者后端专项 36／相邻 284、组合后端 70、正式两组 TypeScript／64 Node／23 导出分别取得证据。真实临时浏览器 **R1 前两项通过＋R4 仅第三项通过**，合计三条唯一流程；R1 整体 2/3、R2 定位失败及 R4 退出前异步关闭警告保持原记录，不写成 R4 单次 3/3。实际隔离 Linux **354/354、零失败／错误／跳过**；与本地套件有重叠，不累加为全量覆盖。

独立生产审计核对 1007 安装文件、app／sync／media 各 128 运行文件、75 个正常 TLS 资源，四服务 running、0 重启、app healthy；一户两库 69/9 的停写与 app 启动保全原件一致。两份备份仅核验字节，未生产恢复；27+3 匿名 GET 为 401，不证明 POST 身份验证或本人操作。A–D 本人完整验收仍 0/4，真实云写入和实体电视未验。提醒后续发布的 82 后端／31 Node 等结果见文首，不计入该任务依赖历史批次。

此前资料与照片搜索于 05:15:29 发布、05:16:46 独立审计通过，其 R1／R2 分段浏览器与 Linux 175 原范围见[资料搜索验收](ASSISTANT-DOCUMENT-SEARCH-ACCEPTANCE.md#assistant-document-search-release)。以下历史结果保持各自当时范围。

## 采购截止日与优先级：2026-09-20 03:54:39 发布

本地后端 229／229、正式 R3 两组 TypeScript／106 Node／23 导出、R3 真实临时浏览器 4／4 流程与 8 图、1 次真实模型请求的 29 条检查记录、实际隔离 Linux 298／298（0 失败／错误／跳过）分别通过。后端 229 项包含在 Linux 298 项内；模型只有 28 个唯一检查名，不把这些数字累加为一次全量测试。

最终 source `67e5b0d5cafd28b50abce7e42fd64f64c8059997`，main PR #7；03:54:39（北京时间）激活，03:55:48 独立只读审计通过。实际一户两库 69/9 完整保全，980 个安装文件、app／sync／media 各 127 个运行文件及 75 个正常 TLS 资源匹配；24 项匿名 GET 与另 3 项助理 GET 探针均 401，不能代替 POST 或本人操作验收。四服务运行、0 重启、app 健康、env／marker 保持、备份定时器 active；两份停机备份仅核验字节，未执行恢复。完整身份、原件 SHA、历史失败、390px HelperText 裁切和 A–D 0/4／实体电视未验边界见[本批集中验收](SHOPPING-SCHEDULE-ACCEPTANCE.md#shopping-schedule-release)。本文不重写下面各历史发布的当时记录。

## Expo 旅行准备同步：2026-09-20 02:33:11 发布

**02:33:11（北京时间）实际激活，02:35:58 独立生产只读审计通过。** 固定 source `8f7af253261f28269465f3a77b6c8433f2dc8142`，main 合并提交 `b35a141af0822b980891a5a3d07ba1a8b90ff50b`。本批保持户内 69／平台 9 表；完整身份、原件 SHA 与使用路径集中在[本批验收](EXPO-JOURNEY-TASKS-ACCEPTANCE.md#expo-trip-task-publish-release)。

正式 R2 Expo 的两组 TypeScript、53 Node、23 产物；R3 临时 HTTPS Flask／SQLite／Edge 八条流程与六图；实际 Linux 61 项零失败／错误／跳过；发布策略 48 个唯一用例分别通过，不相加为一次全量测试。浏览器两种提供方 HTTP 均为合成替身。R1 的刷新竞态已修正并重新构建；R2 浏览器因 Windows 长路径八项 setup 失败、零业务流程执行，R3 同字节改用扩展路径通过。首轮 SCP 传输失败、策略测试 40 通过／8 长路径失败及仅重跑这 8 项的原件全部保留。

实际一户两库停写备份与新 app 启动后保全通过；独立审计核对 968 个安装文件、app／sync／media 各 127 个运行文件、固定 104 个非 Expo 文件和 75 个正常 TLS 资源。24 个匿名 GET 拒绝探针均为 401，另三个助理 GET 探针也均为 401；四服务运行、0 重启、app 健康，env／marker 保持、备份定时器 active。审计未调用 provider、写业务或执行恢复；匿名 GET 不证明本人 POST 流程。真实云与实体电视未验，A–D 本人完整验收仍为 0/4。下一批采购候选的后端 229／Node 104 不计入此版本。

以下保留各历史版本的当时记录。

**2026-09-18 05:49:18（北京时间）家庭与成员已激活，05:49:55 post 通过，05:55:19 独立读回及随后审计通过。** 实际 Linux 8 模块、227 项＝226 通过＋1 个精确 Windows junction 跳过，0 失败／错误／删选；本地 7／7 流程、8 图及客户端最终 13 项另行记录。58→58 只增角色列，旧数据与注册库保全，app 启动后等于迁移后快照。集中证据及边界见[成员验收](EXPO-HOUSEHOLD-MEMBERS-ACCEPTANCE.md#household-members-release)，各套数字不相加。
...
北京时间 07:38:17 实际激活，07:39:46 正常 TLS 读回通过。旅行简报可进入新建草稿，补齐准备／采购负责人和采购预算，再预览确认保存；地图沿用既有独立入口，本轮完整地图联动仍另验，完整场景 A 仍未完成。以下是实际安装身份，后续文档合并不重新构建或部署。
...
| 验证 | 已完成结果与边界 |
| --- | --- |
| Windows 组合 | 17 个受影响模块，424 passed／0 failed／0 errors／0 skipped，114.423 秒。含新 API、旧投资／财务、数据导出、打包及历史迁移回归；不是整个仓库全部测试 |
| 迁移／导出 | 早前单独 53 项已包含于最终 424 项，不相加；两户合成 54→55 精确空表新增、原数据保全、中断拒绝和含已删除持仓历史回执的完整恢复通过 |
| Node | 29 passed：持仓 helper 10、导入 helper 19；精确金额、Unicode 字符、回执绑定、失败读的身份与后台代次检查 |
| TypeScript | 在 `ab1fd81` 对相同前端源码完成完整 typecheck；第一次误从仓库根运行 npm，因找不到 package.json 失败，改在 frontend 执行后通过，没有修改代码 |
| Expo Web | `expo-holdings-build-20260917-r1` 从上述冻结 source 导出 23 文件；主 JS 为 `entry-c01ccc43ca15d3e67a78afc98c61276f.js` |
| 浏览器 | 实际临时 Flask／SQLite／Edge 7 场景通过，0 页面错误／外站请求；source 与 bundle 字节在运行前后相同，未写生产 |
| 截图 | 12 张，320／390／1040／1440 四宽；root 与文档作者人工查看其中 7 张。所见列表、编辑器与导入上半区无阻断裁切；内部滚动区的完整前后预览和确认按钮未被这些截图覆盖 |

七个浏览器场景分别为：手工增改删与重启后读回（未知估值与零分开）；丢失真实手工 POST 回复后读取原操作回执；文件预览不写持仓、明确确认与稳定来源追溯、重复文件回放；丢失真实导入成功回复后读取文件回执；错误金额阻断及 XLSX 显式选表和精确金额；真实并发 PATCH 触发 409、保留草稿并核对后确认新版本；离线／后台隐藏及真实成员登录切换隔离。业务成功未由 mock 代替。跨家庭及电视隔离在后端测试中验证，不属于本次浏览器 7 场景；本人实财务、新云操作、实体电视及原生安装均未验收。

### 原件与修订记录

以下目录均在私有 `family-dashboard-access/` 下，报告、截图与构建产物不入 Git；本轮文档已重新读取并核对三份原件的 SHA256。

| 原件 | SHA256 |
| --- | --- |
| `worktrees/integration/test-results/holdings-combined-r1.xml` | `7de7e6e274e0c6182ba4db46fbb47d70ba586115f27f4c033b9901e7e2a62f5f` |
| `expo-holdings-build-20260917-r1/build-evidence.json` | `d1502cfd7b49a1914ab1d5fa3887606c1abdc7185cf9e84905b28a4b9527f484` |
| `worktrees/integration/test-results/expo-investments-20260916T212444497834Z/result.json` | `d1b5529e20b6d070dd816fc30332ab9507fb69d727638ccd85ddfb69590a345c` |

浏览器执行脚本 SHA256 为 `f866fbb016c1cd259cefa92d855ee0d40cef475119a24e33eb6fd042b194e022`，实际加载的财务 fixture 为 `4bcb31d3a4acf329569f8db1f51ba4e898dd2031625c1badb30c4e718bd92e95`；脚本与 fixture 均校验对应冻结源码字节。人工查看范围为 editor 390／1440、list 320／1440、import 320／390／1440；手机截图不会展开内部滚动区域。

后端早期 r1 因经典 PATCH 错误检查顺序出现 1 个失败，修正后 r2 为 222 passed；进一步修正 GET 在读取快照结束后的真实会话检查后，r3 为 225 passed。上述局部测试均由最终 424 项覆盖，不重复累加。非作者审查还修正了导入回执失败后的身份检查、迟到 404 沿用原代次、合法非 BMP 字符长度，以及浏览器脚本对保存后详情页和真实 fixture 来源的核对。它们是源码／测试修订，没有生产故障或回滚。

候选使用与数据模型见 [Expo 持仓](EXPO-INVESTMENTS.md)、[持仓 API](INVESTMENTS-API.md)、[55 表模型](DATA-MODEL.md#investment-operations55)。完整资产账户、负债管理、估值历史、FX 及收益归因尚未实现；不能用本轮持仓记录管理宣称这些目标完成。
