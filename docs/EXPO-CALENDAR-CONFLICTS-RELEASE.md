...
**使用状态：**本适配已完成 2026-09-20 08:47:27（北京时间）激活，08:49:19 独立生产只读审计通过。固定包和已消费计划见[使用与集中验收](CALENDAR-CONFLICTS-ACCEPTANCE.md#calendar-conflicts-release)；下方描述保留当时父版绑定，不能直接重放为下一次部署。
