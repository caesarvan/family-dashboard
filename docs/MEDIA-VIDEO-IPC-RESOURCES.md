...
每个 Docker 操作保留固定 argv、开始时间、子进程 PID、原 stdout／stderr、SHA、退出码；只跟踪原句柄，未知结果不重放。每个角色分别保存 cgroup 实际限额、`memory.peak/events`、CPU／pids、退出码、实际加载模块与工具摘要。峰值包含各自 fixture／应用启动全过程，不把阶段采样或两个容器的数值相加后冒充单进程峰值。
