...
本批资源R2已实际通过并独审：63dc源／b63镜像，四个真实请求均200且服务端重叠，独立恢复200；扫描计数8／恢复2、请求期间图片／视频BLOB读取0，数据库摘要一致。384／64MiB两容器未触发内存事件，123件原件、监控及owned收尾已核；仅证明这组元数据负载。旧R1绑定455源，在四请求开始前因客户端只读数据库观察失败，外层又报缺失 `database.json`，两层失败保留。本批仍未生成发布计划或上线，完整身份与证据见 [候选验收](DISCOVERY-ACCEPTANCE.md#discovery-candidate)。`deploy/discovery_duplicate_linux_probe.py` 只运行一次隔离 profile，不构建镜像、不访问生产、不降低限额、不自动重试；沿用已审 Docker 命令原件、主机内存监控和 cgroup 读取 helper。
