...
联审修正：Provider 与 15 个异步面板统一使用 `sessionIdentity.ts`，四项新身份字段在操作前后都参与校验。真实 Photo/Members/Finance/Device fence 专项 5 项通过。前端开启 TypeScript 的 `allowImportingTsExtensions`（Expo `noEmit`），保证同一源文件能直接供 Node 测试及 Metro 使用。

连接间共享平台锁租约，按最后一个事务释放，覆盖连接按非嵌套顺序提交的情况。个人切换可从无效旧路由或已缺失旧库恢复到有效目标；旧库不重建，目录明确标为不可用，个人代次旋转仍撤销原派生身份。新增修正后的主接线专项同轮 13 项通过（XML：`test-results/membership-wiring-r2.xml`），尚未替代全量组合验证。

恢复入口进一步独立于家庭数据：公共 Expo HTML、资源及首页只使用原文件白名单，不读取成员会话；无效家庭 Cookie 或缺失家庭库仍能加载账户恢复界面。家庭 API 继续返回明确错误，默认户缺库同子户返回 JSON 503 与恢复地址，不创建空库。会话证明同时绑定实际 Request 和原 Flask 应用，避免旧 app context 遗留身份或新家庭初始化借用父家庭会话。

组合 R2 实际为 390 通过、1 失败；唯一失败是已登录成员创建另一家庭时，父请求的证明被新应用初始化误用。此失败原件保留，后续应用来源绑定修正单独复测，不把 R2 改记为全通过。恢复 shell 与原静态/认证边界实际 57 项通过，另新增两项真实 `/app` 冷启动恢复检查通过；这些本地合成检查不代表线上发布。
