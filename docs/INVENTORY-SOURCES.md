...
实现为 `inventory_sources.py`，由每个家庭的 `inventory_api.register_inventory` 显式注册，沿用 `inventory_source_links` 和 `inventory_operations`，无新表或 DDL 变化。`inventory_core.py` 的 `apply_with_receipt` 负责回执、当前库存 ACL、双 revision 和事务 savepoint。本文是接口契约；[操作说明](EXPO-INVENTORY-SOURCES.md)和[本批验收](INVENTORY-SOURCES-ACCEPTANCE.md#inventory-sources-release)分别记录使用、实际测试与发布状态。功能已于 2026-09-19 13:57:12（北京时间）激活，13:59:04 独立只读审计通过。

## 使用路径与隐私

从本人财务订单详情进入，明确选择一条商品行；可打开已有可见关联，或选择一个库存物品及本人创建的 `purchase` 批次。只有本人明确点击「用此商品新建私人物品」后，才将所选商品的名称、规格预填为可编辑草稿；默认保持 `private`，不自动保存或共享，单位和数量仍须本人手动填写。日后本人明确共享该物品时，当前物品名称、规格按普通库存权限可见，私人订单来源仍不共享。已有新物品／新批次表单继续负责数量、单位、共享范围和可选采购关联。创建后再预览并明确关联，取消关联不会删除已创建的物品或批次。每个创建操作沿用自己的既有回执，未知结果先查询原操作编号，不能重新建一份。
...
本模块测试使用真实合成 XLSX 导入、真实 Cookie/CSRF 和独立临时数据库，覆盖重导、来源修改/删除、隐私/家庭/会话、并发、未知结果、各写入点回滚以及无金额和库存数量变化。受验 API head `f415edf938bb709180b855e9474fb1e8d2c65385` 的来源、既有库存、售后、导出和淘宝分组五模块共 227/227 通过；后续只修订预填说明，运行与测试字节保持。精确原件及其他独立验证见[本批验收](INVENTORY-SOURCES-ACCEPTANCE.md#inventory-sources-release)，不将合成测试视为本人真实家庭数据验收。
