...
**实际结果：R5 已在独立 Linux 场景完成正常和注入部分失败两条链，非作者审查通过。** 固定工具 source `971487d3`，5347 份原件核验；两场景均执行原完整组恢复及回退核验。详见[集中验收](MEDIA-VIDEO-ACCEPTANCE.md#media-video-release)。本页保留工具合同和此前失败历史；作者本机离线测试与协调者 Linux 实际执行分别记录，不表示曾恢复生产数据库。
...
第四轮成功场景已完成真实迁移、健康、全组三库恢复及 `verify_rollback`；失败场景在第二户只读迁移报错后已收停，但观察器因遗留 0 字节 WAL／32768 字节 SHM 拒绝继续，因此整轮仍失败、该场景尚未恢复。窄修仅在再次证明无卷／项目写者后，允许本轮精确注入第二户的这对普通侧文件供 immutable 观察；其它库侧文件、非空 WAL、rollback journal、链接或非普通文件均拒绝。观察前后保存 DB／侧文件哈希、大小、模式、inode 和 mtime，并要求完全相同，不删除或 checkpoint 日志。恢复前再核对冻结观察记录及停写状态，只将本轮注入 DB 和这对已核侧文件改回 0600，保留变更前后证据；固定 SQLite 恢复算法、控制器和资源限额不变。新代码须独审与全新 Linux 演练，不把本地 SQLite／文件边界测试当作完整恢复通过。
