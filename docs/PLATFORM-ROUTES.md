...
Expo 发布增量：在下方既有 145 个方法／路径模板之外，`frontend_runtime.py` 新增以下三个静态托管 GET 入口，合计 148；业务 API 与 53+2 表不变。`/app/` 是同一入口的尾斜线形式，不另计。具体资源白名单、缺失资产与 OAuth 返回规则见 [Expo Web 契约](EXPO-WEB.md)。下方生成表及机器 JSON 保留既有基线。

| 方法 | 新增路径 | 用途 |
|---|---|---|
| GET | `/classic` | 已有高级模块前端 |
| GET | `/app` | Expo 导出首页 |
| GET | `/app/<path:name>` | 客户端深链和允许的生成资产 |
...
Flask HTTP 方法与路径组合：**145**；另有 `GET /space/<slug>`。HEAD/OPTIONS 不重复列出。动态 `<action>` 路由算一个模板，允许的具体动作见 [待办发布契约](TASK-PUBLISH.md)。
