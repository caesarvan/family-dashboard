...
R1 实际整体 2/3：第二、三条通过；第一条在修改原事件并刷新后，通用裁切检查把横向日期条中可滚动到的按钮误报，最终 DB proof 尚未写出。原失败保留。修正只将具有真实水平滚动祖先、在其内容范围内、可完整滚入可视区域的条目另记为 scrollableClipped；祖先本身越出页面、固定定位、隐藏裁切或非滚动控件仍不能豁免。页面／root 宽度、其他控件裁切、全文高度断言保持。定向静态 Edge 用例检查可滚达的日期条和三类真实越界，不代替产品流程补验；修正审查后仅需重新执行第一条。
