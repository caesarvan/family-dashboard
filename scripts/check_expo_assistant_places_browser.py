"""Run only a reviewed fixed source + fresh Expo export against synthetic assistant place navigation."""
import argparse
...
from browser_expo_assistant_places_check import Run, CASES, CASE_SCREENSHOTS, HARNESS

WRAPPER = 'scripts/check_expo_assistant_places_browser.py'
BUILD_REUSE_PATHS = frozenset({WRAPPER, HARNESS, 'tests/test_expo_assistant_places_browser.py', 'docs/ASSISTANT-PLACES-BROWSER.md'})
...
def exclusive_output(root):
    out = root / 'test-results' / ('expo-assistant-places-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
...
def main():
    ...
    assert Path(sys.modules['browser_expo_assistant_places_check'].__file__).resolve() == (root / HARNESS).resolve()
    ...
    report = dict(passed=False, checks=[], pageErrors=[], externalRequests=[], unexpectedProviderAttempts=[],
        ...
        scope='Actual local Flask/SQLite/HTTPS/Edge. Synthetic original places/trips and explicitly confirmed local PNG; delayed completed real reads, offline events and actual member login. No business DTO stubs, map/model/provider calls, production or physical-TV acceptance; only requestedCases ran.')
