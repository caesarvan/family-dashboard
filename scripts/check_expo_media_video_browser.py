"""Run only a reviewed fixed source + fresh Expo export against synthetic local media."""
import argparse
...
from browser_expo_media_video_check import Run, CASES, CASE_SCREENSHOTS, HARNESS
from media_videos import VideoTools

WRAPPER = 'scripts/check_expo_media_video_browser.py'
BUILD_REUSE_PATHS = frozenset({WRAPPER, HARNESS, 'tests/test_expo_media_video_browser.py', 'docs/MEDIA-VIDEO-BROWSER.md'})
...
def exclusive_output(root):
    out = root / 'test-results' / ('expo-media-video-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
    out.mkdir(parents=True, exist_ok=False)
    return out


def tool_identity(path, expected):
    path = path.resolve()
    assert path.is_file() and re.fullmatch('[a-f0-9]{64}', expected) and sha(path) == expected
    result = subprocess.run([str(path), '-version'], capture_output=True, timeout=10)
    assert result.returncode == 0
    return {'path': str(path), 'sha256': expected, 'version': result.stdout.decode('utf-8', errors='replace')}
...
def main():
    ...
    assert Path(sys.modules['browser_expo_media_video_check'].__file__).resolve() == (root / HARNESS).resolve()
    # A member-only or pre-protocol bundle/source must fail before opening a browser.
    assert (root / 'media_playback_progress.py').is_file()
    assert (root / 'frontend/src/ui/TVPhotoPlayer.web.tsx').is_file()
    tools = VideoTools(str(args.ffmpeg.resolve()), str(args.ffprobe.resolve()))
    identities = {'ffmpeg': tool_identity(args.ffmpeg, args.ffmpeg_sha256),
                  'ffprobe': tool_identity(args.ffprobe, args.ffprobe_sha256)}
    ...
    report = dict(passed=False, checks=[], pageErrors=[], externalRequests=[], unexpectedProviderAttempts=[],
        ...
        buildSourceDelta=build_delta, buildReusePolicy='only-four-reviewed-tool-paths; identical-complete-frontend-and-build-inputs',
        sourceRoot=str(root), bundleRoot=str(bundle), sourceHashesBefore=hashes(), bundleHashesBefore=export_hashes(bundle),
        tools=identities, productionWrites=0, realCloud=False, realModel=False, physicalTelevision=False,
        scope='Actual Flask/SQLite/HTTPS/Chromium and FFmpeg-generated 8-second media; synthetic Google transport. Native video time and ended observations, real confirmation, current ACL and per-TV grants. Only requestedCases executed. Not real Google import, Linux resource, long/large media, native-app or physical-TV acceptance.')
    ...
    try:
        with patch.object(socket.socket, 'connect', local_connect), sync_playwright() as pw:
            ...
            try:
                Run.run_scenarios(root, runtime_bundle, report, out, browser, temp_root, tools, cases)
        ...
        try:
            ...
            report['passed'] = report['passed'] and all(report[k] for k in ('sourceUnchanged', 'bundleUnchanged', 'fixturesUnchanged', 'toolsUnchanged', 'sourceStillFrozen', 'temporaryFixtureRemoved'))
        except Exception:
