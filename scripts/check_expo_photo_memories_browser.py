"""Run only a reviewed fixed source + fresh Expo export against synthetic local photo memories."""
import argparse
...
from browser_expo_photo_memories_check import Run, CASES, CASE_SCREENSHOTS, HARNESS

WRAPPER = 'scripts/check_expo_photo_memories_browser.py'
BUILD_REUSE_PATHS = frozenset({WRAPPER, HARNESS, 'tests/test_expo_photo_memories_browser.py', 'docs/PHOTO-MEMORIES-BROWSER.md'})
...
def exclusive_output(root):
    out = root / 'test-results' / ('expo-photo-memories-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
...
def main():
    ...
    assert Path(sys.modules['browser_expo_photo_memories_check'].__file__).resolve() == (root / HARNESS).resolve()
    ...
    report = dict(passed=False, checks=[], pageErrors=[], externalRequests=[], unexpectedProviderAttempts=[],
        ...
        scope='Actual local Flask/SQLite/HTTPS/Edge. Confirmed synthetic JPEGs and real encrypted source metadata. Only provider job completions, server clock, lifecycle/offline events and delayed real reads are fixtures. No business DTO stubs, production, cloud, model, physical-TV display or native-app acceptance; only requestedCases ran.')
