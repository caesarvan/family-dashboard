...
async function boot(){
  ...
  try{
    ...
    if(JSON.stringify([user?.role,user?.householdId,user?.id,user?.auth_version,csrf])!==JSON.stringify([me.user?.role,me.user?.householdId,me.user?.id,me.user?.auth_version,me.csrf||'']))window.JourneyMap?.notifyIdentityChanged();
...
async function refresh(force=false){
  ...
  try{const next=await api('/state');const changed=force||!data||next.revision!==data.revision;data=next;online=true;lastFetch=displayTime(new Date());if(changed)renderBoard();else {updateFooter();window.JourneyMap?.notifyStateChanged()}}
