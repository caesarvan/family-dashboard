...
window.HouseholdMedia = (() => {
  ...
  async function mount(node,{openJourney,initialJourneyId,returnToMap,onIdentityChanged} = {}) {
    ...
    f.submit = event => {
      ...
      if (event.target.matches('[data-hm-filters]')) {clearSuggestions(f.editor); const values = new FormData(event.target); f.scope = values.get('scope'); if (!f.mapVisit) f.journeyId = values.get('journeyId'); f.offset = 0; void job(f,check => gallery(f,check));}
    ...
    f.offline = () => {if (!alive(f)) return; clearSuggestions(f.editor); if (f.mapVisit) {f.epoch++; f.busy = false; clearTimeout(f.timer); concealGallery(f); f.journeys = []; f.error = '网络已断开，照片已收起。恢复连接后重新读取。'; render(f);} else {renderSuggestions(f); renderMessage(f);}};
