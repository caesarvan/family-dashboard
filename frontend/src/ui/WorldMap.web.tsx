...
export default function WorldMap({ places, selected, disabled, picking, picked, onSelect, onPick, routeMap, selectedRouteIndex, onRouteSelect }: WorldMapProps) {
  ...
  return <View style={{ gap: 10 }}>
    <svg ref={svg} viewBox="0 0 1000 500" preserveAspectRatio="none" role={picking ? 'application' : 'group'} aria-label={picking ? '地图选点：方向键移动，Shift 加快，回车确认' : routeMap ? '旅行路线顺序示意；编号对应下方站点列表，不是导航' : '世界足迹地图；所有地点也可在下方列表打开'}
      ...
      {routeMap?.lines.map((line, i) => {
        const a = project(line.from), b = project(line.to);
      ...
      {routeMap?.markers.map((marker, i) => {
        const projected = project(marker.point), point = { x: Math.max(18, Math.min(982, projected.x)), y: Math.max(18, Math.min(482, projected.y)) }, active = marker.indices.includes(selectedRouteIndex ?? -1), label = marker.indices.map(n => n + 1).join('、');
        ...
        return <g key={'route-marker-' + i} role="button" tabIndex={disabled ? -1 : 0} aria-disabled={!!disabled}
          ...
          <title>{`第 ${label} 站`}</title><circle cx={point.x} cy={point.y} r="16" fill={theme.colors.surface} stroke={theme.colors.primary} strokeWidth={active ? 4 : 2} strokeDasharray={marker.approximate ? '4 3' : undefined} />
          <text x={point.x} y={point.y + 4} textAnchor="middle" fontSize="12" fontWeight="600" fill={theme.colors.onSurface}>{marker.indices.length > 3 ? `${marker.indices[0] + 1}…` : label}</text>
        </g>;
      })}
    ...
    {picking ? <Text variant="bodySmall" accessibilityLiveRegion="polite">选点仅提供大致位置。光标：{cursor.latitude.toFixed(2)}，{cursor.longitude.toFixed(2)}。方向键移动，回车写入；也可手填经纬度。</Text> : routeMap ? <Text variant="bodySmall">数字为站点顺序 · 虚线为大致位置 · 缺少坐标处断开。连线不是导航、实际路径或距离。</Text> : <Text variant="bodySmall">● 已到访　■ 已计划　○ 想去 · 仅显示本页有权查看的坐标</Text>}
