...
export default function ItemEditor({kind,item,onDismiss}:{kind:ItemKind;item?:Entity;onDismiss:()=>void}) {
  ...
  return <Portal><Dialog visible onDismiss={()=>{if(!busy&&!uploading)onDismiss();}} style={styles.dialog}>
    ...
    <Dialog.ScrollArea style={styles.scroll}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.fields}>
      <TextInput {...common} accessibilityLabel={kind==='shopping'?'物品名称':'名称'} label={kind==='shopping'?'物品名称':'名称'} value={title} onChangeText={setTitle} maxLength={100} autoFocus />
      ...
      {kind==='shopping'&&<><TextInput {...common} accessibilityLabel="数量" label="数量" value={quantity} onChangeText={setQuantity} maxLength={30}/><TextInput {...common} accessibilityLabel="预计总价（元，可选）" label="预计总价（元，可选）" value={budget} onChangeText={setBudget} keyboardType="decimal-pad"/><View style={styles.photos}>{photos.map((id,index)=><View key={id}><Image source={{uri:'/api/photos/'+encodeURIComponent(id)}} style={styles.photo}/><IconButton icon="close" accessibilityLabel={'移除第'+(index+1)+'张图片'} size={16} disabled={locked} onPress={()=>{if(!locked)setPhotos(values=>values.filter(value=>value!==id));}}/></View>)}</View><Button mode="outlined" icon="image-plus" disabled={locked||photos.length>=3} loading={uploading} onPress={addPhoto}>添加参考图片 · {photos.length}/3</Button><Text variant="bodySmall">保存后，参考图片与家庭共享。</Text></>}
      {kind==='events'&&<><TextInput {...common} accessibilityLabel="开始日期（YYYY-MM-DD）" label="开始日期（YYYY-MM-DD）" value={startDay} onChangeText={setStartDay}/>{!allDay&&<TextInput {...common} accessibilityLabel="开始时间（HH:mm）" label="开始时间（HH:mm）" value={startTime} onChangeText={setStartTime}/>}<TextInput {...common} accessibilityLabel={allDay?'结束日期（不包含当天）':'结束日期（YYYY-MM-DD）'} label={allDay?'结束日期（不包含当天）':'结束日期（YYYY-MM-DD）'} value={endDay} onChangeText={setEndDay}/>{!allDay&&<TextInput {...common} accessibilityLabel="结束时间（HH:mm）" label="结束时间（HH:mm）" value={endTime} onChangeText={setEndTime}/>}<Checkbox.Item label="全天安排" status={allDay?'checked':'unchecked'} disabled={locked} onPress={()=>{if(!locked)setAllDay(!allDay);}}/><TextInput {...common} accessibilityLabel="地点（可选）" label="地点（可选）" value={location} onChangeText={setLocation} maxLength={200}/><Text variant="bodySmall">时间使用北京时间</Text></>}
      <List.Accordion title="更多选项" left={props=><List.Icon {...props} icon="tune"/>}>
        <View style={styles.fields}>{kind==='tasks'&&<TextInput {...common} accessibilityLabel="截止日期（YYYY-MM-DD，可选）" label="截止日期（YYYY-MM-DD，可选）" value={due} onChangeText={setDue}/>}<TextInput {...common} accessibilityLabel="备注（可选）" label="备注（可选）" value={note} onChangeText={setNote} multiline maxLength={500}/>{kind!=='events'&&!taskCloud&&<Checkbox.Item label={kind==='shopping'?'已买到':'已完成'} status={done?'checked':'unchecked'} disabled={locked} onPress={()=>{if(!locked)setDone(!done);}}/>} {kind==='shopping'&&<TextInput {...common} accessibilityLabel="实际总价（元，可选）" label="实际总价（元，可选）" value={actual} onChangeText={setActual} keyboardType="decimal-pad"/>}</View>
      </List.Accordion>
