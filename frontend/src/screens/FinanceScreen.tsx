...
function FinanceWorkspace(props: ScreenProps & { identityKey: string }) {
  ...
  return <View style={styles.page}>
    <PageHeader title={detail && privateVisible ? '交易详情' : '家庭资金'} description={detail && privateVisible ? '保留原始金额，核对分类与关联。' : '共同资金一起看，个人账本自己管。'} action={<View style={styles.row}>{detail && privateVisible ? <Button disabled={locked} onPress={closeDetail}>返回账本</Button> : <Button icon="file-upload-outline" mode="contained" disabled={locked || !privateVisible} onPress={() => { closeDetail(); setImporting(true); }}>导入账单</Button>}<Button icon="refresh" disabled={busy || !online() || denied.current} onPress={() => { if (!active.current) enter(); else void reload(true); }}>{pending ? '刷新核对结果' : '刷新账本'}</Button></View>} />
    ...
    {privateVisible && data && <>
      ...
      {!detail ? <>
        ...
        <View style={styles.row}><Button icon="chart-donut" disabled={locked} onPress={() => props.onLegacy('finance')}>资产与投资管理</Button><Button icon="folder-outline" disabled={locked} onPress={() => props.onLegacy('finance')}>来源报告与高级财务</Button></View><Text style={styles.muted}>资产、持仓导入与来源报告暂在经典财务页面管理。</Text>
      </> : <>
