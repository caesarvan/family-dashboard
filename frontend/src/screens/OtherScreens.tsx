...
export function FinanceScreen(props:ScreenProps) {
  ...
  return <View style={styles.page}><PageHeader title="家庭资金" description="共同资金一起看，个人明细自己管。" action={<Button mode="contained" onPress={()=>props.onLegacy('finance')}>核对与管理</Button>}/>
    <SectionCard title="共同余额"><Text variant="displaySmall" style={{fontWeight:'700'}}>{confirmed?money(finance.wallet):'待核对'}</Text><Text variant="bodySmall" style={{color:theme.colors.onSurfaceVariant}}>周转目标 {money(finance.reserveTarget)}</Text></SectionCard>
...
export function TripsScreen(props:ScreenProps) {
  const trips=[...props.state.trips].sort((a,b)=>a.start.localeCompare(b.start));
  return <View style={styles.page}><PageHeader title="旅行计划" description="下一站，一起出发。" action={<Button mode="contained" icon="plus" onPress={()=>props.onLegacy('trips')}>规划旅行</Button>}/>
    {!trips.length?<EmptyState title="下一站，想去哪里？" description="从出发日期和目的地开始。" action={<Button onPress={()=>props.onLegacy('trips')}>开始计划</Button>}/>:trips.map(trip=><SectionCard key={trip.id} title={trip.title} action={<Button onPress={()=>props.onLegacy('trips')}>查看行程</Button>}><Text variant="bodyLarge">{trip.destination}</Text><Text>{trip.start} — {trip.end}</Text><Text>总预算 {money(trip.budget)} · 已付 {money(trip.paid)}</Text></SectionCard>)}
  </View>;
}
export function MoreScreen(props:ScreenProps) {
  ...
  return <View style={styles.page}><PageHeader title="更多" description="常用工具，集中在这里。"/>{groups.map(group=><SectionCard title={group.title} key={group.title}>{group.items.map(([route,title,icon],index)=><React.Fragment key={route}>{index>0&&<Divider/>}<List.Item title={title} left={p=><List.Icon {...p} icon={icon}/>} right={p=><List.Icon {...p} icon="chevron-right"/>} onPress={()=>route==='trips'||route==='finance'?props.onNavigate(route):props.onLegacy(route)} /></React.Fragment>)}</SectionCard>)}</View>;
}
