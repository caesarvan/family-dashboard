...
import {compareShoppingItems, shoppingScheduleText, copy, editDraft, eventDates, initialPlanningDraft, memberKey, newDraft, newKey, previewPayload, purchaseBudgetText, readForMember, recordVersions, SessionChangedError, summaryCounts, upgradeDraft, validateReceipt, type Draft, type Journey, type Plan, type Preview} from '../lib/trips';
import {EmptyState, PageHeader, SectionCard} from '../ui/components';
import {ShoppingScheduleFields} from '../ui/ItemEditor';
...
export default function TripsScreen(props:Props) {
  ...
  const journeyFinancePending=useRef(false);
  const pendingJourneyFinance=useCallback((value:boolean)=>{journeyFinancePending.current=value;props.onJourneyFinancePending?.(value);},[props.onJourneyFinancePending]);
  ...
  useFocusEffect(useCallback(()=>{focused.current=true;return()=>{focused.current=false;setPanel(current=>current?.kind==='finance'&&journeyFinancePending.current||current?.kind==='routes'||current?.kind==='reschedule'||current?.kind==='tasks'&&reschedulePending.current||current?.kind==='documents'&&documentsPending.current||current?.kind==='segments'&&segmentsPending.current||current?.kind==='import'&&tripImportPending.current?current:null);setMapReturn(undefined);};},[identityKey]));
  ...
  useEffect(()=>{
    ...
    return()=>{alive.current=false;++readVersion.current;++importReturnEpoch.current;importReturnController.current?.abort();pendingReschedule(false);pendingDocuments(false);pendingSegments(false);pendingTripImport(false);pendingJourneyFinance(false);};
  },[actor]);
  ...
  const openPanel=(kind:'calendar'|'tasks'|'places'|'reschedule'|'documents'|'segments'|'recap'|'routes'|'finance')=>{if(!canNavigate()||!detail||busy||reading||draft||documentsPending.current||segmentsPending.current)return;++readVersion.current;setPanel({kind,journeyId:detail.id,tripId:detail.tripId});};
  ...
  if(panel?.kind==='finance')return <JourneyFinancePanel key={identityKey+':'+panel.journeyId} journeyId={panel.journeyId} onPendingChange={pendingJourneyFinance} onBack={()=>{if(!canNavigate()||journeyFinancePending.current)return;pendingJourneyFinance(false);backToTrip(panel.tripId);}}/>;
  ...
  if(panel?.kind==='tasks')return <JourneyTasksPanel journeyId={panel.journeyId} onPendingChange={pendingReschedule} onBack={()=>backToTrip(panel.tripId)} onConnections={()=>props.onNavigate('connections')}/>;
  ...
  return <View style={styles.page}>
    ...
    {draft?<>
      <SectionCard title="基本安排"><View style={styles.fields}>
        ...
        <Text variant="bodySmall">{draft.plan.schemaVersion===2?'参考时区：'+draft.plan.referenceTimezone+'。改变总日期不会自动移动分段、准备或采购截止。':'日期包含出发和返程当天，生成北京时间的全天日程。已有分段和截止日期不会随总日期自动移动。'}</Text>
        {!!draft.journeyId&&<Text variant="bodySmall">需要一起调整行程、准备或采购截止、地点日期时，请先结束本次编辑，再从旅行详情进入「调整日期」。</Text>}
      ...
      <List.Accordion title="准备清单与采购" description={draft.plan.checklist?`${draft.plan.checklist.length} 项准备 · ${draft.plan.shopping.length} 件采购`:'保存时生成常用准备清单'}>
        <View style={styles.fields}>{!draft.plan.checklist?<><Text>准备建议由服务器根据当前计划生成，可先展开，再分配负责人。</Text><Button accessibilityLabel="展开并调整建议" disabled={locked} loading={busy==='preview'} onPress={()=>void makePreview(true)}>展开并调整建议</Button><Button disabled={locked} onPress={()=>change(plan=>{plan.checklist=[];})}>改为自己填写</Button></>:<>{draft.plan.checklist.map((row,index)=><View style={styles.fields} key={row.key}>{field('准备事项 '+(index+1),row.title,value=>change(plan=>{plan.checklist![index].title=value;}))}{ownerChoices('准备负责人 '+(index+1),row.owner,value=>change(plan=>{plan.checklist![index].owner=value;}))}{field('准备截止 '+(index+1),row.due||'',value=>change(plan=>{plan.checklist![index].due=value;}),10)}<Button disabled={locked} onPress={()=>change(plan=>{plan.checklist!.splice(index,1);})}>移出准备事项 {index+1}</Button></View>)}<Button disabled={locked||draft.plan.checklist.length>=100} icon="plus" onPress={()=>change(plan=>{plan.checklist!.push({key:newKey(),title:'',owner:'shared',due:plan.start,note:'',category:'preparation'});})}>增加准备事项</Button></>}
          {draft.plan.shopping.map((row,index)=><View style={styles.fields} key={row.key}>{field('采购名称 '+(index+1),row.title,value=>change(plan=>{plan.shopping[index].title=value;}))}{field('采购数量 '+(index+1),row.quantity,value=>change(plan=>{plan.shopping[index].quantity=value;}),30)}{ownerChoices('采购负责人 '+(index+1),row.owner,value=>change(plan=>{plan.shopping[index].owner=value;}))}<ShoppingScheduleFields suffix={' '+(index+1)} due={row.due??''} priority={row.priority} disabled={locked} onDueChange={value=>change(plan=>{plan.shopping[index].due=value;})} onPriorityChange={value=>change(plan=>{plan.shopping[index].priority=value;})}/>{field('采购预算（元，可不填） '+(index+1),purchaseBudgetText(draft,row),value=>purchaseAmount(row.key,value),14)}<Text variant="bodySmall">留空表示尚未估算，0 表示预计无需花费。保存后可添加参考图片。</Text><Button disabled={locked} onPress={()=>change(plan=>{plan.shopping.splice(index,1);})}>移出采购 {index+1}</Button></View>)}<Button icon="plus" disabled={locked||draft.plan.shopping.length>=100} onPress={()=>change(plan=>{plan.shopping.push({key:newKey(),title:'',quantity:'1 件',owner:'shared',budget:null,note:'',due:'',priority:'normal'});})}>增加采购</Button><Text variant="bodySmall">移出计划的已有记录保留为独立事项，完成状态与图片不会删除。</Text>
        </View>
      </List.Accordion>
      ...
      {preview&&<SectionCard title="确认变更"><View style={styles.fields}>
        ...
        {preview.plan.shopping.map(item=><Text variant="bodySmall" key={item.key}>{item.title} · {item.owner==='shared'?'一起':props.state.people.find(person=>person.id===item.owner)?.name||'家庭成员'} · {shoppingScheduleText(item,'',true)} · 预算 {money(item.budget)}</Text>)}
      ...
      {detail?<>
        <View style={styles.wrap}><Button mode="outlined" accessibilityLabel="我的旅行费用" disabled={!!busy||reading||!online} onPress={()=>openPanel('finance')}>我的旅行费用</Button><Button mode="outlined" icon="map-marker-path" accessibilityLabel="旅行路线" contentStyle={{minHeight:44}} disabled={!!busy||reading||!online} onPress={()=>openPanel('routes')}>旅行路线</Button><Button mode="outlined" icon="image-multiple-outline" accessibilityLabel="旅行回顾" contentStyle={{minHeight:44}} disabled={!!busy||reading||!online} onPress={()=>openPanel('recap')}>旅行回顾</Button><Button mode="outlined" icon="airplane" accessibilityLabel="行程分段" contentStyle={{minHeight:44}} disabled={!!busy||reading||!online} onPress={()=>openPanel('segments')}>行程分段</Button><Button mode="outlined" icon="file-document-outline" accessibilityLabel="旅行资料" contentStyle={{minHeight:44}} disabled={!!busy||reading||!online} onPress={()=>openPanel('documents')}>旅行资料</Button><Button mode="outlined" icon="calendar-edit" accessibilityLabel="调整日期" disabled={!!busy||reading||!online} onPress={()=>openPanel('reschedule')}>调整日期</Button><Button mode="outlined" icon="map-marker-outline" disabled={!!busy||reading||!online} onPress={()=>openPanel('places')}>旅行地点</Button><Button mode="outlined" icon="calendar-sync-outline" disabled={!!busy||reading||!online} onPress={()=>openPanel('calendar')}>同步到日历</Button></View>
        <SectionCard title={`准备 · ${detail.progress.done}/${detail.progress.total}`} action={<View style={styles.wrap}><Button disabled={!!busy||reading||!online} icon="cloud-sync-outline" onPress={()=>openPanel('tasks')}>同步到主清单</Button><Button disabled={!!busy||!online} onPress={()=>setDraft(editDraft(detail))}>管理清单</Button></View>}>{detail.tasks.length?group(detail.tasks,'tasks'):<Text>没有准备事项。可以在编辑旅行中添加。</Text>}</SectionCard>
