...
import { FinanceScreen, MoreScreen, TripsScreen } from './OtherScreens';

const titles:Record<RouteName,string>={home:'首页',calendar:'日程',tasks:'待办',shopping:'采购',trips:'旅行',finance:'家庭资金',more:'更多'};
...
export default function HouseholdApp({screen='home'}:{screen?:string}) {
  ...
  const props:ScreenProps={state,user,focus:household.focus,mode:preferences.homeView,layout:household.layout,setFocus:household.setFocus,setMode:mode=>household.savePreferences({homeView:mode}),onNavigate,onLegacy,pendingId,
    onEdit:(kind,item)=>{
      if(kind==='trips'){onLegacy('trips');return;}
      if(item?.sync){setNotice('同步内容请在原应用修改');return;}
      if(kind==='events'&&(item as any)?.travelTiming){onLegacy('trips');return;}
  ...
  return <><AppShell route={route} title={titles[route]} name={user.name} householdName={state.household?.name||'我们的家'} onNavigate={onNavigate} onCreate={kind=>props.onEdit(kind)} onRefresh={()=>void refresh()} onLogout={()=>handle(household.logout)} refreshing={household.refreshing} offline={!online} themeMode={preferences.theme} onThemeChange={theme=>handle(()=>household.savePreferences({theme}))} onLegacy={onLegacy}>
    {route==='home'?<HomeScreen {...props}/>:route==='calendar'?<CalendarScreen {...props}/>:route==='tasks'||route==='shopping'?<ListScreen key={route} kind={route} {...props}/>:route==='finance'?<FinanceScreen {...props}/>:route==='trips'?<TripsScreen {...props}/>:<MoreScreen {...props}/>}
