...
export default function LoginScreen() {
  ...
  return <View style={[styles.page, { backgroundColor: theme.colors.background }]}>
    ...
    <Card mode="outlined" style={styles.card}><Card.Content style={styles.content}>
      ...
      <TextInput accessibilityLabel="登录密码" label="登录密码" mode="outlined" disabled={busy} value={password} onChangeText={setPassword} secureTextEntry autoComplete="current-password" onSubmitEditing={() => { if (!busy && password) void submit(); }} />
    ...
    <Portal><Dialog visible={spaceOpen} onDismiss={()=>setSpaceOpen(false)} style={{maxWidth:440,width:'90%',alignSelf:'center'}}><Dialog.Title>切换家庭</Dialog.Title><Dialog.Content><TextInput mode="outlined" accessibilityLabel="家庭入口名称" label="家庭入口名称" value={slug} onChangeText={setSlug} autoCapitalize="none"/><HelperText type="info">输入邀请时使用的家庭入口名称。</HelperText></Dialog.Content><Dialog.Actions><Button onPress={()=>setSpaceOpen(false)}>取消</Button><Button disabled={!/^[a-z0-9][a-z0-9-]{2,39}$/.test(slug)} onPress={()=>openLocal('/space/'+encodeURIComponent(slug))}>进入家庭</Button></Dialog.Actions></Dialog></Portal>
  </View>;
}
