...
import { Button, Chip, IconButton, ProgressBar, SegmentedButtons, Text, TouchableRipple, useTheme } from 'react-native-paper';
...
export function RangeControls({ props }: { props: ScreenProps }) {
  ...
  return <View style={styles.controls}>
    <SegmentedButtons value={props.mode} onValueChange={change} density="small" buttons={[
      { value: 'today', label: '今日', disabled: pending }, { value: 'week', label: '本周', disabled: pending },
      { value: 'around', label: '前后 3 天', disabled: pending }]} />
    <View style={styles.people}><Text variant="labelMedium">日程侧重</Text>{props.state.people.map(person =>
      <Chip key={person.id} selected={props.focus === person.id} onPress={() => props.setFocus(person.id)} compact>{person.name}</Chip>)}</View>
...
export function WorkloadStrip({ summary, selected, onSelect }: {
  ...
  const theme = useTheme(), peak = Math.max(60, ...summary.days.map(day => day.busyMinutes));
  return <ScrollView horizontal showsHorizontalScrollIndicator={false} accessibilityLabel="所选范围每日安排">
    <View style={styles.week}>{summary.days.map(day => <TouchableRipple key={day.day} onPress={() => onSelect(day.day)}
      ...
      <View style={styles.dayInner}><Text variant="labelMedium">{weekNames[weekday(day.day)]}</Text><Text variant="bodySmall">{shortDay(day.day)}</Text>
...
export function EventRow({ event, day, props }: { event: CalendarEvent; day: string; props: ScreenProps }) {
  const theme = useTheme(), editable = props.user.role === 'member' && isLocalEvent(event);
  ...
  return <TouchableRipple onPress={() => editable && props.onEdit('events', event)} disabled={!editable}
    ...
    <View style={[styles.event, { borderBottomColor: theme.colors.outlineVariant }]}>
...
export default function CalendarScreen(props: ScreenProps) {
  ...
  return <View style={styles.screen}>
    <PageHeader title="日程" description="安排看得清，生活从容一点。" action={<Button mode="contained" icon="plus" onPress={() => props.onEdit('events')}>添加安排</Button>} />
    <RangeControls props={props} />
    <View style={styles.navigation}><IconButton icon="chevron-left" accessibilityLabel="前一段时间" onPress={() => move(-1)} />
      <Text variant="titleSmall">{shortDay(days[0])}{days.length > 1 ? `—${shortDay(days.at(-1)!)}` : ''}</Text>
      <IconButton icon="chevron-right" accessibilityLabel="后一段时间" onPress={() => move(1)} />
      <Button compact onPress={() => { setAnchor(undefined); setSelected(dayKey()); }}>回到今天</Button></View>
    ...
    <SectionCard title={`${shortDay(chosen)} ${weekNames[weekday(chosen)]} · ${daily.total} 项安排`}>
      {daily.events.length ? daily.events.map(event => <EventRow key={event.id} event={event} day={chosen} props={props} />)
        : <EmptyState title="这天还没有安排" action={<Button onPress={() => props.onEdit('events')}>添加安排</Button>} />}
    </SectionCard>
...
const styles = StyleSheet.create({
  screen: { gap: 16 }, controls: { gap: 10 }, people: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 8 },
  navigation: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' },
  week: { flexDirection: 'row', gap: 8 }, day: { width: 84, borderRadius: 12, overflow: 'hidden' },
  dayInner: { padding: 10, gap: 4, minHeight: 112 }, bar: { height: 3, marginVertical: 2, borderRadius: 4 },
  event: { flexDirection: 'row', gap: 12, paddingVertical: 14, minHeight: 66, alignItems: 'center', borderBottomWidth: StyleSheet.hairlineWidth },
