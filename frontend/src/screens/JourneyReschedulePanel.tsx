...
function Workspace(props: Props & { identityKey: string }) {
  ...
  function exit() { if (!mayExitReschedule(live.current.pending, working.current)) return; if (receipt && current()) { conceal(true); props.onSaved({ journeyId: props.journeyId, revision: receipt.revision }); } else { conceal(true); props.onBack(); } }
  function back() { if (!mayExitReschedule(live.current.pending, working.current)) return; if (receipt) { exit(); return; } if (draft && source && JSON.stringify(draft) !== JSON.stringify(initialRescheduleDraft(source))) setLeaving(true); else exit(); }
  ...
  const leavingDialog = <Portal><Dialog visible={leaving && !pending} dismissable={!busy} onDismiss={() => setLeaving(false)}><Dialog.Title>离开改期页面？</Dialog.Title><Dialog.Content><Text>未保存的日期和选择会关闭。</Text></Dialog.Content><Dialog.Actions style={styles.actions}><Button onPress={() => setLeaving(false)}>继续核对</Button><Button disabled={busy || !!pending} onPress={exit}>离开并返回旅行</Button></Dialog.Actions></Dialog></Portal>;
  if (!visible) return <View style={styles.page}><ActivityIndicator animating={busy} /><Text>{error || '正在核对旅行与登录身份…'}</Text>{pending && <Text>原保存结果尚未核实，先恢复身份核对；操作编号仍保留。</Text>}<Button disabled={busy || !!pending} onPress={back}>返回旅行</Button><Button disabled={busy || !household.online} onPress={enter}>重新核对改期身份</Button>{leavingDialog}</View>;
  return <View style={styles.page} testID="journey-reschedule-panel">
    ...
    <Button accessibilityLabel="返回旅行" disabled={busy || !!pending} onPress={back}>返回旅行</Button>
