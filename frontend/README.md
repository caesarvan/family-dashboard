...
- 已发布 `/app/connections`：账户授权、安全回调、日历／清单搜索分页、完整共享范围预览、来源版本冲突核对、逐来源同步状态、失效同步单独停止及断开确认。使用真实账户 API；照片专用授权与日历／清单授权能力分开显示。详情见 [Expo 账户与同步](../docs/EXPO-ACCOUNTS.md)。

- 原生 React Native 组件：首页、今日／本周／前后 3 天日程、待办、采购、登录和导航。待办支持新增与勾选；采购支持预算、数量、参考图片。
- 相册支持真实图库、详情编辑、家庭共享、逐电视许可和 Google 选片/确认；旅行支持基本计划、准备/采购、预览和确认；助理支持真实请求预览、选择性保存和执行回执。`/app/finance` 已提供共同资金、本人账本、导入、来源追溯、订单／付款／退款核对与月预算，操作见 [Expo 财务](../docs/EXPO-FINANCE.md)，实际发布身份以根 README 为准。资产／投资／来源报告等高级财务、复杂旅行分段／资料／云发布、照片旅行推荐及家庭／设备／布局管理可进入明确标注的 `/classic` 功能。
- 已发布 `/app/map`：地点管理、筛选、同源世界地图和关联旅行／只读相册往返。地图和子页面使用同一 URL，仅在当前会话内保留返回条件，返回后重新读取权限；每页 24 个地点，不将旧照片或坐标放入 URL。接口、隐私及版本状态见 [Expo 地图](../docs/EXPO-MAP.md)。
- 已发布 `/app/inventory`：物品、采购批次、分批收货、实物变化、历史纠正、共享和归档；采购页可先查看家中物品，助理本地搜索结果可打开当前库存详情。具体操作见 [Expo 家庭物品](../docs/EXPO-INVENTORY.md)、[库存搜索](../docs/ASSISTANT-INVENTORY-SEARCH.md)，构建、真实浏览器与发布结果见根 README 和验证记录。
...
- `src/screens/`：各业务页面。
- `src/screens/AccountsScreen.tsx` 与 `src/lib/accounts.ts`：本人连接、完整来源替换、停止／断开及未知结果恢复。
- `src/lib/authNavigation.ts`：固定授权回调结果与服务商授权地址校验，不把 URL 标记作为账户已连接证据。
...
每位 agent 使用独立 `codex/` 分支与 worktree，经非作者审查、组合验证后合并。构建锁文件、源提交、导出文件 SHA-256 与发布 manifest 必须对应；不能把未经验证的 `dist` 覆盖到线上。运行数据、凭据、截图和测试产物不入库。完整部署入口与当前安装身份以根 README、`docs/HANDOFF.md` 和发布记录为准。发布后补充的本地文档不改变已安装 manifest 或当时的构建来源；下一次构建须按新的完整输入重新绑定证据。
