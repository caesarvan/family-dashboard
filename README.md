...
**最新上线：本地待办前置事项（2026-09-20 06:28:25，北京时间），06:35:47 独立只读审计通过。** 在「共同待办 → 添加／编辑 → 前置事项」选择本地任务，首页与清单显示阻塞原因；前置未完成时不能完成后续任务。任务依赖、原 ID 与版本保护已接到旅行、例行事项和明确云冲突处理。[使用与集中验收](docs/TASK-DEPENDENCIES-ACCEPTANCE.md#task-dependencies-release)。本地浏览器按 R1 前两项＋R4 第三项分段验证，实际 Linux 354 项通过；一户两库保持 69/9。站内提醒仍为独立候选，A–D 本人完整验收 0/4、真实云写入与实体电视另验。
