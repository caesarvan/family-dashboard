...
**最新上线：旅行分段编辑，2026-09-17 21:12:18（北京时间）激活，21:12:47 正常 TLS 读回通过。** 从旅行详情进入「行程分段」，编辑城市、航班、住宿和活动，核对预览后保存；支持并发核对及原操作结果恢复。本地 15 组流程／19 图、服务器 260 项通过及唯一精确 Windows 跳过分别核验，55→55 原数据与配置保持。使用见 [行程分段](docs/EXPO-JOURNEY-SEGMENTS.md)，运行身份及独立读回见 [发布验收](docs/EXPO-JOURNEY-SEGMENTS-ACCEPTANCE.md#expo-segments-release)。个人资产报告仍为开发候选；实体电视仍待验收。
