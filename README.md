...
**最新发布：JPEG 主图兼容。** 完整 JPEG 主图后的附加内容和不使用的 metadata 会在解码前丢弃，保留合法方向并重新编码展示副本；大小、像素和完整解码限制不变，详见 [兼容说明](docs/MEDIA-JPEG-COMPAT.md)。本人已授权 Google Photos，历史十选三的其余七项原因未记录；后来重试十选五，另外五项 `invalid_image` 已准确显示，但具体解码原因仍未确定，本次兼容补丁是否解决它们仍待本人重选。逐项结果、准确勾选保存数与历史未知提示继续保留，使用见 [媒体交付](docs/MEDIA-DELIVERY.md)。副本默认私密，旅行关联、共享和逐台电视许可需明确操作；视频、iCloud／NAS、自动回忆精选尚未实现，库存不属于本次 48 表生产版本。
...
NVIDIA 已于 16:58 通过独立配置步骤启用，模型为 `us/azure/openai/gpt-4o-mini`。服务器使用真实运行配置完成一次虚构家庭的模型待办草稿请求，未执行草稿或读取真实家庭事项；本次媒体发布保留这些配置。本人相册已授权，历史十选三、后续十选五及 JPEG 补丁的待验收边界见顶部；真实新云写入和实体设备仍单独验收。
...
| 家庭相册 | Google Photos 本人选片、后台净化加密、明确保存为私密副本、旅行关联、家庭共享、逐台电视许可及照片元数据导出 | 已发布；本人已授权，后续十选五的诊断已验证；JPEG 补丁本人重选与实体电视待验收；仅照片，不支持视频、iCloud／NAS或自动回忆精选 |
