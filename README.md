...
> **线上版本：2026-09-17 07:38:17（北京时间），07:39:46 正常 TLS 读回通过。** 在「助理」输入旅行需求并点「整理并预览」，或打开「规划一次旅行」，核对简报后编辑准备事项、负责人和采购预算，最后预览并明确保存。界面沿用 [Expo 官网](https://expo.dev/) 的白底、黑色胶囊按钮和浅灰圆角卡片。使用见 [旅行简报](docs/EXPO-JOURNEY-BRIEF.md)，完整发布身份、测试及失败记录见 [旅行协作验收](docs/EXPO-TRAVEL-ACCEPTANCE.md#expo-travel-release)。

本轮 55→55 不迁移；实际 1 户两库完整备份，原 55 表及注册库的行、schema、序列在新 app 启动核对时保持。新镜像 Linux 329 项通过、1 个精确 Windows junction 测试跳过，零失败／错误；本地真实浏览器 7 场景及四宽 12 张截图另行记录。读回核对 516 源／113 运行文件和 75 项 HTTPS 静态资源，四服务运行、app 健康；新一次逐库在线备份成功，不能等同跨库全局原子快照。本人新流程、真实模型质量、新云操作及实体电视仍待验收；此前 Photos 5／5 成功事实保留。发布后的文档不改变已安装运行包。
...
**旅行从一次输入开始。** 助理将本次文字带入单页简报，可选 AI 需明确勾选；未知预算不当作零，准备与采购仍由成员核对后保存。响应丢失时沿用原预览和操作标识安全重试，不直接重新创建。地图、资料与云日历的完整场景仍须另验，见 [旅行协作验收](docs/EXPO-TRAVEL-ACCEPTANCE.md)。
