...
**最新发布：相册逐项处理结果。** 本人 Google Photos 已授权，曾选择 10 张普通照片并保存 3 张；旧记录未保留另外 7 项的处理原因，不能反推格式、损坏或未勾选。新选择会显示逐项成功／失败／跳过与固定安全原因，确认后保留准确保存数；旧记录明确显示未知，不补成零或全部成功。使用见 [媒体交付说明](docs/MEDIA-DELIVERY.md) 与 [结果说明](docs/MEDIA-IMPORT-RESULTS.md)。相册副本默认私密，旅行关联、家庭共享和逐台电视许可仍需本人明确操作。视频、iCloud／NAS 和自动回忆精选尚未实现；库存仍属独立候选，未包含在本次 48 表生产版本中。
...
NVIDIA 已于 16:58 通过独立配置步骤启用，模型为 `us/azure/openai/gpt-4o-mini`。服务器使用真实运行配置完成一次虚构家庭的模型待办草稿请求，未执行草稿或读取真实家庭事项；本次媒体发布保留这些配置。本人相册已授权，历史十选三结果与新诊断的待验收边界见顶部；真实新云写入和实体设备仍单独验收。
...
| 家庭相册 | Google Photos 本人选片、后台净化加密、明确保存为私密副本、旅行关联、家庭共享、逐台电视许可及照片元数据导出 | 已发布；本人已授权且历史十选三，其余原因未知；新诊断本人验证与实体电视待验收；仅照片，不支持视频、iCloud／NAS或自动回忆精选 |
