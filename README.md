...
> **线上版本：2026-09-15 20:23:00（北京时间），镜像 `sha256:651ecfd6bdb65cf04bb8778c8657a8ec0f27a123940683a44a8b9931a5f22352`。** 旅行资料已完成 42→43 迁移并发布；当前源码 237 文件、40 份 Markdown（含 README 与 AGENTS），106 个方法／路径模板、43 张户内表及 2 张平台表。运行代码已合入 main `670f34f`；本次文档整理随后独立审查与交付。详细证据和实际接入边界见 [HANDOFF](docs/HANDOFF.md) 与 [VALIDATION](docs/VALIDATION.md)。
...
当前生产版本于 **2026-09-15 20:23:00（北京时间）** 发布至 racknerd，镜像为 `sha256:651ecfd6bdb65cf04bb8778c8657a8ec0f27a123940683a44a8b9931a5f22352`。本次新增旅行资料库，原 42 张户内表的结构、行和 SQLite 序号在停写迁移及启动核对窗口内保持不变，只新增空资料表及精确索引／触发器。历史版本及首次失败经过保留在 [VALIDATION](docs/VALIDATION.md)。

| 项目 | 已核验结果 |
|---|---|
| 技术栈 | Flask／原生 JavaScript／SQLite；Docker Compose 运行 app、sync、web |
| 当前源码 | 237 文件、40 份 Markdown（含 README 与 AGENTS）；源码部署清单与后续纯文档交接清单分别记录 |
| Git 基线 | 已审 integration `b1981a4` → main `670f34f`；每任务独立分支/worktree；没有远端或托管平台强制保护 |
| 接口与存储 | 106 个 Flask 方法／路径模板，另有 WSGI 家庭入口；43 张户内表（41 业务 + 2 认证）、平台 2 表 |
| Windows 后端 | 1495 项：1477 passed / 18 skipped；完整首轮与精准重试、环境修订专项按用例去重，不是单次全绿运行 |
| Linux 后端 | 1495 项：1494 passed / 1 skipped；原完整 1461 项剔除旧控制器 66 项、加入当前同镜像实测 100 项 |
| 浏览器 | 资料 43、原细项 10、旅行执行 53、返回 23，共 129 项；实际依赖字节一致，临时 Flask/SQLite、真实 Edge、虚构输入、零外站请求 |
| 内部读回 | 20:23:44：237 源文件与 44 静态资源散列匹配，8 匿名接口均 401；三服务运行、app 健康；实际 1 户 43 表、平台 2 表、quick_check 正常 |
| 来源读取 | 6 个既有来源均有发布后的自然成功、0 错误；成功时间为 20:23:01–20:23:35，不证明新增云写或全部数据完整 |
| 43 表完整组恢复 | 两户合成 Docker 恢复：控制器 65、seed 15、verify 216，含 8 份资料；22 容器、6 卷清理；依赖未变的原证明复用 |
| 42→43 配置与迁移演练 | 独立的单户 + 注册库合成 Docker 演练，6 个 helper、9 个资源，配置特殊字符、旧令牌解密和迁移通过；不与上一行相加 |
| 仍待真实验收 | 新云日历／待办写入、银行或电商自动连接、可选模型、实体电视长期运行、生产覆盖恢复和商业化运营 |

公网浏览器曾受组织 SmartScreen 阻断，未绕过；本次读回仅在 app 容器 loopback 使用 GET 和只读 SQLite，不能替代公网或实体设备验收。
...
交接历史：16:25 原发布包为 218 文件／35 文档，16:54 恢复工具交接为 222／36，之后 Git 规则交接为 225／38；这些历史文档与工具批次当时没有改变运行镜像。当前旅行资料是新的代码与 schema 发布，不能将旧包统计当作当前数量。

## 下一轮候选：旅行新建入口

当前生产状态仍以页首及发布记录为准。新一轮候选将首页快速记录、旅行卡与旅行页的新建入口统一到“填写 → 预览准备／采购／日程 → 确认”的原旅行向导，保留已有草稿及旧旅行编辑。行为与限定验证见 [新建旅行入口](docs/TRAVEL-ENTRY.md)。

本轮仅改变前端运行文件；发布使用[静态更新专用流程](docs/STATIC-RELEASE.md)，继承已验证的不可变镜像，核对全部家庭的 43 表和平台数据保持。构建、Git 审查、隔离演练和生产发布分别记录，不把候选测试当作上线完成。

## 本次发布增量

| 流程 | 已发布实现 | 范围与验收 |
|---|---|---|
| 旅行资料 | 本人资料库／旅行资料夹，默认私有、明确共享、上传重试、元数据 CAS、逐份下载与删除 | 伙伴只读有效共享资料，TV 禁止；删除旅行保留上传者文件；[资料契约](docs/JOURNEY-DOCUMENTS.md) |
| 完整细项与定位 | 航班／住宿／活动字段展示和复制，从日程定位原 segmentKey | 旅行原准备、采购、进度、发布和编辑返回保持；[旅行执行](docs/JOURNEY-EXECUTION.md) |
| 部署与恢复 | 一次性 42→43，配置解析与实际 app 比对，输入 SHA 固定、全组备份和分步启动 | 失败停服保全，不自动恢复；真实合成恢复与生产迁移分别记录 |

此前旅行执行、待同步状态、订单表头兼容及财务各模块保持。真实消费观察文件仍未自动上传或确认，见 [消费观察契约](docs/SPENDING-OBSERVATIONS.md)。
...
平台注册目录为 `/data/platform.sqlite3`；原家庭为 `/data/household.sqlite3`；子家庭为 `/data/spaces/<随机 id>/household.sqlite3`。当前为 **43 张户内表（41 业务 + 2 认证）和 2 张平台表**。本次只新增 `journey_documents` 及其索引／触发器；15:10 历史消费观察两表和其他原表保持原权限与数据。完整结构见 [存储索引](docs/PLATFORM-ROUTES.md) 与 [数据模型](docs/DATA-MODEL.md)。
...
当前为 **106 个 Flask 方法／路径模板**，包含五个成员专用旅行资料接口；另有 WSGI `GET /space/<slug>`，HEAD/OPTIONS 不重复计数。以 [当前路由索引](docs/PLATFORM-ROUTES.md)、[机器结构](docs/contract-inventory.json)和实际源码为准；字段与权限仍见各模块契约。
...
安装、DNS、HTTPS 证书与续期、OAuth 回调、备份及恢复见 [部署指导](docs/DEPLOYMENT.md)。已有环境升级需按实际版本选择已审查流程；当前 43 表后续零结构更新程序尚未提供。两条安装路径要分开：

- **首次安装**：配置新服务器与独立密钥，初始化新数据卷，签发证书，启动三个服务，再绑定第三方账户。
- **已有环境更新**：冻结源码并打包，在独立目录构建和测试候选；核对现有镜像、配置与完整备份，停止写入后逐户初始化并按所选版本已审查的迁移流程核对表结构与数据；当前 43 表不能使用历史 42→42 示例，再启动与读回。
...
历史版本分别完成 37→40、40→42；本次已按 [旅行资料发布指导](docs/JOURNEY-DOCUMENTS-RELEASE.md) 完成一次性 42→43。控制器不能再次用于已迁移库。DEPLOYMENT 第 5.2 节的 42→42 算法仅保留历史，后续 43→43 无结构更新指导尚需独立实现和验证，不能只改表数使用。
...
交接包中的 `RELEASE-MANIFEST.json` 记录逐文件 SHA-256，Git bundle 保留提交和分支。新的开发任务从集成人确认的当前提交建立独立 worktree。服务器本次已安装 237 文件部署清单；原 222 文件基线和旧 218 文件 manifest 已原字节保全，属于迁移前证据。后续纯 Markdown 交接与部署清单分别记录，不能借文档变化改写原测试证据。
...
| [路由与存储索引](docs/PLATFORM-ROUTES.md) | 当前 106 个方法/路径模板、43 张户内表及 2 张平台表，字段见各模块专文 |
