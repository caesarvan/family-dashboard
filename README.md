...
> **线上版本：2026-09-17 00:40:16（北京时间）。** 家庭物品已迁入 React Native／Expo 新版，可登记物品、采购批次、分批收货、实物变动、共享和归档；采购页可先“查看家中物品”，助理本地搜索可查看现有／在途／计划数量并打开当前详情。延续 [Expo 官网风格](docs/EXPO-SITE-STYLE.md) 和横向／底部导航，具体操作与固定身份见 [家庭物品交付](docs/INVENTORY-DELIVERY.md)。

> 此次为 53→53 无迁移发布。实际 1 户两库完整备份，00:40:31 正常 TLS 读回核对 453 源／112 运行／75 公开静态文件，四服务和备份正常，原配置保持。真实临时浏览器 14 项和隔离 Linux 86 项分别通过，边界与失败修订见 [验证记录](docs/VALIDATION.md)。本人真实库存、云操作和实体电视未在本轮重验，未交付原生安装包；此前本人 Photos 5／5 保存成功记录保留。本文为发布后本地交接，不改写已安装 manifest。
...
JPEG 补丁后本人已实际选择 5 张、全部出现预览并明确保存，服务器读回为 5 成功、0 失败。本轮未重新执行真实 Google 或实体电视验收；库存本地搜索已按文首发布，真实用户库存体验仍待验收。旧十选三其余七项未知、十选五的五项 `invalid_image` 与原失败根因未确定继续保留，见 [媒体交付](docs/MEDIA-DELIVERY.md)。实体电视和长期云行为仍另验。
