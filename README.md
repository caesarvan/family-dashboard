...
真实原文件仅完成本地临时预览，没有确认写入私人账本。业务已审主分支为 `c8d4188`；新增的 [43→43 源码发布工具](docs/SOURCE-RELEASE.md) 经非作者审查后合入集成版本 `e0769e3`，当前候选为 255 源文件、43 份 Markdown。Windows 工具组合 269 项通过；新镜像 Linux 测试、完整浏览器组合与双户 Docker 演练仍须完成，尚未上线。不能用 static-only 入口承接 Python 修改，也不能重放旧的 42→43 迁移。
