...
**本地候选，尚未部署：成员外观设置。** 新版「更多 → 外观设置」可为本人选择浅色／深色、标准／紧凑密度，明确保存后应用；伴侣和每台电视保持各自设置。操作与失败恢复见 [成员外观](docs/EXPO-APPEARANCE.md)。接口、类型、最终 40 项 Node 及 R1 真实临时浏览器 11 条流程通过，生成 17 张当前视口截图；Linux 227 项仅已收集，尚未执行或部署，线上仍为下方 14:56 首页布局版。

**线上版本：Expo 首页布局于 2026-09-17 14:56:51（北京时间）激活，14:57:27 正常 TLS 读回通过。** 从「首页 → 安排首页」调整本人卡片顺序、显示或恢复默认，明确保存后供本人手机与电脑使用；伴侣、电视设置与数据权限保持。R2 本地 9 条浏览器流程／9 图、Linux 188 通过及唯一精确 Windows 跳过分别核验。操作见 [首页布局](docs/EXPO-HOME-LAYOUT.md)，完整运行身份、55→55 保全与验证边界集中在 [本轮验收](docs/EXPO-HOME-LAYOUT-ACCEPTANCE.md#expo-home-layout-release)。
...
资产基线和来源报告等高级财务、家庭／成员、照片旅行推荐及复杂旅行分段／资料继续保留经典入口；设备、布局管理与电视展示页均已迁入 Expo，经典电视渲染器保留为明确回退入口。财务账本、本人持仓、地图、家庭物品及旅行日历同步主流程已迁入 Expo；电视继续独立只读 `/tv`。路线、精选回顾、视频、iCloud／NAS、通用成员角色和完整跨模块场景仍在 [产品计划](docs/PRODUCT-PLAN.md) 中逐项跟踪。
...
| 主题与偏好 | Expo 使用统一白黑设计；classic 与电视保留三主题、密度和独立布局偏好，日程默认视图仍保存 | Expo 首页「安排首页」已支持排序／隐藏／恢复默认并明确保存；成员 Expo 主题／密度、拖动与自定义背景未实现，隐藏不改变 API 可见范围 |
...
| [Expo 成员外观](docs/EXPO-APPEARANCE.md) | 本地候选：本人浅色／深色、内容密度、明确保存、冲突及未知结果核对；电视设置独立 |
