...
**最新上线：助理资料与照片搜索（2026-09-20 05:15:29，北京时间），05:16:46 独立只读审计通过。** 从「更多 → 家庭助理 → 找资料照片」查找已保存资料和照片，重新核权后打开原内容，返回保留关键词和页码；包含采购保存错误提示的完整换行修复。[使用与集中验收](docs/ASSISTANT-DOCUMENT-SEARCH-ACCEPTANCE.md#assistant-document-search-release)。R1 四条搜索与一条采购提示、R2 照片单项及实际 Linux 175 项分别验证；一户两库保持 69/9。任务依赖仍是另一候选，站内提醒尚未实现；A–D 本人完整验收 0/4、真实云和实体电视另验。

**此前上线：采购截止日与优先级（2026-09-20 03:54:39，北京时间），03:55:48 独立只读审计通过。** 「采购」可填写截止和低／普通／高优先级；旅行草案携带这些字段，旅行改期只移动本人明确选中的采购。后端、正式新构建、4 条真实临时浏览器流程、一次真实模型及 Linux 298 项分别通过。[最短使用路径与集中验收](docs/SHOPPING-SCHEDULE-ACCEPTANCE.md#shopping-schedule-release)。该采购批次当时的 390px 离线提示裁切已随本次资料搜索发布修复；旧验收事实保留，真实云／实体电视及 A–D 本人完整验收（0/4）另验。
