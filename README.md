...
**新一轮本地候选：相册与电视照片轮播。** 已接入 Google Photos 本人选择授权、后台净化及加密展示副本、明确保存、私有精选、旅行关联、家庭共享、逐台电视许可、手机播放控制和照片信息导出。正式应用工厂当前为48张户内表、平台两表；线上仍以上方44表版本为准。完整模块、操作和部署边界见 [媒体交付说明](docs/MEDIA-DELIVERY.md)。库存核心已审，库存界面/API及完整成员角色仍在后续开发。
...
进入「旅行 → 地图」，可以保存心愿、已计划和本人确认已到访的地点，按年份、成员及旅行筛选，并打开关联旅行。地点默认仅本人可见；明确共享时可隐藏或粗化坐标。世界概览底图与应用同源，无需外部地图账号。电视暂不显示地点详情，精选相册与回顾仍在开发。使用见 [地图界面](docs/JOURNEY-MAP-UI.md)、[地点接口](docs/JOURNEY-PLACES.md)。
...
部署使用 [地点迁移流程](docs/JOURNEY-PLACES-RELEASE.md)；已经迁到 44 表后不能再次运行 43→44，也不能使用旧的 43→43 SOURCE 入口。后续发布必须从实际 44 表基线重新制定。恢复演练显式选择 `journey_places44`，见 [恢复指导](docs/RECOVERY-REHEARSAL.md)。

相册首个来源继续采用 Google Photos 本人选择授权；电视先按浏览器使用，型号以后补充。私有精选、旅行关联、逐台电视许可、库存及完整成员角色仍未交付。NVIDIA 已通过独立配置步骤启用，当前模型为 `us/azure/openai/gpt-4o-mini`。服务器使用真实运行配置完成一次虚构家庭的模型待办草稿请求，未执行草稿或读取真实家庭事项。真实新云写入、相册授权和实体设备均单独验收。
...
| 旅行地图 | 心愿/已计划/确认到访地点，手动坐标、年份/成员/旅行筛选、隐私共享投影及旅行关联 | 世界概览；无街道导航/外部地理编码，TV不读；照片与路线回顾待实现 |
