...
本轮以 `daf5ab4` 为共同基线，继续采用每任务独立 Git 分支与 worktree。新增两项改进：

- **待办起步：** 空清单可添加首项本地待办，或先设置常用清单再返回；云端发布仍需选择、预览和确认。接口和身份保护见 [连接返回](docs/CONNECTION-RETURN.md)。
- **订单文件兼容：** 修正只含未使用二进制类型声明的纯值 XLSX 被误判为宏文件的问题。实际宏、二进制部件、外链及公式仍被拒绝。此改进不代表所有淘宝文件均已支持；多商品订单的合并单元格续行尚未适配，仍阻止确认导入，详见 [导入契约](docs/FINANCE-IMPORT.md)。

两项功能在非作者审查后合入集成版本 `83a1a71`，源码为 249 文件，新增一份起步流程测试。候选修改包含 `financial_files.py`，不能使用只允许静态文件变化的发布工具。合并、组合测试与生产发布分别记录在 [交接说明](docs/HANDOFF.md) 和 [验证记录](docs/VALIDATION.md)；下方版本表描述已有线上版本。
