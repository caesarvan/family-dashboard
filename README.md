...
**候选未上线：电视单趟旅行回顾。** 从保存的旅行进入回顾，选择一台已配对电视与可选共享路线，核对获准内容后明确开始；没有媒体但有可展示路线也可回顾。每台电视独立授权，撤权或旅行失效不退回全相册。[候选使用说明](docs/TV-TRIP-USER-GUIDE.md)与[接口合同](docs/TV-TRIP-PLAYBACK.md)说明恢复及隐私边界；本地77/9不改变上述生产75/9。两条真实本地浏览器流程已分轮通过；后续TV元数据读取优化另有24项Flask／SQLite验证，Linux容量与迁移仍待执行，见[候选验收](docs/TV-TRIP-ACCEPTANCE.md#tv-trip-candidate)。
...
| 调整并保存助理清单 | 家庭助理 → 描述待办／采购 → 整理并预览 → 调整 → 勾选并确认保存；未知结果先核对原计划 |
...
| 家庭助理 | 本地概览、已有旅行／资料／照片／库存搜索；旅行草案与改期、预算和支出查询；待办／采购逐项调整及原结果恢复 | 写入前预览确认；可选模型与本地规则分开，不自动向云端发布 |
...
| 打包／发布 | [deploy](deploy) | [当前五服务发布合同](docs/ASSISTANT-LIST-RELEASE.md)、[完整验收索引](docs/VALIDATION.md) |
