...
> **线上版本：2026-09-16 11:39:20（北京时间），镜像 `sha256:f82bc0fc147038ca3cdba546756192259a6f28c68c21621f75494ab9dc262ee6`。** 待办起步、XLSX 声明兼容、淘宝多商品订单与明细展示已通过 SOURCE 43→43 更新发布；255 源文件、43 份 Markdown（含 README 与 AGENTS）、44 静态资源，106 个方法／路径模板、43 张户内表及 2 张平台表。发布源码为同树 integration `ecc3edc`／main `54c0854`；后续纯文档修订不代表已同步服务器。详见 [HANDOFF](docs/HANDOFF.md) 与 [VALIDATION](docs/VALIDATION.md)。
...
本轮从已审本地 `fa72df4` 建立独立后端、前端和交接分支；生产仍为上文的 `f82bc0fc…`。候选增加本人账本分页和标题／编号／商品搜索，使同月超过 500 条的旧记录也能打开核对。进入「财务 → 账单与订单」，选择月份后搜索、翻页；核对或删除后保留查询和合理页码。页面筛选只影响列表，月度预算与汇总仍按全月账本计算；账本在翻页期间变化时要求刷新，避免混用两次结果。

文件导入增加金额格式检查：完整的 `1,234.56` 可解析，`1,23` 等不完整分组报行错误，不能确认该批次。不会猜测原金额、修订历史记录或改变手工金额接口。请求参数与边界见 [财务 API](docs/FINANCE-API.md) 和 [导入格式](docs/FINANCE-IMPORT.md)，本轮审查、验收及部署状态单列在 [交接记录](docs/HANDOFF.md) 与 [验证记录](docs/VALIDATION.md)。

固定运行组合 `98fb739` 已完成本地 Windows 受影响后端 **435 项**及分组浏览器 **215 项**验证。候选为 258 源文件、43 Markdown、44 静态资源、107 方法／路径模板，数据库仍为每户 43 表及平台 2 表。新 Linux 镜像、升级演练和生产发布尚未执行；上述数字不能替代部署验收。
...
当前生产版本于 **2026-09-16 11:39:20（北京时间）** 发布，镜像为 `sha256:f82bc0fc147038ca3cdba546756192259a6f28c68c21621f75494ab9dc262ee6`，固定继承原 `14c7db7b…` 父镜像并追加完整 Python/static 层。requirements、Dockerfile、Compose、Nginx、原 `.env` 与完整解析配置保持，未执行 schema 初始化或数据库恢复。

| 项目 | 已核验结果 |
|---|---|
| 技术栈 | Flask／原生 JavaScript／SQLite；Docker Compose 运行 app、sync、web |
| 发布源码 | 255 文件、43 份 Markdown（含 README 与 AGENTS），manifest `169c9621…`；本次后续文档修订尚未同步服务器 |
| Git 基线 | 发布源码 integration `ecc3edc` → main `54c0854`，相同 tree `08c1377…`；各任务独立分支/worktree，无托管平台强制保护 |
| 接口与存储 | 106 个 Flask 方法／路径模板，另有 WSGI 家庭入口；43 张户内表（41 业务 + 2 认证）、平台 2 表 |
| 受影响后端 | Windows 406 passed；新镜像 Linux 406 passed；各 0 skipped / failed / error。旧 1495 项仅作历史，不与本次相加 |
| 新部署工具 | 新镜像 Linux 269 passed，0 skipped / failed / error；原失败与测试支持目录修订后重试分别保留 |
| 受影响浏览器 | 187 项实际补测与 65 项未变依赖历史结果，共 252 项功能，59 项路由另列；真实临时 Flask/SQLite/Edge、虚构输入、零外站 |
...
| 内部读回 | 44 静态资源 200 且 SHA 匹配、8 匿名接口均 401；新 app/sync/web 三服务运行，app 健康，字节码排除检查通过；发布后另读回全部 255 源文件、manifest、原配置和镜像 |
| 仍待真实验收 | 新云日历／待办写入、银行或电商自动连接、可选模型、用户端公网浏览器与实体设备、生产覆盖恢复和商业化运营 |

服务器另经正常 TLS 验证完成域名 HTTPS 读回：healthz 200、匿名 state 401、task-publish.js 200 且 SHA 匹配。此结果仅来自服务器 HTTPS 客户端；用户端浏览器曾受 SmartScreen 阻断，未绕过，不代表手机或实体电视已验收。完整数据相等只覆盖停写至 HTTP 核对窗口，恢复 sync/web 后允许正常业务变化。
...
交接历史：16:25 原发布包为 218 文件／35 文档，16:54 恢复工具为 222／36，Git 规则为 225／38；20:23 旅行资料为 237／40，22:32 静态发布为 248／42。本次 SOURCE 发布为 255／43，旧迁移与恢复证据各自保留。
