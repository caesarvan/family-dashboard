...
> **线上版本：2026-09-15 22:32:04（北京时间），镜像 `sha256:14c7db7ba55cb4bbbed919b0b14d4bb1b1670eeb07d9a6f90e05799220590dfb`。** 新建旅行统一向导已通过严格限定的静态更新发布；248 源文件、42 份 Markdown（含 README 与 AGENTS）、44 静态资源，106 个方法／路径模板、43 张户内表及 2 张平台表。已审 integration `fec0e55` 合入 main `cd75b0b`；后续纯文档交接与运行发布分别记录。详见 [HANDOFF](docs/HANDOFF.md) 与 [VALIDATION](docs/VALIDATION.md)。
...
以已审主分支 `9239754` 为共同基线，淘宝多商品订单与明细展示已通过模块审查和组合验证，集成候选为 `481c907`（251 源文件）；开发、审查、合并和上线分别记录在 [HANDOFF](docs/HANDOFF.md) 和 [VALIDATION](docs/VALIDATION.md)。

- **待办起步：** 已审候选支持从空清单添加首项本地待办，或设置常用清单后返回；云端发布仍需选择、预览和确认。
...
真实原文件仅完成本地临时预览，没有确认写入私人账本。业务已审主分支为 `c8d4188`；新增的 [43→43 源码发布工具](docs/SOURCE-RELEASE.md) 经非作者审查后合入集成版本 `e0769e3`，当前候选为 255 源文件、43 份 Markdown。Windows 工具组合 269 项通过；浏览器补测 187 项功能与 59 项路由通过，另 65 项按未变依赖分段保留。新镜像 Linux 测试与双户 Docker 演练仍须完成，尚未上线。不能用 static-only 入口承接 Python 修改，也不能重放旧的 42→43 迁移。

## 版本与交接状态

当前生产版本于 **2026-09-15 22:32:04（北京时间）** 发布，镜像为 `sha256:14c7db7ba55cb4bbbed919b0b14d4bb1b1670eeb07d9a6f90e05799220590dfb`，继承 20:23 的不可变父镜像，只追加完整静态层。旅行新建入口统一到原向导；根 Python、依赖、部署配置及全部家庭数据保持，未执行 schema 初始化。

| 项目 | 已核验结果 |
|---|---|
| 技术栈 | Flask／原生 JavaScript／SQLite；Docker Compose 运行 app、sync、web |
| 当前源码 | 248 文件、42 份 Markdown（含 README 与 AGENTS）；部署清单与后续纯文档交接清单分别记录 |
| Git 基线 | 已审 integration `fec0e55` → main `cd75b0b`，相同 tree；各任务独立分支/worktree，无托管平台强制保护 |
| 接口与存储 | 106 个 Flask 方法／路径模板，另有 WSGI 家庭入口；43 张户内表（41 业务 + 2 认证）、平台 2 表 |
| 继承的后端证据 | Windows 1495：1477 passed / 18 skipped；Linux 1495：1494 passed / 1 skipped。按未变依赖复用原分段、精准重试证据，本轮未重跑完整 1495 项 |
| 新部署工具 | 最终 Linux 镜像 187 项通过：构建 23、控制器与数据库 112、演练安全 52；与旧后端集合分列 |
| 受影响浏览器 | 新入口 27、原草稿 32、原确认 5，共 64 项；另有原旅行核心流程通过记录。真实临时 Flask/SQLite/Edge、虚构输入、零外站；旧资料版 129 项仅保留历史 |
| 实际静态更新演练 | 两户合成 Docker：43 表、旧会话、8 份资料下载和 3 库完整备份组保持；不执行恢复，13 个隔离资源清理通过 |
| 生产数据与配置 | 实际 1 户 43 表 + 2 平台表、2 库完整组备份；安装前、启动后和 HTTP 后三次完整比较通过，原 `.env` 保持 |
| 内部读回 | 44 静态资源 SHA 匹配、8 匿名接口均 401，三服务正常且 app 健康；22:32:15 追加核对 248 源文件、配置及服务 |
| 仍待真实验收 | 新云日历／待办写入、银行或电商自动连接、可选模型、公网与实体设备、生产覆盖恢复和商业化运营 |
...
交接历史：16:25 原发布包为 218 文件／35 文档，16:54 恢复工具为 222／36，Git 规则为 225／38；20:23 旅行资料发布及后续纯文档交接为 237／40。旧包数量不代表当前 248／42，旧迁移与恢复证据各自保留。

## 本次发布增量

首页快速记录、旅行卡和旅行页的新建入口现在统一进入“填写 → 预览准备／采购／日程 → 确认”的原旅行向导；已有草稿保留，旧旅行仍能编辑。确认前不保存，也不自动订票、付款或写入云端。使用与公共接口见 [新建旅行入口](docs/TRAVEL-ENTRY.md)。

本轮运行差异只有三个前端文件；[静态更新专用流程](docs/STATIC-RELEASE.md) 固定父镜像、完整配置、所有家庭与备份，完成真实隔离演练后发布。首轮合成静态 URL 404 与生产源码预检拒绝分别保留，修复后受控重试；详见 [验收记录](docs/VALIDATION.md)。
...
历史版本分别完成 37→40、40→42 和一次性 [42→43](docs/JOURNEY-DOCUMENTS-RELEASE.md)。本轮已使用 [严格限定的静态 43→43 流程](docs/STATIC-RELEASE.md)，不执行 warm 或 DDL；后端、依赖或配置变化仍需另行设计、审查与验收。DEPLOYMENT 第 5.2 节的历史 42→42 算法不可用于当前库，也不能重跑一次性迁移。
...
交接包中的 `RELEASE-MANIFEST.json` 记录逐文件 SHA-256，Git bundle 保留提交和分支。新任务从集成人确认的当前提交建立独立 worktree。服务器本次安装 248 文件清单；旧 237 文件源码与原部署清单分别固定并保留。后续纯 Markdown 交接单列清单，不借文档变化改写原测试证据。
