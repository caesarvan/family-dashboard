...
> **线上版本：2026-09-17 01:47:27（北京时间）。** Expo 足迹地图已上线，支持地点管理、状态／年份／成员／旅行筛选、坐标隐私和旅行相册返回；从「更多 → 足迹地图」进入 `/app/map`。延续白底、黑色胶囊按钮和浅灰圆角卡片，操作与完整版本身份见 [Expo 地图](docs/EXPO-MAP.md)。

> 真实临时浏览器 16 项（含四宽）、Node 18 项与新镜像 Linux 142 项分别通过。实际 1 户两库完整备份，53+2 表无迁移且原数据与配置保持；01:47:42 正常 TLS 读回核对 464 源／112 运行／75 静态资源，四服务及备份正常。失败修订与证据见 [验证记录](docs/VALIDATION.md)。本人新入口、真实云与实体电视仍另验，原生安装包尚未交付；此前本人 Photos 5／5 保存成功事实保留。这些发布后本地文档不改变已安装 manifest。
...
详细财务、账户连接、家庭／成员、设备与布局管理、照片旅行推荐及复杂旅行分段／资料／云发布继续保留经典入口。地图与家庭物品的主入口迁入 Expo；电视继续独立只读 `/tv`。路线、精选回顾、视频、iCloud／NAS、通用成员角色和完整跨模块场景仍在 [产品计划](docs/PRODUCT-PLAN.md) 中逐项跟踪。
...
| 开发新版 React Native 界面 | [前端开发与构建](frontend/README.md)、[同源托管与安全边界](docs/EXPO-WEB.md)、[官网风格](docs/EXPO-SITE-STYLE.md)、[地图与旅行相册](docs/EXPO-MAP.md)、[三模块发布记录](docs/EXPO-NEXT-RELEASE.md) |
