...
> **线上版本：2026-09-17 06:15:34（北京时间），06:16:14 正常 TLS 读回通过。** 从「更多 → 我的持仓」或「家庭资金 → 我的持仓」进入 `/app/investments`，查看本人持仓、分币种汇总，手工维护或导入整理表。继续采用当前 [Expo 官网](https://expo.dev/) 的白底、黑色胶囊按钮和浅灰圆角卡片；操作见 [Expo 持仓](docs/EXPO-INVESTMENTS.md)，完整发布身份见 [验证记录](docs/VALIDATION.md#expo-holdings-release)。

> 本轮 Windows 446 项通过；新镜像 Linux 实际为 445 通过、1 个精确的 Windows junction 测试跳过、零失败／错误。沿用同字节前端的 Node 29 项、真实临时浏览器 7 场景与四宽 12 张截图；12 张均已人工查看，导入面板内部滚动区的完整预览仍未获得截图覆盖。实际 1 户两库备份后完成 54→55，只新增空操作回执表；原数据、schema、序列、注册库在新 app 启动核对时保全。本人真实文件、新云操作、原生安装及实体电视仍未验收，此前 Photos 5／5 成功事实保留。发布后的本地文档不改变已安装 manifest。

最终读回核对 503 源文件、113 运行文件、75 项 HTTPS 静态资源，22 个匿名 API 均拒绝访问；四服务运行且零重启、app 健康，原配置保持。新一次备份 service 成功、timer active，完整 1 户两库备份的散列和 55 表／注册表结构核验通过；这是逐库在线备份，没有重新核对 live 多库全组快照，也不保证跨库全局原子性。两次发布前本地检查拒绝及修订保留在 [验证记录](docs/VALIDATION.md#expo-holdings-release)。
...
新增源码候选：助理可将旅行文字带入可编辑简报，再核对准备事项、成员分工和采购预算并保存。当前构建、失败修正和验收边界见 [旅行协作验收](docs/EXPO-TRAVEL-ACCEPTANCE.md)；该候选不改变顶部已安装版本。
...
资产基线和来源报告等高级财务、家庭／成员、设备与布局管理、照片旅行推荐及复杂旅行分段／资料／云发布继续保留经典入口。财务账本、本人持仓、地图与家庭物品的主入口已迁入 Expo；电视继续独立只读 `/tv`。路线、精选回顾、视频、iCloud／NAS、通用成员角色和完整跨模块场景仍在 [产品计划](docs/PRODUCT-PLAN.md) 中逐项跟踪。
