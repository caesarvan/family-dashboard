...
> **线上版本：2026-09-16 18:05:30（北京时间），镜像 `sha256:87c1513e52c4188a27e324e8e02fb674e32c3fa8ce91015ba703fe14e5a02b39`。** 相册与电视照片轮播已完成 44→48 迁移发布；340 源文件、68 Markdown、51 静态资源、130 个方法／路径模板、48 张户内表及 2 张平台表。安装源码对应 main `2c2332d`／integration `8607b14` 的同一文件树；本次交接文档更新仅在本地，不改服务器安装清单。本人 Google Photos 全流程与实体电视仍待验收。
...
**最新发布：相册与电视照片轮播。** 从「家庭相册」连接本人 Google Photos，明确选择照片并确认保存；展示副本经后台净化、加密后默认私密。可手动关联旅行、共享给家人，再单独选择允许展示的电视，从手机控制照片轮播。使用步骤与范围见 [媒体交付说明](docs/MEDIA-DELIVERY.md)。这次生产迁移保留旧 44 表及全部原行／schema／序列，新增四张空表，实际 1 户两库完整备份。真实 Google 授权尚待本人操作；视频、iCloud／NAS 和自动回忆精选尚未实现。库存核心、API、界面及真实临时浏览器闭环属于独立候选，尚未正式接线、迁移或上线；更多成员角色仍待开发。
...
当时部署使用 [地点迁移流程](docs/JOURNEY-PLACES-RELEASE.md)，恢复演练显式选择 `journey_places44`。现在已是媒体 48 表，不能重放该 43→44、旧 43→43 或本轮一次性 44→48 迁移；后续发布须从实际 48 表基线重新制定，媒体恢复见 [媒体交付说明](docs/MEDIA-DELIVERY.md)。

NVIDIA 已于 16:58 通过独立配置步骤启用，模型为 `us/azure/openai/gpt-4o-mini`。服务器使用真实运行配置完成一次虚构家庭的模型待办草稿请求，未执行草稿或读取真实家庭事项；本次媒体发布保留这些配置。真实新云写入、本人相册授权和实体设备仍单独验收。
...
| 家庭相册 | Google Photos 本人选片、后台净化加密、明确保存为私密副本、旅行关联、家庭共享、逐台电视许可及照片元数据导出 | 已发布软件；本人 Google 实际授权／照片和实体电视待验收；仅照片，不支持视频、iCloud／NAS或自动回忆精选 |
