...
from deploy.expo_assistant_trip_search_release import prepare as adapter
...
def test_scope_and_reviewed_source_guards():
    ...
    assert set(adapter.UNCHANGED) == set(adapter.previous.UNCHANGED) | {'shopping_settlement.py'}
    assert set(adapter.BACKEND_UNCHANGED) == set(adapter.previous.BACKEND_UNCHANGED) | {'shopping_settlement.py'}
    ...
    assert pins == adapter.previous.reviewed_sources()
    assert pins['shopping_settlement.py'] == '90543ed9a2cbb41136a36d27da1558922fbf144030a50a689300ec15fd3e2067'
    assert len(adapter.BACKEND_UNCHANGED) == 39
    assert 'tests/test_expo_assistant_flow.py' in adapter.FORBIDDEN_TESTS
...
def test_missing_or_malformed_review_pin_creates_no_output(tmp_path, monkeypatch):
    output = tmp_path/'expo-assistant-trip-search-tools-synthetic'
    for pin in (None, '', '0'*63, 'A'*64, True):
        monkeypatch.setattr(adapter, 'SOURCE_PINS', {**adapter.previous.reviewed_sources(), 'shopping_settlement.py': pin})
        with pytest.raises(RuntimeError, match='reviewed backend source pins'):
            adapter.prepare(tmp_path, output)
        assert not output.exists()
    pins = adapter.previous.reviewed_sources()
    pins.pop('shopping_settlement.py')
    monkeypatch.setattr(adapter, 'SOURCE_PINS', pins)
    with pytest.raises(RuntimeError, match='reviewed backend source pins'):
        adapter.prepare(tmp_path, output)
    assert not output.exists()
...
def test_generation_is_exclusive_confined_unbound_and_rechecks_inputs(tmp_path, monkeypatch):
    ...
    raced = tmp_path/'expo-assistant-trip-search-tools-race'
...
def check_pinned_local_operators(source_root, git_repo, git_revision, evidence_dir):
    ...
    root_backends = {n for n in old if '/' not in n and n.endswith('.py')}
    assert root_backends == set(adapter.BACKEND_UNCHANGED)
    ...
    with tempfile.TemporaryDirectory(prefix='assistant-trip-search-release-') as folder, \
        ...
        output = access/'expo-assistant-trip-search-tools-synthetic'
        ...
        assert anonymous == parent_anonymous and len(set(anonymous)) == 39
        contract = shared.load_module('trip_search_contract', output/'operators/expo_contract.py')
        contract.verify_docker_delta(old_docker, blobs['Dockerfile'])
        package = shared.load_module('trip_search_package', output/'prepare-package.py')
        ...
        assert missing <= {'tests/browser_expo_assistant_trip_search_check.py'}
        rejected = []
        for name, function in (('operators/ops_common.py', 'source_state'), ('prepare-package.py', 'inspect_inputs')):
            ...
            guards = [n for n in body if isinstance(n, ast.Expr) and 'Reviewed assistant trip search source changed:' in ast.unparse(n)]
        ...
        frozen = access/'expo-assistant-trip-search-tools-frozen'; frozen_report = adapter.prepare(access, frozen, freeze)
        ...
        binder = shared.load_module('trip_search_binder', output/'bind-release.py')
        ...
        try:
            ...
            sys.modules['expo_contract'] = shared.load_module('trip_search_frozen_contract', ops/'expo_contract.py')
            common = shared.load_module('trip_search_frozen_common', ops/'ops_common.py'); sys.modules['ops_common'] = common
            ...
            for name in ('build.py', 'validate.py', 'stage.py', 'activate.py', 'post_readback.py'):
                phase = shared.load_module('trip_search_'+name[:-3], ops/name)
                argv = [str(ops/name)]
        ...
        return dict(sourceHashes=adapter.PINNED, reviewedSourceHashes=pins, generatedHashes=report['generatedHashes'],
            gitRevision=git_revision, pendingFinalIntegrationFiles=sorted(missing), requiredBuildInputs=len(required_inputs),
