...
from deploy.journey_reschedule_release import prepare as adapter
...
def test_new_output_is_confined_exclusive_and_unbound(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'journey-reschedule-tools-synthetic'
...
def test_required_scope_contains_current_and_previous_preservation_suites():
    ...
    assert {'Dockerfile', 'journey_workflows.py'} <= adapter.REQUIRED_CHANGED
    assert 'journey_reschedule.py' in adapter.REQUIRED_ADDED
    assert not adapter.REQUIRED_ADDED & adapter.REQUIRED_CHANGED
    assert 'Dockerfile' not in adapter.UNCHANGED
    assert {'finance_hub.py', 'investment_operations.py', 'deploy/backup.py', 'calendar_publish.py', 'journey_places.py'} <= set(adapter.UNCHANGED)
    assert len(adapter.PINNED) == 9 and len(set(adapter.PINNED.values())) == 9


@pytest.mark.parametrize('damage', ['unchanged', 'extra_line', 'normalize_all', 'duplicate_old', 'wrong_module', 'second_change', 'remove_line', 'old_lf'])
def test_only_exact_docker_copy_addition_is_permitted(damage):
    old = b'FROM fixed-parent\r\n' + adapter.DOCKER_OLD + b'COPY app.py ./\r\n'
    new = old.replace(adapter.DOCKER_OLD, adapter.DOCKER_NEW, 1)
    ns = {'need': adapter.need}; exec(adapter.DOCKER_CONTRACT, ns)
    ns['verify_docker_delta'](old, new)
    bad_old, bad_new = old, new
    if damage == 'unchanged': bad_new = old
    elif damage == 'extra_line': bad_new += b'\n'
    elif damage == 'normalize_all': bad_new = new.replace(b'\r\n', b'\n')
    elif damage == 'duplicate_old': bad_old += adapter.DOCKER_OLD; bad_new = bad_old.replace(adapter.DOCKER_OLD, adapter.DOCKER_NEW)
    elif damage == 'wrong_module': bad_new = new.replace(b'journey_reschedule.py', b'other.py')
    elif damage == 'second_change': bad_new = new.replace(b'COPY app.py', b'COPY removed.py')
    elif damage == 'remove_line': bad_new = new.replace(adapter.DOCKER_NEW, b'')
    else: bad_old = old.replace(adapter.DOCKER_OLD, adapter.DOCKER_OLD.replace(b'\r\n', b'\n'))
...
def check_pinned_local_operators(source_root, git_repo, git_revision):
    ...
    with tempfile.TemporaryDirectory(prefix='trip-coordination-guards-') as folder:
        ...
        output = access / 'journey-reschedule-tools-synthetic'; report = adapter.prepare(access, output)
        ...
        expected_paths = contract.runtime_paths(docker, set(manifest) | {'journey_reschedule.py'})
        assert 'journey_reschedule.py' in expected_paths
        validation = inputs[adapter.BASE + 'operators/validate.py']
        assert b"'--memory','768m'" in validation and b'/tmp:rw,size=536870912,mode=1777' in validation
        with pytest.raises(RuntimeError): contract.verify_docker_delta(old_docker, docker + b'\n')
        # Stage and the successful r2 validation resources remain byte-identical.
        for name in ('stage.py', 'validate.py'):
            assert (operator_dir / name).read_bytes() == inputs[adapter.BASE + 'operators/' + name]
        for name in ('activate.py', 'post_readback.py'):
            original = inputs[adapter.BASE + 'operators/' + name].replace(b'trip-coordination', b'journey-reschedule')
            if name == 'post_readback.py':
                original = original.replace(b"'/api/accounts','/api/journey-places','/api/journeys',", b"'/api/accounts','/api/journey-places','/api/journeys','/api/journeys/'+'0'*24+'/reschedule','/api/journeys/operations/'+'0'*32,")
        ...
        return dict(pinnedSources=adapter.PINNED, generatedHashes=report['generatedHashes'],
            ...
            actualDockerAdditionVerified=docker == old_docker.replace(adapter.DOCKER_OLD, adapter.DOCKER_NEW, 1), gitRevision=git_revision,
            preservationStageValidationUnchanged=True, recursiveGeneratedSyntaxVerified=True,
            validationMemoryMiB=768, validationTmpfsMiB=512,
