...
from deploy.expo_devices_release import prepare as adapter
...
def test_new_output_is_confined_exclusive_and_unbound(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'expo-devices-tools-synthetic'
...
def test_required_scope_contains_current_and_previous_preservation_suites():
    ...
    assert {'app.py', 'frontend/src/screens/HouseholdApp.tsx'} <= adapter.REQUIRED_CHANGED
    assert 'frontend/src/screens/DevicesScreen.tsx' in adapter.REQUIRED_ADDED
...
def check_pinned_local_operators(source_root, git_repo, git_revision):
    ...
    with tempfile.TemporaryDirectory(prefix='expo-device-guards-') as folder:
        ...
        output = access / 'expo-devices-tools-synthetic'; report = adapter.prepare(access, output)
        ...
        for name in ('activate.py', 'post_readback.py'):
            original = inputs[adapter.BASE + 'operators/' + name].replace(b'journey-reschedule', b'expo-devices')
            if name == 'post_readback.py':
                original = original.replace(b"'/api/accounts','/api/journey-places'", b"'/api/accounts','/api/devices','/api/media-playback/devices/'+'0'*24,'/api/journey-places'")
                original = original.replace(b"'/app/connections','/app/finance'", b"'/app/connections','/app/devices','/app/finance'")
