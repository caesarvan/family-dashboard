...
from deploy.expo_documents_release import prepare as adapter
...
def test_new_output_is_confined_exclusive_and_unbound(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'expo-documents-tools-synthetic'
...
def test_required_scope_covers_documents_sessions_hosting_and_preservation():
    ...
    assert {'journey_documents.py', 'frontend/src/screens/CalendarScreen.tsx',
            'frontend/src/screens/ListScreen.tsx', 'frontend/src/screens/TripsScreen.tsx'} <= adapter.REQUIRED_CHANGED
    assert {'frontend/src/screens/JourneyDocumentsPanel.tsx', 'frontend/src/lib/journeyDocuments.ts',
            'tests/browser_expo_journey_documents_check.py'} <= adapter.REQUIRED_ADDED
    ...
    assert 'journey_documents.py' not in adapter.UNCHANGED
...
def check_pinned_local_operators(source_root, git_repo, git_revision):
    ...
    with tempfile.TemporaryDirectory(prefix='expo-documents-guards-') as folder:
        ...
        output = access / 'expo-documents-tools-synthetic'; report = adapter.prepare(access, output)
        ...
        assert {'journey_documents.py', 'journey_reschedule.py', 'media_playback.py', 'tv_display.py'} <= expected_paths
        ...
        for name in ('activate.py', 'post_readback.py'):
            original = inputs[adapter.BASE + 'operators/' + name].replace(b'expo-appearance', b'expo-documents')
        ...
        frozen_output = access / 'expo-documents-tools-frozen-synthetic'
