...
from deploy.expo_finance_source_release import prepare as adapter
...
def test_new_output_is_confined_exclusive_and_unbound(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'expo-finance-source-tools-synthetic'
...
def test_scope_preserves_parent_guards_and_local_build_inputs():
    ...
    assert set(adapter.UNCHANGED) == set(adapter.previous.UNCHANGED) | set(adapter.BACKEND_UNCHANGED)
    assert len(adapter.BACKEND_UNCHANGED) == 37
    assert {'finance_baseline.py', 'finance_source_bridge.py', 'spending_observations.py',
            'financial_files.py', 'shopping_settlement.py'} <= set(adapter.UNCHANGED)
...
def check_pinned_local_operators(source_root, git_repo, git_revision, freeze=None, build_evidence=None):
    ...
    with tempfile.TemporaryDirectory(prefix='expo-finance-source-guards-') as folder, \
        ...
        output = access / 'expo-finance-source-tools-synthetic'; report = adapter.prepare(access, output)
        ...
        original_activation = inputs[adapter.BASE + 'operators/activate.py'].replace(b'expo-baseline', b'expo-finance-source')
        ...
        expected_post = original_post.replace('expo-baseline-55-', 'expo-finance-source-55-')
        ...
        assert len(set(old_paths)) == len(set(new_paths)) == 34
        ...
        frozen_output = access / 'expo-finance-source-tools-frozen-synthetic'
        ...
        return dict(pinnedSources=adapter.PINNED, generatedHashes=report['generatedHashes'],
            ...
            exactAnonymousEndpointsAdded=[], anonymousUniquePathsPreserved=34, anonymous401Required=True,
