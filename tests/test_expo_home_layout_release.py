...
from deploy.expo_home_layout_release import prepare as adapter
...
def test_new_output_is_confined_exclusive_and_unbound(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'expo-home-layout-tools-synthetic'
...
def test_required_scope_covers_layout_auth_hosting_and_preservation():
    ...
    assert 'dashboard_preferences.py' in adapter.REQUIRED_CHANGED
    assert 'frontend/src/screens/HomeLayoutPanel.tsx' in adapter.REQUIRED_ADDED
    ...
    assert {'Dockerfile', 'finance_hub.py', 'investment_operations.py', 'deploy/backup.py',
            ...
            'tv_display.py', 'app.py', 'frontend_runtime.py', 'frontend/src/app/_layout.tsx',
            'frontend/src/screens/TVScreen.tsx', 'frontend/src/ui/TVBoard.tsx',
            'frontend/src/ui/TVPhotoPlayer.web.tsx'} <= set(adapter.UNCHANGED)
...
def check_pinned_local_operators(source_root, git_repo, git_revision):
    ...
    with tempfile.TemporaryDirectory(prefix='expo-home-layout-guards-') as folder:
        ...
        output = access / 'expo-home-layout-tools-synthetic'; report = adapter.prepare(access, output)
        ...
        for name in ('activate.py', 'post_readback.py'):
            original = inputs[adapter.BASE + 'operators/' + name].replace(b'expo-tv', b'expo-home-layout')
        ...
        frozen_output = access / 'expo-home-layout-tools-frozen-synthetic'
