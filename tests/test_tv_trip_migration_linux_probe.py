...
import pytest
from deploy import tv_trip_migration_linux_probe as probe
...
def test_prepared_history_programs_and_source_are_complete_and_closed(prepared):
    ...
    assert {'media_video_storage.py','media_playback_progress.py','media_local_upload.py'}<=meta['historyFiles'].keys()
    assert 'journey_finance.py' in meta['historyFiles'] and 'media_trip_playback.py' not in meta['historyFiles']
    assert {'media_trip_playback.py','journey_finance.py','finance_hub.py','financial_files.py','finance_source_bridge.py','finance_baseline.py','requirements.txt'}<=meta['runtimeFiles'].keys()
    ...
    assert "list(counts.values())==[77,75]" in partial and 'migration.verify_rollback' in partial
...
def test_real_local_closed_migration_app_startup_and_full_restores(prepared,tmp_path):
    ...
    assert len(outcomes)==8 and outcomes['seed']['households']==2 and outcomes['seed']['householdTables']==75
    ...
    assert outcomes['rollback75']['completeGroupRestored'] and outcomes['restore77']['completeGroupRestored']
    assert outcomes['populate77']['rowsPerNewTable']==probe.ROWS
...
@pytest.mark.parametrize('fault',['runtime', 'rows'])
def test_manifest_with_rehashed_but_inconsistent_binding_is_refused(prepared,tmp_path,fault):
    base,result=prepared;target=tmp_path/'bundle';shutil.copytree(base/'bundle',target)
    meta=json.loads((target/'input.json').read_bytes())
    if fault=='runtime':meta['runtimeFiles']['media_trip_playback.py']='0'*64
    else:meta['binding']['rowsPerNewTable']['media_playback_operations']=4
    raw=probe.encoded(meta);(target/'input.json').write_bytes(raw)
    with pytest.raises(ValueError):probe.verify(target,probe.sha(raw))


def test_help_and_windows_run_refusal(tmp_path):
    help_run=subprocess.run([sys.executable,'-B',str(ROOT/probe.SELF),'--help'],capture_output=True,timeout=20)
    assert help_run.returncode==0 and b'prepare' in help_run.stdout and b'run' in help_run.stdout
    if sys.platform!='linux':
        with pytest.raises(ValueError,match='Linux root'):
            probe.run(tmp_path/'absent','a'*64,IMAGE,tmp_path/'output')
