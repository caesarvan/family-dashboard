"""Stopped synthetic two-household 73/9 -> 75/9 and complete-group restores.
...
import pytest
from deploy import check_journey_finance_migration as data
from deploy import journey_finance_release_data as current
...
PARENT = 'dc2b54cfc7fc9063eb839ece87fdab72a21855be'
...
def baseline73(tmp_path_factory):
    base = tmp_path_factory.mktemp('journey-parent73')
    source = base / 'source'
...
def source(tmp_path_factory):
    path = tmp_path_factory.mktemp('journey-candidate')
    hashes = {}
...
@pytest.fixture
def group(baseline73, tmp_path, source):
    root = tmp_path / 'live'; clone_databases(baseline73, root)
...
def populate(root):
    for path in root.rglob('household.sqlite3'):
        execute(path, """
          PRAGMA foreign_keys=ON;
          INSERT INTO hub_journey_allocations VALUES
            ('active','member1','journey','trip','payment',2500,'active',1,'digest',5000,0,'now','now'),
            ('revoked','member1','gone-journey','gone-trip','gone-payment',1000,'revoked',2,'digest',2000,0,'now','later'),
            ('orphan','member2','gone-journey','gone-trip','gone-payment',800,'active',1,'digest',900,0,'now','now');
          INSERT INTO hub_journey_allocation_operations VALUES
            ('member1','request','intent','apply','active','{"requestId":"request","operation":"apply","allocationId":"active","revision":1,"completedAt":"now"}','now');
        """)
...
def test_two_household_exact_addition_actual_app_only_restart_and_empty_tables(group, source):
    ...
    assert all(c == {'originalTablesPreserved':73,'newTables':2,'newTablesEmpty':True,
    ...
    assert len(definition['objects']) == 8  # 2 tables, 4 indexes, 2 automatic PK indexes
    assert not any(r[0] == 'trigger' for r in definition['objects'])
    for path in root.rglob('household.sqlite3'):
        value = shared.migration._read(path)
        assert len(set(value['rows']) - {'sqlite_sequence'}) == 75
...
@pytest.mark.parametrize('target,sql', [
    ...
    ('child',"INSERT INTO hub_journey_allocation_operations VALUES('member1','r','h','apply','a','{}','now')"),
])
...
def test_preflight_failure_preserves_all_databases_without_migration_attempt(group, fault):
    ...
    if fault == 'marker': (root / shared.ROOT_ATTEMPT).write_bytes(b'changed')
        ...
        identity=dict(kwargs['source_identity'],runtimeHashes={**kwargs['source_identity']['runtimeHashes'],'journey_finance.py':'0'*64})
...
def test_real_second_household_lock_partial_failure_and_complete_73_rollback(group,tmp_path,monkeypatch,partial):
    root,proof,kwargs=group;data.begin(root,proof,**kwargs)
    if partial:
...
def test_populated75_restart_backup_and_documented_complete_restore_preserve_orphans(group,source,tmp_path,monkeypatch):
    ...
    proof75=tmp_path/'current-proof';proof75.mkdir();current.begin(root,proof75,**kwargs)
    assert startup(source[0],root,'restart')['households']==2
    checked=current.check_stopped(root,proof75,**kwargs)
    assert checked['logicalSha256']==shared._digest(shared._logical(reference))
    receipt=json.loads((proof75/'backup.json').read_bytes())
    assert current.validate_backup(root,reference,proof75/'backup-group/backups'/receipt['manifest'])['databases']==3
    restored=tmp_path/'restored75'
    documented_restore(proof75/'backup-group',receipt['manifest'],restored,(root/shared.ROOT_ATTEMPT).read_bytes(),monkeypatch)
    ...
    for name,value in reference['databases'].items():
        if name!='platform.sqlite3':
            assert value['tables']['hub_journey_allocations']['count']==3
            assert value['tables']['hub_journey_allocation_operations']['count']==1
    edit(child(restored),'DELETE FROM hub_journey_allocation_operations')
    with pytest.raises(ERRORS):current.verify_restore(reference,restored,marker_sha256=kwargs['marker_sha256'])
    with pytest.raises(FileExistsError):current.begin(root,proof75,**kwargs)


def test_partial_index_ddl_error_rolls_back_actual_initializer(group,monkeypatch):
    ...
    def connect(*args,**kwargs):
        ...
        if args[0]==path.as_uri()+'?mode=rw':
            def gate(code,a,b,*rest):
                ...
                return sqlite3.SQLITE_DENY if code==sqlite3.SQLITE_CREATE_INDEX and a in data.NEW_INDEXES else sqlite3.SQLITE_OK
    ...
    assert any(code==sqlite3.SQLITE_CREATE_TABLE and a=='hub_journey_allocations' for code,a,b in observed)
    assert shared.migration._read(path)==before


@pytest.mark.parametrize('fault',['schema','import','dependency-import','initializer-hash'])
def test_fixed_initializer_schema_and_import_closure_reject_drift(group,monkeypatch,fault):
    root,proof,kwargs=group;data.begin(root,proof,**kwargs)
    if fault=='schema':
        m=data.runtime_module('journey_finance');monkeypatch.setattr(m,'SCHEMA_SQL',m.SCHEMA_SQL+'CREATE TABLE extra(id);')
    elif fault.endswith('import'):
        m=data.runtime_module('finance_hub' if fault=='dependency-import' else 'journey_finance')
...
@pytest.mark.parametrize('fault',['old71','partial74','name-collision','unregistered'])
def test_wrong_baseline_or_name_collision_rejected_before_backup(group,fault):
    ...
    if fault=='unregistered':
        ...
        execute(root/'household.sqlite3',{
            'old71':'DROP TRIGGER media_video_deleted; DROP TABLE media_playback_progress; DROP TABLE media_video_cache;',
            'partial74':'CREATE TABLE hub_journey_allocations(id TEXT);',
            'name-collision':'CREATE INDEX hub_journey_allocations_payment ON users(name);'}[fault])
