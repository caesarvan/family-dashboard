...
from deploy.expo_routines_release import prepare as adapter
...
def test_new_output_is_confined_exclusive_and_unbound(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'expo-routines-tools-synthetic'
...
def test_required_scope_covers_routines_sessions_hosting_and_preservation():
    assert adapter.REQUIRED_TESTS == {
        'tests/test_household_routines.py', 'tests/test_routine_journeys.py',
        'tests/test_routine_sessions.py',
        'tests/test_member_sessions.py', 'tests/test_household_spaces.py',
        'tests/test_frontend_runtime.py', 'tests/test_platform_backup.py',
        'tests/test_investment_operation_migration.py', 'tests/test_expo_routines_release.py',
    }
    assert {'household_routines.py', 'tests/test_household_routines.py',
            'frontend/src/screens/HouseholdApp.tsx', 'frontend/src/screens/OtherScreens.tsx'} <= adapter.REQUIRED_CHANGED
    assert {'frontend/src/screens/RoutinesPanel.tsx', 'frontend/src/lib/routines.ts',
            'tests/browser_expo_routines_check.py'} <= adapter.REQUIRED_ADDED
    ...
    assert 'household_routines.py' not in adapter.UNCHANGED
...
def test_added_routine_endpoints_accept_only_unauthorized(status):
    original = """for path in ('/api/accounts','/api/devices'):
    try:
...
"""
    ...
    for endpoint in ('/api/routines/context', '/api/routines/operations/' + '0' * 64):
        if status == 401:
...
def test_anonymous_insertion_refuses_missing_or_duplicate_anchor():
    for source in ('for path in ():', "for path in ('/api/accounts',): pass\n" * 2):
...
def check_pinned_local_operators(source_root, git_repo, git_revision):
    ...
    with tempfile.TemporaryDirectory(prefix='expo-routines-guards-') as folder:
        ...
        output = access / 'expo-routines-tools-synthetic'; report = adapter.prepare(access, output)
        ...
        original_activation = inputs[adapter.BASE + 'operators/activate.py'].replace(b'expo-documents', b'expo-routines')
        ...
        expected_post = original_post.replace('expo-documents-55-', 'expo-routines-55-').replace(
            "for path in ('/api/accounts',",
        ...
        assert new_paths == ('/api/routines/context', '/api/routines/operations/' + '0' * 64) + old_paths
        assert len(set(new_paths)) == len(set(old_paths)) + 2
        ...
        frozen_output = access / 'expo-routines-tools-frozen-synthetic'
