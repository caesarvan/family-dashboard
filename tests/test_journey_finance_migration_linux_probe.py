...
import pytest
from deploy import journey_finance_migration_linux_probe as probe
...
def test_prepared_history_programs_and_source_are_complete_and_closed(prepared):
    ...
    assert 'journey_finance.py' not in meta['historyFiles']
    assert {'journey_finance.py','finance_hub.py','financial_files.py','finance_source_bridge.py','finance_baseline.py','requirements.txt'}<=meta['runtimeFiles'].keys()
    ...
    assert "list(counts.values())==[75,73]" in partial and 'migration.verify_rollback' in partial
...
def test_real_local_closed_migration_app_startup_and_full_restores(prepared,tmp_path):
    ...
    assert len(outcomes)==8 and outcomes['seed']['households']==2 and outcomes['seed']['householdTables']==73
    ...
    assert outcomes['rollback73']['completeGroupRestored'] and outcomes['restore75']['completeGroupRestored']
    assert outcomes['populate75']['rowsPerNewTable']==probe.ROWS
...
def test_manifest_with_rehashed_but_inconsistent_binding_is_refused(prepared,tmp_path,fault):
    ...
    if fault=='runtime':meta['runtimeFiles']['journey_finance.py']='0'*64
    else:meta['binding']['rowsPerNewTable']['hub_journey_allocation_operations']=3
