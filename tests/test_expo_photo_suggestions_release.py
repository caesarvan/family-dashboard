"""Photo release guards; optional CLI checks actual pinned local operators without deployment."""
...
from deploy.expo_photo_suggestions_release import prepare as adapter
from tests import test_expo_trip_recap_release as parent_tests


def test_only_reviewed_media_backend_can_change():
    assert set(adapter.previous.BACKEND_UNCHANGED) - set(adapter.BACKEND_UNCHANGED) == {'household_media.py'}
    assert set(adapter.previous.UNCHANGED) - set(adapter.UNCHANGED) == {'household_media.py'}
    assert len(adapter.BACKEND_UNCHANGED) == 37
    scope = {'need': adapter.need, 'manifest': {'household_media.py': adapter.MEDIA_SOURCE_SHA256}}
    exec(adapter.MEDIA_GUARD.strip(), scope)
    for wrong in (None, '0' * 64, adapter.MEDIA_SOURCE_SHA256 + '0'):
        scope['manifest'] = {'household_media.py': wrong}
        with pytest.raises(RuntimeError, match='Reviewed photo session source changed'):
            exec(adapter.MEDIA_GUARD.strip(), scope)
    assert len(adapter.REQUIRED_TESTS) == 9
...
def test_changed_pinned_source_and_generation_race_leave_no_output(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'expo-photo-suggestions-tools-synthetic'
...
def test_output_cannot_overwrite_or_escape_source_root(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError): adapter.prepare(tmp_path, tmp_path / 'expo-trip-recap-tools-replay')
...
def check_pinned_local_operators(source_root):
    ...
    with tempfile.TemporaryDirectory(prefix='photo-release-check-') as folder, \
        ...
        output = access / 'expo-photo-suggestions-tools-synthetic'
        ...
        for name in ('build.py', 'activate.py', 'post_readback.py'):
            ...
            assert generated['operators/' + name].decode() == old.replace('expo-trip-recap', 'expo-photo-suggestions').replace('Trip Recap', 'photo suggestions')
        ...
        for name, function in (('operators/ops_common.py', 'source_state'), ('prepare-package.py', 'inspect_inputs')):
            ...
            pinned = next(n for n in body if isinstance(n, ast.Expr) and 'Reviewed photo session source changed' in ast.unparse(n))
            program = compile(ast.fix_missing_locations(ast.Module(body=[loop, pinned], type_ignores=[])), name, 'exec')
            old = {path: 'a' * 64 for path in adapter.UNCHANGED}
            manifest = {**old, 'household_media.py': adapter.MEDIA_SOURCE_SHA256}
            scope = {'need': adapter.need, 'old': old, 'manifest': manifest}
            exec(program, scope)
            for path in (*adapter.UNCHANGED, 'household_media.py'):
                scope['manifest'] = {**manifest, path: '0' * 64}
                with pytest.raises(RuntimeError): exec(program, scope)
                rejected.append(name + ':' + path)
        package = parent_tests.load_module('photo_package_guard', output / 'prepare-package.py')
        ...
        frozen = access / 'expo-photo-suggestions-tools-frozen'
        ...
        checked = []
        try:
            contract = parent_tests.load_module('photo_contract_guard', operator_dir / 'expo_contract.py')
            sys.modules['expo_contract'] = contract
            common = parent_tests.load_module('photo_common_guard', operator_dir / 'ops_common.py')
            ...
            for name in ('build.py', 'validate.py', 'stage.py', 'activate.py', 'post_readback.py'):
                value = parent_tests.load_module('photo_' + name[:-3], operator_dir / name)
        ...
        return dict(sourceHashes=adapter.PINNED, generatedHashes=report['generatedHashes'],
                    preservedBackendFiles=37, reviewedMediaSourceSha256=adapter.MEDIA_SOURCE_SHA256,
