...
from deploy.expo_tv_release import prepare as adapter
...
def test_new_output_is_confined_exclusive_and_unbound(tmp_path, monkeypatch):
    ...
    output = tmp_path / 'expo-tv-tools-synthetic'
...
def test_required_scope_covers_tv_auth_hosting_and_preservation():
    assert {'tests/test_frontend_runtime.py', 'tests/test_media_playback.py', 'tests/test_device_sessions.py', 'tests/test_household_spaces.py', 'tests/test_platform_backup.py', 'tests/test_investment_operation_migration.py'} <= adapter.REQUIRED_TESTS
    assert {'app.py', 'frontend_runtime.py', 'frontend/src/app/_layout.tsx'} <= adapter.REQUIRED_CHANGED
    assert 'frontend/src/screens/TVScreen.tsx' in adapter.REQUIRED_ADDED
    assert not adapter.REQUIRED_ADDED & adapter.REQUIRED_CHANGED
    assert {'Dockerfile', 'finance_hub.py', 'investment_operations.py', 'deploy/backup.py', 'calendar_publish.py',
            'journey_places.py', 'journey_reschedule.py', 'media_playback.py', 'tv_display.py'} <= set(adapter.UNCHANGED)
...
def check_pinned_local_operators(source_root, git_repo, git_revision):
    ...
    with tempfile.TemporaryDirectory(prefix='expo-tv-guards-') as folder:
        ...
        output = access / 'expo-tv-tools-synthetic'; report = adapter.prepare(access, output)
        ...
        for name in ('activate.py', 'post_readback.py'):
            original = inputs[adapter.BASE + 'operators/' + name].replace(b'expo-devices', b'expo-tv')
            if name == 'post_readback.py':
                original = original.replace(b"'/app/devices','/app/finance'", b"'/app/devices','/app/tv','/tv','/app/finance'")
                original = original.replace(b"('/classic','/tv','/demo')", b"('/classic','/tv?classic=1','/demo')")
                original = original.replace(b"if path=='/':need(response.geturl()==origin+'/app','Home did not redirect to Expo')",
                    b"if path=='/':need(response.geturl()==origin+'/app','Home did not redirect to Expo')\n            if path=='/tv':need(response.geturl()==origin+'/app/tv','TV did not redirect to Expo display')")
