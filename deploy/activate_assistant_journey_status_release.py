"""Assistant journey-status update on the installed five-service 77/9 database group."""
...
class Controller(shared.Controller):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs, mode='assistant-journey-status-source-update')


def verify_restored_group(root, proof, *, source_identity, plan_sha256):
    """Read-only verification after an explicit complete 77/9 group restore."""
    ...
    core.need(attempt == {'kind': data.SPEC.attempt_kind, 'planSha256': plan_sha256,
        ...
        'assistant_journey_status_restore_binding')
...
def main(argv=None):
    return shared.main(argv, mode='assistant-journey-status-source-update')
