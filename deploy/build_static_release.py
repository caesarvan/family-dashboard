...
def build(source, parent, manifest_sha, output, *, runner=command):
    ...
    values = source_files(source, manifest['files'])
    ...
    parent_probe = probe(runner, parent, backend, source)
    need(parent_probe['hashes'] == backend and parent_probe['backendFiles'] == sorted(backend), 'parent_backend_differs')
    ...
    source_files(source, manifest['files'])
    ...
    report = {'status': 'building', 'startedAt': datetime.now(timezone.utc).isoformat(),
              'mode': 'static-only', 'parentImage': parent, 'manifestSha256': manifest_sha,
    ...
    def frozen_context():
        ...
        need(found == set(static) | {'Dockerfile'}, 'build_context_changed')
        for name, digest in static.items():
    ...
    try:
        ...
        record()
        raise RuntimeError('static_build_failed; inspect_private_build_report') from None
