"""UI-only 71/9 source update; preserve populated reminders without migration."""
...
from deploy import activate_task_reminders_release as previous
from deploy import build_expo_calendar_conflicts_release as profile
from deploy.membership_release_controller import PUBLIC_ORIGIN, need

SPEC = core.ReleaseSpec(
    profile.BASELINE, 'expo-calendar-conflicts-release-plan', profile.PARENT_IMAGE, profile.OLD_MANIFEST,
    profile.DOCKER_BEFORE, profile.DOCKER_AFTER, profile.COPY_BEFORE, profile.COPY_AFTER,
    profile.CHANGED_ROOT_FILES, 'activate_expo_calendar_conflicts_release.py',
    previous.SPEC.operators + ('build_expo_calendar_conflicts_release.py',
        'activate_expo_calendar_conflicts_release.py', 'expo_calendar_conflicts_release_data.py'),
    'expo-calendar-conflicts-71-', previous.SPEC.env_sha256, schema_pair=profile.SCHEMA_AFTER)
...
class Controller(core.Controller):
    ...
    def backup_program(self):
        return ("from deploy import expo_calendar_conflicts_release_data as steady\n"
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit('expo_calendar_conflicts_release_failed; preserve evidence and stopped state; no automatic restore') from None
