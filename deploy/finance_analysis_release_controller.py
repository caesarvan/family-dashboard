"""Reviewed 61/9 to 66/9 analysis migration with complete old-group preservation.

The membership marker is immutable. Only literal five-table DDL is migrated;
normal new app startup is then checked. Shared admission and failure-stop helpers
...
class Controller(shared.Controller):
    def __init__(self, candidate, plan_sha256, *, runner=run, root=ROOT, releases=RELEASES):
        ...
        need(self.plan.get('schemaVersion') == 1 and self.plan.get('kind') == 'finance-analysis-release-plan', 'plan_kind')
        ...
        operator_names = ('finance_analysis_release_controller.py', 'finance_analysis_release_package.py', 'finance_analysis_release_data.py', 'finance_analysis_release_plan.py',
                          'membership_release_controller.py', 'membership_release_package.py',
                          'membership_release_build.py', 'membership_release_data.py',
             'finance_analysis_release_profile.py', 'check_finance_analysis_migration.py', 'steady_release_data.py')
    ...
    def stage(self):
        ...
        value = {'planSha256': self.plan_sha, 'services': services, 'oldFiles': old, 'tests': counts,
                 'imageId': self.plan['imageId'], 'schemaBefore': [61, 9], 'schemaAfter': [66, 9],
    ...
    def backup_program(self):
        return ("from deploy import finance_analysis_release_data as steady\n"
    ...
    def migration_program(self):
        return ("from deploy import finance_analysis_release_data as analysis\n"
                "result=analysis.migrate(root,proof,source_identity=identity,plan_sha256=" + repr(self.plan_sha) + ")\n"
                "print(json.dumps(result))")
    ...
    def _activate(self):
        ...
        release = self.releases / ('finance-analysis-66-' + stamp)
        ...
        record('exact_five_table_migration_verified')
        ...
        report.update(completed=True, completedAt=datetime.now(timezone.utc).isoformat(), services=current,
                      preservation=after, stoppedBackup=before, migration=migrated, householdTables=66, platformTables=9)
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit('finance_analysis_release_failed; preserve evidence and stopped state; no automatic restore') from None
