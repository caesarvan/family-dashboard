"""Stopped complete-group 69/9 -> 71/9 migration using the reminder module's DDL.
...
from deploy import assistant_trip_items_release_data as baseline
from deploy import assistant_trip_change_release_data as backup_core
from deploy import check_journey_routes_migration as routes

ROOT = Path(__file__).resolve().parents[1]
BASE_TABLES = routes.BASE_TABLES | routes.NEW_TABLES
NEW_TABLES = frozenset({'task_reminders', 'task_reminder_operations'})
SCHEMA_SHA256 = '72470a0ce30bffe0e710184b0498477a0345d8e4197da6c6f5510a6020389657'
...
def reminder_module():
    module = importlib.import_module('task_reminders')
    expected = shared.migration.checked_path(ROOT / 'task_reminders.py')
    ...
    if ROOT == Path('/release'):
        allowed.add(Path('/app/task_reminders.py'))
    need(actual in allowed and actual.read_bytes() == expected.read_bytes(), 'reminder_module_import_changed')
    return module


def _execute_schema(con, sql):
    # The reviewed constant consists only of these two CREATE statements. Avoid
    # executescript: it would commit an already-open caller transaction.
    for statement in (s.strip() for s in sql.split(';') if s.strip()):
        con.execute(statement)


def schema_definition():
    """Derive exact table objects/columns from the runtime constant, not copied DDL."""
    ...
    for statement in (s.strip() for s in sql.split(';') if s.strip()):
        ...
        need(table is not None and table[1] in NEW_TABLES, 'unexpected_schema_statement')
        tables.append(table[1])
    need(len(tables) == 2 and set(tables) == NEW_TABLES, 'unexpected_reminder_schema')
    ...
    return {'profile': 'task_reminders71', 'sql': sql, 'sha256': SCHEMA_SHA256,
            'sourceHashes': {'task_reminders.py': shared.migration.file_digest(ROOT / 'task_reminders.py')},
            'tables': sorted(NEW_TABLES), 'objects': objects, 'columns': columns}


def verify_baseline(value):
    need(len(BASE_TABLES) == 69, 'baseline_definition_changed')
    routes.verify_current(value, routes.schema_definition())


def verify_current(value, schema):
    need({n for n in value['rows'] if not n.startswith('sqlite_')} == BASE_TABLES | NEW_TABLES,
         'expected_71_table_profile')
    need([r for r in value['schema'] if r[2] in NEW_TABLES] == schema['objects'], 'reminder_schema_changed')
    need(all(value['columns'][n] == schema['columns'][n] for n in NEW_TABLES), 'reminder_columns_changed')
...
def verify_addition(before, after, schema):
    ...
    return {'originalTablesPreserved': 69, 'newTables': 2, 'newTablesEmpty': True,
...
def initialize_database(path, schema):
    ...
    need(module.SCHEMA_SQL == schema['sql'] and
         shared.migration.file_digest(ROOT / 'task_reminders.py') == schema['sourceHashes']['task_reminders.py'],
         'reminder_source_changed')
    with closing(sqlite3.connect(path.as_uri() + '?mode=rw', uri=True, timeout=0)) as con:
        con.execute('PRAGMA trusted_schema=OFF')
        con.execute('PRAGMA foreign_keys=ON')
        con.execute('BEGIN IMMEDIATE')
        try:
            _execute_schema(con, module.SCHEMA_SQL)
            need(con.in_transaction, 'initializer_committed_transaction')
            con.commit()
...
def begin(root, proof, **kwargs):
    ...
    # It admits populated 69/9, backs up every registry DB and closes only empty WAL.
...
def evidence(root, proof, *, source_identity, plan_sha256, marker_sha256=MEMBERSHIP_MARKER_SHA256):
    ...
    need(attempt == {'kind': baseline.SPEC.attempt_kind, 'planSha256': plan_sha256,
        'beforeSha256': shared._digest(before), 'sourceIdentity': identity,
        'sourceIdentitySha256': shared._digest(identity), 'markerSha256': marker_sha256}, 'reminder_attempt_binding')
    ...
    backup, paths = backup_core._backup(root, before, manifest, spec=baseline.SPEC)
...
def snapshot_current(root):
    """Read a complete populated 71/9 group; retain reminder states and operation receipts."""
...
def verify_rollback(proof, restored_root):
    """Read-only verification of full 69/9 rollback; never invalidate retained receipts."""
