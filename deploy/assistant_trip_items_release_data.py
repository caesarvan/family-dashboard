"""Populated 69/9 preservation using the existing closed reader and backup core.

No DDL or initializer runs here. All three route tables, including soft-deleted
routes and their operation receipts, are part of the complete logical snapshot.
"""
import re
...
snapshot = snapshot_current = routes.snapshot_current
verify_restore = routes.verify_restore
...
def _profile(value):
    ...
    for path, fingerprint in value['databases'].items():
        ...
        expected = (shared.migration.BASE_PLATFORM_TABLES | shared.migration.NEW_PLATFORM_TABLES
                    if platform else routes.BASE_TABLES | routes.NEW_TABLES)
        tables = {n for n in fingerprint['tables'] if not n.startswith('sqlite_')}
        shared.need(tables == expected and len(tables) == (9 if platform else 69), 'expected_69_9_profile')
...
SPEC = shared.DataSpec(_profile, snapshot_current, 'assistant-trip-items-release-attempt')
