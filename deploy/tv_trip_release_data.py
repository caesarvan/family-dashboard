"""Populated 77/9 preservation using the existing closed reader and backup core.

No DDL or initializer runs here. Travel display selections and receipts, allocations, media and reminders and every previous table remain in the complete logical snapshot.
"""
import re

from deploy import assistant_trip_change_release_data as shared
from deploy import check_tv_trip_migration as journey
...
def _profile(value):
    ...
    for path, fingerprint in value['databases'].items():
        ...
        shared.need(tables == expected and len(tables) == (9 if platform else 77), 'expected_77_9_profile')
...
SPEC = shared.DataSpec(_profile, snapshot_current, 'tv-trip-source-update-attempt')
