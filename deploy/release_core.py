"""Shared 43-to-43 transaction with fixed, entry-selected policies. No schema initialization or automatic restore.
...
CORE = 'deploy/release_core.py'
CONTROLLER = 'deploy/activate_static_release.py'
SOURCE_ENTRY = 'deploy/source_release.py'


class Policy(Enum):
    STATIC = ('static-only', 'static', CONTROLLER)
    SOURCE = ('source-update', 'source', SOURCE_ENTRY)

    @property
    def mode(self): return self.value[0]
    @property
    def prefix(self): return self.value[1]
    @property
    def entry(self): return self.value[2]


STATIC, SOURCE = Policy.STATIC, Policy.SOURCE
TOOLS = {CORE, HELPER, CONTROLLER, 'deploy/build_static_release.py', 'deploy/rehearse_static_release.py'}
DEPENDENCIES = {
...
REQUIRED = {CORE, HELPER, CONTROLLER, 'journey_documents.py', 'deploy/backup.py',
...
COMPATIBILITY = {'noSchemaChanges': True, 'noDataMigration': True,
                 'noDependencyChanges': True, 'noConfigurationChanges': True}


def compatible(value):
    return isinstance(value, dict) and set(value) == set(COMPATIBILITY) and all(v is True for v in value.values())


def bind_modules(values, paths):
    """Bind every executed shared module, not just the thin command entry."""
    root = Path(__file__).resolve().parents[1]
    for name in paths:
        need(values.get(name) == plain_file(root / name), 'controller_changed')


def runtime_changes(base, candidate):
    return [{'path': n, 'beforeSha256': base.get(n), 'afterSha256': d,
             'kind': 'static' if n.startswith('static/') else 'rootPython'}
            for n, d in sorted(candidate.items()) if base.get(n) != d
            and (n.startswith('static/') or '/' not in n and n.endswith('.py'))]


def validate_source_changes(base, candidate, changed):
    ...
    python = [n for n in changed if '/' not in n and n.endswith('.py')]
    ...
    need(set(backend_hashes(base)) == set(backend_hashes(candidate)), 'backend_file_set_changed')
    need(base.get('requirements.txt') == candidate.get('requirements.txt'), 'dependencies_changed')


def source_evidence(parsed, ready, hashes, image, read):
    """Current runs and explicit applicability; never relabel the old full suite."""
    base = ready['baseHashes']
    changes = runtime_changes(base, hashes)
    python = sorted(x['path'] for x in changes if x['kind'] == 'rootPython')
    build = parsed['sourceBuild']
    need(build.get('image') == image and build.get('parentImage') == ready['previousImage']
         and build.get('mode') == SOURCE.mode and build.get('manifestSha256') == ready['manifestSha256']
         and build.get('baseManifestSha256') == ready['baseManifestSha256']
         and build.get('parentRuntimeHashes') == {**backend_hashes(base), **static_hashes(base)}
         and build.get('candidateRuntimeHashes') == {**backend_hashes(hashes), **static_hashes(hashes)}
         and build.get('approvedRuntimeChanges') == changes
         and all(build.get(k) is True for k in ('parentLayersPreserved', 'configurationUnchanged',
                                              'parentBackendVerified', 'childBackendVerified', 'staticVerified'))
         and build.get('bytecodeExcluded') is True and build.get('pipExecuted') is False, 'source_build')
    validation = parsed['backendValidation']
    need(validation.get('coverageMode') == 'affected-source-tests'
         and validation.get('changedPython') == python and python
         and compatible(validation.get('compatibility')), 'source_backend_validation')
    runs = validation.get('runs')
    need(isinstance(runs, list) and runs, 'source_backend_runs')
    covered = {'windows': set(), 'linux': set()}
    for run in runs:
        platform = run.get('platform')
        need(platform in covered and type(run.get('exitCode')) is int and run['exitCode'] == 0, 'source_run_failed')
        scripts = run.get('scripts')
        need(isinstance(scripts, list) and scripts and len(set(scripts)) == len(scripts)
             and all(isinstance(n, str) and n.startswith('tests/test_') and n.endswith('.py') and n in hashes for n in scripts), 'source_test_scripts')
        deps = run.get('dependencyHashesBefore')
        need(isinstance(deps, dict) and deps == run.get('dependencyHashesAfter')
             and set(backend_hashes(hashes)) | set(scripts) <= set(deps)
             and all(hashes.get(n) == d for n, d in deps.items()), 'source_test_dependencies')
        claimed = run.get('coveredChanges')
        need(isinstance(claimed, list) and claimed and set(claimed) <= set(python), 'source_test_coverage')
        covered[platform].update(claimed)
        if platform == 'linux': need(run.get('image') == image, 'source_test_image')
        # Parse actual JUnit, not only the normalizer's count fields.
        cases = list(ET.fromstring(read(run['junit'])).iter('testcase'))
        ...
        need(counts['passed'] > 0 and counts['failures'] == counts['errors'] == 0
             and all(type(run.get(k)) is int and run[k] == v for k, v in counts.items()), 'source_junit_failed')
        need(all(any(c.get('classname', '') == n[:-3].replace('/', '.')
                     or c.get('classname', '').startswith(n[:-3].replace('/', '.') + '.')
                     for c in cases) for n in scripts), 'source_junit_scope')
        read(run['log'])
    need(all(v == set(python) for v in covered.values()), 'source_platform_coverage')
    historical = validation.get('historicalReuse')
    need(isinstance(historical, list), 'source_history_shape')
    for item in historical:
        deps = item.get('dependencyHashes')
        need(isinstance(deps, dict) and deps and not set(deps).intersection(python)
             and all(hashes.get(n) == d for n, d in deps.items())
             and item.get('excludedChangedFiles') == python
             and isinstance(item.get('reason'), str) and item['reason'].strip(), 'source_history_applicability')
        read(item['record'])


def validate_changes(base, candidate, changed, *, policy=STATIC):
    need(policy in (STATIC, SOURCE), 'unknown_release_policy')
    if policy is SOURCE:
        return validate_source_changes(base, candidate, changed)
    need(set(base) <= set(candidate), 'source_removal')
...
def evidence(candidate, ready, hashes, image, *, policy=STATIC):
    need(policy in (STATIC, SOURCE), 'unknown_release_policy')
    refs = ready.get('evidence')
    kinds = {'staticBuild', 'backendReuse'} if policy is STATIC else {'sourceBuild', 'backendValidation'}
    need(isinstance(refs, dict) and set(refs) == kinds | {'browsers', 'releaseSafety', 'gitReview'}, 'evidence_set')
    frozen, parsed = {}, {}
    ...
    if policy is SOURCE:
        source_evidence(parsed, ready, hashes, image, read)
    else:
    ...
    if policy is SOURCE:
        need(review.get('approvedRuntimeChanges') == ready.get('approvedRuntimeChanges')
             and compatible(review.get('compatibility')), 'source_review_binding')
    return frozen
...
if (not sys.dont_write_bytecode or sys.pycache_prefix is not None
    or not Path('/app').is_dir()
    or any(p.is_file() and p.suffix in ('.pyc','.pyo') for p in Path('/app').rglob('*'))):
    raise RuntimeError('source_bytecode_unsafe')
'''
SOURCE_CONTAINER_CODE = BYTECODE_CHECK + CONTAINER_CODE
SOURCE_READBACK_CODE = BYTECODE_CHECK + READBACK_CODE
...
def _activate(candidate_root, image, manifest_sha, ready_sha, *, root=ROOT, releases=RELEASES,
              project='family-dashboard', volume=VOLUME, runner=command, policy=STATIC):
    need(policy in (STATIC, SOURCE), 'unknown_release_policy')
    ...
    validate_changes(base['files'], candidate['files'], changed, policy=policy)
    if policy is SOURCE:
    ...
    bind_modules(values, [CORE, policy.entry])
    ...
    proof_bytes = evidence(candidate_root, ready, candidate['files'], image, policy=policy)
    ...
    report = {'phase': 'preflight', 'status': 'pending', 'image': image, 'previousImage': previous,
              ...
              'oldManifestSha256': ready['oldManifestSha256'], 'helperSha256': candidate['files'][HELPER], 'mode': policy.mode, 'releaseDirectory': str(release),
    ...
    def data(action):
        ...
        program = CONTAINER_CODE if policy is STATIC else SOURCE_CONTAINER_CODE
    ...
    try:
        ...
        program = READBACK_CODE if policy is STATIC else SOURCE_READBACK_CODE
        ...
        if policy is SOURCE: report['bytecodeExcluded'] = True
...
def activate(candidate_root, image, manifest_sha, ready_sha, *, root=ROOT, releases=RELEASES,
    ...
    with release_lock(releases):
        return _activate(candidate_root, image, manifest_sha, ready_sha, root=root, releases=releases,
                         project=project, volume=volume, runner=runner, policy=policy)
