"""Explicit 54 -> 55 investment-operation migration after all writers stop.

Only the reviewed investment operation schema constant is used. No application factory,
...
from deploy import check_finance_receipt_migration as previous
from deploy import check_media_migration as media

ROOT = Path(__file__).resolve().parents[1]
BASE_TABLES = frozenset(previous.BASE_TABLES | previous.NEW_TABLES)
NEW_TABLES = frozenset({'hub_investment_operations'})
need, snapshot, validate_backup = media.need, media.snapshot, media.validate_backup


def schema_definition():
    source = ROOT / 'investment_operations.py'
    need(source.is_file() and not source.is_symlink(), 'schema_source_missing')
    need(Path(investment_operations.__file__).resolve() == source.resolve(), 'schema_module_origin')
    sql = getattr(investment_operations, 'INVESTMENT_OPERATIONS_SCHEMA_SQL', None)
    ...
    with closing(sqlite3.connect(':memory:')) as con:
        ...
        columns = [list(row) for row in con.execute('PRAGMA table_xinfo(hub_investment_operations)')]
        foreign_keys = [list(row) for row in con.execute('PRAGMA foreign_key_list(hub_investment_operations)')]
    return {'profile': 'investment_operations55', 'sql': sql,
...
def verify_baseline(before):
    need(len(BASE_TABLES) == 54, 'baseline_definition_changed')
    ...
    for value in before['households'].values():
        ...
        need(actual == BASE_TABLES, 'expected_54_table_baseline')


def verify_current(current, schema):
    """Validate a populated 55-table snapshot, without requiring empty receipts."""
    ...
    for value in current['households'].values():
        actual = {name for name in value['tables'] if not name.startswith('sqlite_')}
        need(actual == BASE_TABLES | NEW_TABLES, 'expected_55_table_snapshot')
        addition = [row for row in value['schema'] if row[2] in NEW_TABLES]
        need(addition == schema['objects'], 'receipt_schema_changed')
        table = value['tables']['hub_investment_operations']
        need(table['columns'] == schema['columns'] and table['foreignKeys'] == schema['foreignKeys'], 'receipt_columns_or_foreign_keys_changed')
    return {'profile': 'investment_operations55', 'households': len(current['households']),
            'tablesPerHousehold': 55, 'receiptRows': sum(v['tables']['hub_investment_operations']['count'] for v in current['households'].values())}
...
def verify_addition(before, after, schema):
    ...
    for uid, old in before['households'].items():
        ...
        need(new['tables']['hub_investment_operations']['count'] == 0, 'new_table_not_empty')
    return {'originalTablesPreserved': 54, 'newTables': 1, 'households': len(before['households']),
...
def main():
    ...
    if args.action in ('snapshot', 'snapshot-current'):
        ...
        if all('hub_investment_operations' in value['tables'] for value in before['households'].values()):
