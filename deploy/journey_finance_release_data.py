"""Populated 75/9 preservation using the existing closed reader and backup core.

No DDL or initializer runs here. Both journey allocation tables, video and reminder states/receipts and every previous table remain in the complete logical snapshot.
"""
import re

from deploy import assistant_trip_change_release_data as shared
from deploy import check_journey_finance_migration as journey

MEMBERSHIP_MARKER_SHA256 = shared.MEMBERSHIP_MARKER_SHA256
marker_digest = shared.marker_digest
snapshot = snapshot_current = journey.snapshot_current
verify_restore = journey.verify_restore
...
def _profile(value):
    ...
    for path, fingerprint in value['databases'].items():
        ...
        expected = (shared.migration.BASE_PLATFORM_TABLES | shared.migration.NEW_PLATFORM_TABLES
                    if platform else journey.BASE_TABLES | journey.NEW_TABLES)
        tables = {n for n in fingerprint['tables'] if not n.startswith('sqlite_')}
        shared.need(tables == expected and len(tables) == (9 if platform else 75), 'expected_75_9_profile')
...
SPEC = shared.DataSpec(_profile, snapshot_current, 'journey-finance-source-update-attempt')
