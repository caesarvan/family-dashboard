"""Generate a reviewed, unbound 55 -> 55 Expo household routines release; no remote actions."""
...
from deploy.expo_documents_release import prepare as previous

BASE = 'expo-documents-tools-20260917-r1/'
PREPARE_SOURCE = BASE + 'prepare-package.py'
PINNED = {
    BASE + 'operators/ops_common.py': '7183af919eb01c05dc57caf75bcd61cdfedddb26721c3e4f26b86fb325f706f6',
    BASE + 'operators/expo_contract.py': '87b27d86951c54cc44ed3619f256ec37d83d9c804624cb776e2ff97cc7ededbf',
    BASE + 'operators/build.py': 'a467f78bd3c1135f535e09d6f9fc55502572053b5f0ec5b6f061a0642c0b34f0',
    ...
    BASE + 'operators/activate.py': '93d7161a9af8e98caaf1d2732c54c6a1a7220b8d975b5b4c46e1055082ce264d',
    BASE + 'operators/post_readback.py': '264d4de09d6bec789f11cdec437757cc2d6295c01547969f24467e716d7c409c',
    PREPARE_SOURCE: 'ebbec15071473164b96dd9f3d8e400e7e0f5144c6f449f07a36a932af73c7928',
    BASE + 'bind-release.py': '214b6108d1792bc2da31b1b83aa946337dcf9036b17e320e449f44c4b5a4610e',
}
OLD_MANIFEST = '9fe4a665f724c1e54a9d28b93bdaeb94df0ec8b39f027c15b669547e02dd5b92'
OLD_ARCHIVE = '68eefe89a79deb6373cdeacc9f115c9e29a8fde27481b3b22ca2089911a006e4'
PARENT_IMAGE = 'sha256:79d20992e17a2ed5f1c03a47acd0bd8af91625d2445f5eb8b7c79d1b7ef39019'
# Exact final test count must come from collection, not a previous release.
REQUIRED_TESTS = {
    'tests/test_household_routines.py', 'tests/test_routine_journeys.py',
    'tests/test_routine_sessions.py', 'tests/test_member_sessions.py',
    'tests/test_household_spaces.py', 'tests/test_frontend_runtime.py',
    'tests/test_platform_backup.py', 'tests/test_investment_operation_migration.py',
    'tests/test_expo_routines_release.py',
}
FORBIDDEN_TESTS = previous.FORBIDDEN_TESTS
REQUIRED_CHANGED = {
    'household_routines.py', 'tests/test_household_routines.py',
    'frontend/src/lib/types.ts', 'frontend/src/screens/HouseholdApp.tsx',
    'frontend/src/screens/OtherScreens.tsx',
}
REQUIRED_ADDED = {
    'frontend/src/lib/routines.ts', 'frontend/src/screens/RoutinesPanel.tsx',
    'tests/test_expo_routines.mjs', 'tests/test_routine_sessions.py',
    'tests/browser_expo_routines_check.py',
    'deploy/expo_routines_release/prepare.py', 'tests/test_expo_routines_release.py',
}
# The parent's tuple does not include household_routines.py. Preserve every
...
def add_anonymous_checks(code):
    ...
    return replace(code, "for path in ('/api/accounts',",
                   "for path in ('/api/routines/context','/api/routines/operations/'+'0'*64,'/api/accounts',")
...
def adapt(inputs, access, output, config=None):
    ...
    common = replace(common, '55 -> 55 Expo journey documents source-update guards', '55 -> 55 Expo household routines source-update guards')
    common = replace(common, repr(previous.UNCHANGED), repr(UNCHANGED))
    operators['ops_common.py'] = common
    operators['build.py'] = replace(operators['build.py'], 'family-dashboard-expo-documents:', 'family-dashboard-expo-routines:')
    operators['activate.py'] = replace(operators['activate.py'], 'expo-documents', 'expo-routines', count=4)
    operators['post_readback.py'] = replace(operators['post_readback.py'], 'expo-documents-55-', 'expo-routines-55-')
    ...
    for before, after in ((previous.OLD_MANIFEST, OLD_MANIFEST), (previous.OLD_ARCHIVE, OLD_ARCHIVE),
                          (previous.PARENT_IMAGE, PARENT_IMAGE), ('Wrong installed Expo member appearance archive', 'Wrong installed Expo journey documents archive'),
...
def freeze_config(path, access, generated):
    ...
    namespace = {'__name__': 'expo_routines_freeze_check', '__file__': str(access / 'routines-prepare-validation.py')}
...
def prepare(access, output, freeze=None):
    ...
    need(output.parent == access and re.fullmatch('expo-routines-tools-[a-z0-9-]+', output.name), 'New routines output must be directly under access root')
