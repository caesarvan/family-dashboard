"""Fixed 69/9 -> 71/9 reminder release with full stopped backups and failure stop.
...
order, with only this profile's prefix, two-table step and final count changed.
"""
...
from deploy import activate_task_dependencies_release as previous
...
SPEC = shared.ReleaseSpec(
    profile.BASELINE, 'task-reminders-release-plan', profile.PARENT_IMAGE, profile.OLD_MANIFEST,
    profile.DOCKER_BEFORE, profile.DOCKER_AFTER, profile.COPY_BEFORE, profile.COPY_AFTER,
    profile.CHANGED_ROOT_FILES, 'activate_task_reminders_release.py',
    previous.SPEC.operators + ('check_task_reminders_migration.py', 'build_task_reminders_release.py',
                              'activate_task_reminders_release.py'),
    'task-reminders-71-', previous.SPEC.env_sha256, schema_pair=profile.SCHEMA_AFTER)
...
class Controller(previous.Controller):
    ...
    def stage(self):
        ...
        value = {'planSha256': self.plan_sha, 'services': services, 'oldFiles': old, 'tests': counts,
                 'imageId': self.plan['imageId'], 'schemaBefore': list(profile.SCHEMA_BEFORE), 'schemaAfter': list(profile.SCHEMA_AFTER),
    ...
    def backup_program(self):
        return ("from deploy import check_task_reminders_migration as reminders\n"
                "result=reminders.begin(root,proof,source_identity=identity,plan_sha256=" + repr(self.plan_sha) + ")\n"
                "print(json.dumps(result))")

    def migration_program(self):
        return ("from deploy import check_task_reminders_migration as reminders\n"
                "result=reminders.migrate(root,proof,source_identity=identity,plan_sha256=" + repr(self.plan_sha) + ")\n"
                "print(json.dumps(result))")

    def check_program(self):
        return ("from deploy import check_task_reminders_migration as reminders\n"
                "result=reminders.check_stopped(root,proof,source_identity=identity,plan_sha256=" + repr(self.plan_sha) + ")\n"
                "print(json.dumps(result))")
    ...
    def _activate(self):
        ...
        record('exact_two_table_migration_verified')
        ...
        report.update(completed=True, completedAt=datetime.now(timezone.utc).isoformat(), services=current,
                      preservation=after, stoppedBackup=before, migration=migrated, householdTables=71, platformTables=9,
                      anonymousGuards=guards)
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit('task_reminders_release_failed; preserve evidence and stopped state; no automatic restore') from None
