"""Generate a reviewed, unbound 55 -> 55 Expo finance source import UI release; no remote actions."""
...
from deploy.expo_baseline_release import prepare as previous

BASE = 'expo-baseline-tools-20260917-r1/'
PREPARE_SOURCE = BASE + 'prepare-package.py'
PINNED = {
    BASE + 'operators/ops_common.py': '11e0fe54b74a58afc126fd3c4eff420579e9988f907c62af9ee1520a08790016',
    BASE + 'operators/expo_contract.py': '87b27d86951c54cc44ed3619f256ec37d83d9c804624cb776e2ff97cc7ededbf',
    BASE + 'operators/build.py': '4b3c7309967fa6367af3583b632a3790398beb8bd7218eec23b9b6e17a3a9a02',
    ...
    BASE + 'operators/activate.py': 'e8ac39452cb52b99956aa92e993b2c0aa17f4b48fb5b9eea481143bbd51c089d',
    BASE + 'operators/post_readback.py': '212782b1fca523cf4822ae00a7f3cbb230ffd24dcab78f87ad8c2c0cc468cc47',
    BASE + 'prepare-package.py': '30fb5b75cfded931af491cbf0db8969f8dddc3c89baa44ce83ca45bcfc352d27',
    BASE + 'bind-release.py': 'c9bee73448b1a7bc1c76a8887b5bf675a362b97c0e3895e422d475ff44c8a8a2',
}

OLD_MANIFEST = '265830d1037696d39f1712a7f19cec25780de788071406cc0442956b06eb3252'
OLD_ARCHIVE = 'bee0c0e751f5aca0599920560bb20fadaeeb1de7779a81f9949fe4e39fe5bef8'
PARENT_IMAGE = 'sha256:4e3de8d45b39e251a7c0d3a068847c758617957028652b2f7dcc1f80e01cc5f2'
...
REQUIRED_TESTS = {
    ...
    'tests/test_finance_baseline.py', 'tests/test_frontend_runtime.py',
    'tests/test_household_spaces.py', 'tests/test_platform_backup.py',
    'tests/test_investment_operation_migration.py', 'tests/test_expo_finance_source_release.py',
}
FORBIDDEN_TESTS = previous.FORBIDDEN_TESTS
REQUIRED_CHANGED = {
    'frontend/src/lib/types.ts', 'frontend/src/screens/FinanceScreen.tsx',
    'frontend/src/screens/HouseholdApp.tsx',
}
LOCAL_BUILD_SOURCES = previous.LOCAL_BUILD_SOURCES
REQUIRED_ADDED = {
    'frontend/src/lib/financeSourceImport.ts', 'frontend/src/components/FinanceSourceImportPanel.tsx',
    'tests/test_expo_finance_source_import.mjs', 'tests/browser_expo_finance_source_check.py',
    'deploy/expo_finance_source_release/prepare.py', 'tests/test_expo_finance_source_release.py',
}
# This batch changes only the client. Bind every existing root Python runtime,
# including all amount, source normalization, privacy and session implementations.
BACKEND_UNCHANGED = (
    'app.py',
    'calendar_publish.py',
    'cloud_accounts.py',
...
UNCHANGED = previous.UNCHANGED + tuple(name for name in BACKEND_UNCHANGED if name not in previous.UNCHANGED)
ANONYMOUS_PATHS = previous.ANONYMOUS_PATHS  # Existing five source checks; never insert them twice.
...
def adapt(inputs, access, output, config=None):
    ...
    common = replace(common, '55 -> 55 Expo private baseline source-update guards', '55 -> 55 Expo finance source import UI source-update guards')
    common = replace(common, repr(previous.UNCHANGED), repr(UNCHANGED))
    operators['ops_common.py'] = common
    operators['build.py'] = replace(operators['build.py'], 'family-dashboard-expo-baseline:', 'family-dashboard-expo-finance-source:')
    operators['activate.py'] = replace(operators['activate.py'], 'expo-baseline', 'expo-finance-source', count=4)
    operators['post_readback.py'] = replace(operators['post_readback.py'], 'expo-baseline-55-', 'expo-finance-source-55-')
    ...
    for before, after in ((previous.OLD_MANIFEST, OLD_MANIFEST), (previous.OLD_ARCHIVE, OLD_ARCHIVE),
                          (previous.PARENT_IMAGE, PARENT_IMAGE), ('Wrong installed Expo journey segments archive', 'Wrong installed Expo private baseline archive'),
...
def freeze_config(path, access, generated):
    ...
    namespace = {'__name__': 'expo_finance_source_freeze_check', '__file__': str(access / 'finance-source-prepare-validation.py')}
...
def prepare(access, output, freeze=None):
    ...
    need(output.parent == access and re.fullmatch('expo-finance-source-tools-[a-z0-9-]+', output.name), 'New finance source output must be directly under access root')
