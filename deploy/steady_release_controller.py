...
class Controller(shared.Controller):
    def __init__(self, candidate, plan_sha256, *, runner=run, root=ROOT, releases=RELEASES):
        ...
        need(self.plan.get('schemaVersion') == 1 and self.plan.get('kind') == 'steady-release-plan', 'plan_kind')
        ...
        operator_names = ('steady_release_controller.py', 'steady_release_package.py', 'steady_release_data.py', 'steady_release_plan.py',
    ...
    def stage(self):
        ...
        value = {'planSha256': self.plan_sha, 'services': services, 'oldFiles': old, 'tests': counts,
                 'imageId': self.plan['imageId'], 'schemaBefore': [61, 9], 'schemaAfter': [61, 9],
    ...
    def backup_program(self):
        return ("from deploy import steady_release_data as steady\n"
    ...
    def _activate(self):
        ...
        release = self.releases / ('steady-61-' + stamp)
        ...
        report.update(completed=True, completedAt=datetime.now(timezone.utc).isoformat(), services=current,
                      preservation=after, stoppedBackup=before, householdTables=61, platformTables=9)
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit('steady_release_failed; preserve evidence and stopped state; no automatic restore') from None
