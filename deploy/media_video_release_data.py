"""Populated 73/9 preservation using the existing closed reader and backup core.

No DDL or initializer runs here. Both video tables, reminder states/receipts and every previous table remain in the complete logical snapshot.
"""
import re
...
snapshot = snapshot_current = video.snapshot_current
verify_restore = video.verify_restore
...
def _profile(value):
    ...
    for path, fingerprint in value['databases'].items():
        ...
        expected = (shared.migration.BASE_PLATFORM_TABLES | shared.migration.NEW_PLATFORM_TABLES
                    if platform else video.BASE_TABLES | video.NEW_TABLES)
        tables = {n for n in fingerprint['tables'] if not n.startswith('sqlite_')}
        shared.need(tables == expected and len(tables) == (9 if platform else 73), 'expected_73_9_profile')
...
SPEC = shared.DataSpec(_profile, snapshot_current, 'media-video-source-update-attempt')
