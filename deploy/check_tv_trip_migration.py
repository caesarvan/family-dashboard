"""Stopped complete-group 75/9 -> 77/9 migration using the frozen TV trip initializer.
...
from deploy import journey_finance_release_data as baseline
from deploy import assistant_trip_change_release_data as backup_core
from deploy import check_journey_finance_migration as previous

ROOT = Path(__file__).resolve().parents[1]
BASE_TABLES = previous.BASE_TABLES | previous.NEW_TABLES
NEW_TABLES = frozenset({'media_playback_journeys', 'media_playback_operations'})
NEW_INDEXES = frozenset()
SCHEMA_SHA256 = 'ef72f3dab990b6d849a038153da38e9810d3abc9b58384d76603dae049a576b9'
MODULE_SHA256 = '4766393a7b72df22ac7abeffaae35f714da902d00dc0e522fbd38d48455d8e14'  # Rebind after full API source freezes.
SOURCE_MODULES = ('media_trip_playback',)
...
def runtime_module(name):
    need(name in SOURCE_MODULES, 'unexpected_tv_trip_module')
    ...
    need(actual in allowed and actual.read_bytes() == expected.read_bytes(), 'tv_trip_module_import_changed')
    return module
...
def schema_definition():
    expected = shared.migration.checked_path(ROOT / 'media_trip_playback.py')
    ...
    module = modules['media_trip_playback']
    ...
    need(len(tables) == 2 and set(tables) == NEW_TABLES and len(indexes) == 0 and
         set(indexes) == NEW_INDEXES, 'unexpected_tv_trip_schema')
    with closing(sqlite3.connect(':memory:')) as con:
        ...
        con.execute('CREATE TABLE journey_workflows(id TEXT PRIMARY KEY)')
        con.execute('CREATE TABLE journey_routes(id TEXT PRIMARY KEY)')
    ...
    return {'profile': 'tv_trip77', 'sql': sql, 'sha256': SCHEMA_SHA256,
...
BEGIN_SPEC = backup_core.DataSpec(baseline.SPEC.profile, snapshot_baseline, 'tv-trip-migration-attempt')


def verify_current(value, schema):
    need({n for n in value['rows'] if not n.startswith('sqlite_')} == BASE_TABLES | NEW_TABLES,
         'expected_77_table_profile')
    need([r for r in value['schema'] if _new_object(r)] == schema['objects'], 'tv_trip_schema_changed')
    need(all(value['columns'][n] == schema['columns'][n] for n in NEW_TABLES), 'tv_trip_columns_changed')
...
def verify_addition(before, after, schema):
    ...
    return {'originalTablesPreserved': 75, 'newTables': 2, 'newTablesEmpty': True,
            'oldRowsSchemaAndSequencesPreserved': True}


def initialize_database(path, schema):
    need(schema_definition() == schema, 'tv_trip_source_changed')
    with closing(sqlite3.connect(path.as_uri() + '?mode=rw', uri=True, timeout=0)) as con:
        con.execute('PRAGMA trusted_schema=OFF')
        con.execute('PRAGMA foreign_keys=ON')
        con.execute('BEGIN IMMEDIATE')
        try:
            runtime_module('media_trip_playback').init_schema(con)
            need(con.in_transaction, 'initializer_committed_transaction')
            con.commit()
...
def evidence(root, proof, *, source_identity, plan_sha256, marker_sha256=MEMBERSHIP_MARKER_SHA256):
    ...
    need(attempt == {'kind': BEGIN_SPEC.attempt_kind, 'planSha256': plan_sha256,
        ...
        'sourceIdentitySha256': shared._digest(identity), 'markerSha256': marker_sha256}, 'tv_trip_attempt_binding')
...
def snapshot_current(root):
    """Read a complete populated 77/9 group; retain allocations, receipts and every previous record."""
...
def verify_rollback(proof, restored_root):
    """Read-only verification of full 75/9 rollback; never invalidate retained receipts."""
