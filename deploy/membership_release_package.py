...
def baseline_values(baseline=None):
    ...
    if baseline == 'tv-trip-r1-assistant-journey-status':
    ...
    if baseline == 'assistant-list-r1-tv-trip':
    ...
    if baseline == 'finance-flow-r1-journey-finance':
        ...
        return photos.KIND, photos.PARENT_IMAGE, photos.OLD_MANIFEST
    if baseline == 'discovery-r1-finance-flow':
        from deploy import build_finance_flow_release as photos
        return photos.KIND, photos.PARENT_IMAGE, photos.OLD_MANIFEST
    if baseline == 'local-photo-r1-discovery':
...
def runtime_files(files, *, baseline=None):
    ...
    if baseline in ('media-video-r1-local-photo', 'local-photo-r1-discovery', 'discovery-r1-finance-flow',
                    'finance-flow-r1-journey-finance', 'journey-finance-r1-media-date', 'media-date-r1-assistant-list', 'assistant-list-r1-tv-trip', 'tv-trip-r1-assistant-journey-status'):
...
def validate_maps(metadata, manifest, evidence, *, baseline=None):
    ...
    if baseline == 'tv-trip-r1-assistant-journey-status':
        ...
        need(len(non_expo) == photos.NON_EXPO_RUNTIME_COUNT
             ...
             'assistant journey status runtime differs from reviewed preservation boundary')
        for field, names in (('supplementalTestInputs', photos.SUPPLEMENTAL_INPUTS),
            ...
            if names:
                ...
                need(type(extra) is dict and not extra, 'explicit empty assistant journey status build inputs required')
            need(set(extra) == names and all(source.get(n) == h for n, h in extra.items()),
                 'assistant journey status supplemental build inputs differ')
    if baseline == 'assistant-list-r1-tv-trip':
        from deploy import build_tv_trip_release as photos
        non_expo = {n: h for n, h in metadata['runtimeFiles'].items() if not n.startswith(PREFIX)}
        preserved = {n: h for n, h in non_expo.items() if n not in photos.CHANGED_RUNTIME_FILES}
        need(len(non_expo) == photos.NON_EXPO_RUNTIME_COUNT
             ...
             and digest(encoded(preserved)) == photos.PRESERVED_RUNTIME_SHA256
             and all(non_expo.get(n) == h for n, h in photos.CHANGED_MODULES.items()),
             'TV trip runtime differs from reviewed preservation boundary')
        for field, names in (('supplementalTestInputs', photos.SUPPLEMENTAL_INPUTS),
                             ('additionalTestInputs', photos.ADDITIONAL_INPUTS)):
            extra = evidence.get(field)
            if names:
                ...
                need(type(extra) is dict and not extra, 'explicit empty TV trip build inputs required')
            need(set(extra) == names and all(source.get(n) == h for n, h in extra.items()),
                 'TV trip supplemental build inputs differ')
    if baseline == 'media-date-r1-assistant-list':
        from deploy import build_assistant_list_release as photos
        non_expo = {n: h for n, h in metadata['runtimeFiles'].items() if not n.startswith(PREFIX)}
        preserved = {n: h for n, h in non_expo.items() if n not in photos.CHANGED_RUNTIME_FILES}
    ...
    if baseline == 'finance-flow-r1-journey-finance':
        ...
        need(len(non_expo) == photos.NON_EXPO_RUNTIME_COUNT
             ...
             and len(preserved) == photos.PRESERVED_RUNTIME_COUNT
             and digest(encoded(preserved)) == photos.PRESERVED_RUNTIME_SHA256,
             'journey finance runtime differs from installed 110-file preservation baseline')
        need(all(non_expo.get(n) == h for n, h in photos.CHANGED_MODULES.items()),
             'journey finance changed modules differ from reviewed source')
        for field, names in (('supplementalTestInputs', photos.SUPPLEMENTAL_INPUTS),
                             ('additionalTestInputs', photos.ADDITIONAL_INPUTS)):
            extra = hash_map(evidence.get(field))
            need(set(extra) == names and all(source.get(n) == h for n, h in extra.items()),
                 'journey finance supplemental build inputs differ')
    if baseline == 'discovery-r1-finance-flow':
        from deploy import build_finance_flow_release as photos
        non_expo = {n: h for n, h in metadata['runtimeFiles'].items() if not n.startswith(PREFIX)}
        preserved = {n: h for n, h in non_expo.items() if n not in photos.CHANGED_RUNTIME_FILES}
        need(len(non_expo) == photos.NON_EXPO_RUNTIME_COUNT
             ...
             and len(preserved) == photos.PRESERVED_RUNTIME_COUNT
             and digest(encoded(preserved)) == photos.PRESERVED_RUNTIME_SHA256,
             'finance flow runtime differs from installed 111-file preservation baseline')
        need(all(non_expo.get(n) == h for n, h in photos.CHANGED_MODULES.items()),
             'finance flow changed modules differ from reviewed source')
        for field, names in (('supplementalTestInputs', photos.SUPPLEMENTAL_INPUTS),
                             ('additionalTestInputs', photos.ADDITIONAL_INPUTS)):
            extra = hash_map(evidence.get(field))
            need(set(extra) == names and all(source.get(n) == h for n, h in extra.items()),
                 'finance flow supplemental build inputs differ')
    if baseline == 'local-photo-r1-discovery':
        from deploy import build_discovery_release as photos
        non_expo = {n: h for n, h in metadata['runtimeFiles'].items() if not n.startswith(PREFIX)}
        preserved = {n: h for n, h in non_expo.items() if n not in photos.CHANGED_RUNTIME_FILES}
        need(len(non_expo) == photos.NON_EXPO_RUNTIME_COUNT
             ...
             and len(preserved) == photos.PRESERVED_RUNTIME_COUNT
             and digest(encoded(preserved)) == photos.PRESERVED_RUNTIME_SHA256,
             'discovery runtime differs from installed 110-file preservation baseline')
        need(all(non_expo.get(n) == h for n, h in photos.CHANGED_MODULES.items()),
             'discovery changed modules differ from reviewed source')
    if baseline == 'media-video-r1-local-photo':
        from deploy import build_local_photo_release as photos
        non_expo = {n: h for n, h in metadata['runtimeFiles'].items() if not n.startswith(PREFIX)}
        preserved = {n: h for n, h in non_expo.items() if n not in photos.CHANGED_RUNTIME_FILES}
